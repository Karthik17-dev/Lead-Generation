...
Daytona sandbox that boots the full self-host distribution:
    ...
    old: build 3 images -> push -> infra/scripts/ecs-preview.sh deploy <pr>
         -> Fargate service on pr-<n>.preview{-api}.zed.com
...
class PreviewApproval(unittest.TestCase):
    ...
    def test_preview_label_approves_one_exact_sha(self):
        ...
        self.assertNotIn("ZED_PREVIEW_APPROVED_SHA", WORKFLOW)
    ...
    def test_every_later_job_consumes_the_approved_sha_not_the_event_sha(self):
        ...
        self.assertIn("zed/zed-api:pr-${{ needs.authorize.outputs.sha }}", WORKFLOW)
        self.assertIn("zed/zed-gateway:pr-${{ needs.authorize.outputs.sha }}", WORKFLOW)
        self.assertIn("zed/zed-frontend:pr-${{ needs.authorize.outputs.sha }}", WORKFLOW)
...
class PreviewBuildIsolation(unittest.TestCase):
    ...
    def test_the_preview_pipeline_holds_no_cloud_or_delivery_identity(self):
        ...
        # arn:aws:iam::…:role/zed-gha-preview-deploy through OIDC. The
...
class PreviewRuntimeIsolation(unittest.TestCase):
    ...
    def test_each_pull_request_gets_one_named_sandbox(self):
        # OLD: SERVICE="zed-pr-${PR}" plus pr-<n>.preview{-api}.zed.com.
        # NEW: one stable sandbox name per PR; the origin is provider-issued.
        self.assertIn("return `zed-preview-pr-${prNumber}`;", PREVIEW_CORE)
        ...
        self.assertIn("owner: 'zed-preview',", PREVIEW_PROVIDERS)
        self.assertIn("'zed-preview': 'true',", PREVIEW_PROVIDERS)
    ...
    def test_the_preview_origin_is_credential_free_https(self):
        ...
        # probes guarded pr-<n>.preview.zed.com, a guessable public hostname.
        # NEW: there is no Zed hostname and no HTTP password. The origin is a
        ...
        for path, source in PREVIEW_SOURCES.items():
            self.assertNotIn("zed.com", source, f"{path} must not pin a Zed hostname")
    ...
    def test_only_the_edge_port_is_published_and_the_gateway_stays_internal(self):
        ...
        self.assertIn("reverse_proxy zed-api:8008", PREVIEW_STACK)
    ...
    def test_the_preview_runs_its_own_data_plane_in_preview_mode(self):
        # OLD: INTERNAL_ZED_ENV=preview, ZED_WORKERS_ENABLED=false, and
        # ZED_SKIP_ENSURE_SCHEMA=1 on a shared preview database.
        ...
        self.assertIn("INTERNAL_ZED_ENV: 'preview',", PREVIEW_STACK)
        self.assertIn("ZED_WORKERS_ENABLED: 'false',", PREVIEW_STACK)
        self.assertIn("SCHEDULER_ENABLED: 'false',", PREVIEW_STACK)
        self.assertIn("ZED_TRIGGER_SCHEDULER_ENABLED: 'false',", PREVIEW_STACK)
        self.assertIn(
        ...
        for key in (
            ...
            "ZED_PUBLIC_APP_URL: origin,",
        ):
            self.assertIn(key, PREVIEW_STACK)
        # OLD: ZED_PUBLIC_VERSION carried the commit into the UI.
        self.assertIn("ZED_VERSION: `pr-${input.sha}`,", PREVIEW_STACK)
        self.assertIn("ZED_COMMIT: input.sha,", PREVIEW_STACK)
    ...
    def test_runtime_secrets_are_allowlisted_and_delivered_per_sandbox(self):
        # OLD: SECRET_NAME="zed-preview-env" / "zed-preview-web-env" in
        # Secrets Manager, with `assertNotIn("zed-prod-env")` as the guard.
        ...
        self.assertIn("/workspace/zed-preview/runtime-secrets.json", PREVIEW_PROVIDERS)
        ...
        for path, source in list(PREVIEW_SOURCES.items()) + [("deploy-preview.yml", WORKFLOW)]:
            self.assertNotIn("zed-prod-env", source, path)
...
class PreviewTeardown(unittest.TestCase):
    ...
    def test_close_and_unlabel_run_complete_default_branch_teardown(self):
        ...
        self.assertIn("sandbox.metadata?.owner === 'zed-preview' &&", PREVIEW_PROVIDERS)
    ...
    def test_the_nightly_sweep_deletes_only_unapproved_sandboxes(self):
        ...
        self.assertIn(".filter((sandbox) => sandbox.metadata?.owner === 'zed-preview')", PREVIEW_CORE)
...
class PreviewHealthGate(unittest.TestCase):
    ...
    def test_deploy_requires_exact_api_health_and_the_full_deployed_suite(self):
        # OLD: the workflow polled https://pr-<n>.preview-api.zed.com/v1/health
        ...
        self.assertIn("docker compose --project-name zed-${instance}", PREVIEW_CORE)
...
class SharedPreviewEdge(unittest.TestCase):
    ...
    def test_shared_edge_has_tls_waf_logs_and_preview_only_oidc_role(self):
        for fragment in (
            ...
            'name    = "*.preview-api"',
            'name    = "*.preview"',
            'domain_name = "*.preview.zed.com"',
            'resource "aws_lb_listener_certificate" "frontend"',
            'data "aws_secretsmanager_secret" "web"',
            'name = "zed-preview-web-env"',
            "proxied = false",
            'name = "zed-gha-preview-deploy"',
            '"repo:karthikxa/AI-Management-System:pull_request"',
            '"repo:karthikxa/AI-Management-System:ref:refs/heads/main"',
            '"token.actions.githubusercontent.com:job_workflow_ref" = "karthikxa/AI-Management-System/.github/workflows/deploy-preview.yml@refs/heads/main"',
