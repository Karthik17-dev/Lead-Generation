...
SERVICE="zed-pr-${PR}"
FAMILY="$SERVICE"
API_TARGET_GROUP_NAME="zed-pr-${PR}"
WEB_TARGET_GROUP_NAME="zed-pr-${PR}-web"
HOST="pr-${PR}.preview-api.zed.com"
WEB_HOST="pr-${PR}.preview.zed.com"
SECRET_NAME="zed-preview-env"
WEB_SECRET_NAME="zed-preview-web-env"
EXECUTION_ROLE="arn:aws:iam::935064898258:role/zed-preview-exec"
TASK_ROLE="arn:aws:iam::935064898258:role/zed-preview-task"
LOG_GROUP="/ecs/zed-preview"
...
listener_arn() {
  local alb
  alb="$(aws_text aws elbv2 describe-load-balancers --region "$REGION" --names zed-preview-alb \
...
reconcile() {
  ...
  for service_arn in $(aws ecs list-services --region "$REGION" --cluster "$CLUSTER" \
    ...
    [[ "$service_name" =~ ^zed-pr-([1-9][0-9]*)$ ]] || continue
...
for service_arn in $active_services; do
  case "${service_arn##*/}" in
    zed-pr-*) active_count=$((active_count + 1)) ;;
  esac
done
...
VPC_ID="$(aws ec2 describe-subnets --region "$REGION" --filters Name=tag:Name,Values=zed-dev-private-* \
  --query 'Subnets[0].VpcId' --output text)"
SUBNETS="$(aws ec2 describe-subnets --region "$REGION" --filters Name=tag:Name,Values=zed-dev-private-* \
  --query 'Subnets[].SubnetId' --output text)"
SERVICE_SG="$(aws ec2 describe-security-groups --region "$REGION" --filters Name=group-name,Values=zed-preview-service \
...
common = [{"name": "ZED_ENV_JSON", "valueFrom": secret}]
...
task = {
    ...
    "containerDefinitions": [
        {
            ...
            "environment": [
                ...
                {"name": "INTERNAL_ZED_ENV", "value": "preview"},
                {"name": "ZED_COMMIT", "value": commit},
                {"name": "ZED_WORKERS_ENABLED", "value": "false"},
                {"name": "ZED_SKIP_ENSURE_SCHEMA", "value": "1"},
        ...
        },
        {
            ...
            "environment": [
                ...
                {"name": "ZED_COMMIT", "value": commit},
                {"name": "ZED_WORKERS_ENABLED", "value": "false"},
                {"name": "ZED_API_URL", "value": "http://127.0.0.1:8008"},
            ],
            "secrets": common,
        ...
        },
        {
            ...
            "environment": [
                ...
                {"name": "ZED_PUBLIC_APP_URL", "value": web_url},
                {"name": "NEXT_PUBLIC_URL", "value": web_url},
                {"name": "NEXT_PUBLIC_BACKEND_URL", "value": backend_url},
                {"name": "ZED_PUBLIC_BACKEND_URL", "value": backend_url},
                {"name": "BACKEND_URL", "value": backend_url},
                {
                    "name": "ZED_PUBLIC_VERSION",
                    "value": f"pr-{family.removeprefix('zed-pr-')}",
                },
                {"name": "WEB_PROTECTION_ENABLED", "value": "true"},
            ],
            "secrets": [{"name": "ZED_ENV_JSON", "valueFrom": web_secret}],
    ...
    "tags": [
        ...
        {"key": "PR", "value": family.removeprefix("zed-pr-")},
    ],
}
