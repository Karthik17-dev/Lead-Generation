...
describe('webDnsRecord', () => {
  test('renders the canonical Dev host and isolated ECS hosts elsewhere', () => {
    const targets = {
      dev: 'zed-dev-web-alb-123.us-west-2.elb.amazonaws.com',
      staging: 'zed-staging-web-alb-123.us-west-2.elb.amazonaws.com',
      prod: 'zed-prod-web-alb-123.eu-west-2.elb.amazonaws.com',
    };
    const expectedHosts = {
      dev: 'dev.zed.com',
      staging: 'staging-fe-ecs.zed.com',
      prod: 'prod-fe-ecs.zed.com',
    };
  ...
  test('selects canonical Dev only and preserves isolated staging and prod hosts', () => {
    const target = 'zed-dev-web-alb-123.us-west-2.elb.amazonaws.com';
    expect(webDnsRecord('dev', target).name).toBe('dev.zed.com');
    expect(webDnsRecord('staging', target).name).toBe('staging-fe-ecs.zed.com');
    expect(webDnsRecord('prod', target).name).toBe('prod-fe-ecs.zed.com');
  });
  ...
  test('converts the existing canonical Vercel A record to the ECS CNAME in place', async () => {
    ...
    globalThis.fetch = async (url, options = {}) => {
      ...
      return Response.json({
        ...
        result: {
          ...
          type: 'CNAME',
          name: 'dev.zed.com',
          content: 'zed-dev-web-alb-123.us-west-2.elb.amazonaws.com',
          proxied: true,
        },
      });
    };
    ...
    try {
      const result = await syncWebDns('dev', 'zed-dev-web-alb-123.us-west-2.elb.amazonaws.com');
      expect(result.action).toBe('updated');
      expect(requests).toHaveLength(2);
      expect(requests[0].url).toContain('name=dev.zed.com');
