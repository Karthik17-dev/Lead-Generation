  ...
  test('rejects an environment that points at another data plane', () => {
    expect(() =>
      renderWebEnvironment('staging', {
        ...profile('staging'),
        NEXT_PUBLIC_BACKEND_URL: 'https://dev-api.zed.com/v1',
      }),
    ).toThrow('NEXT_PUBLIC_BACKEND_URL must target https://staging-api.zed.com');
  });
  ...
  test('rejects a profile whose canonical app URL belongs to another environment', () => {
    expect(() =>
      renderWebEnvironment('dev', {
        ...profile('dev'),
        NEXT_PUBLIC_APP_URL: 'https://staging.zed.com',
      }),
    ).toThrow('NEXT_PUBLIC_APP_URL must target https://dev.zed.com');
  });
