...
Zed deploys the API, gateway, and frontend through GitHub Actions to Amazon
...
| --- | --- | --- | --- | --- |
| Preview | PR with `preview` label | `pr-<number>.preview-api.zed.com` | `pr-<number>.preview.zed.com` on ECS | Disabled |
| Dev | `main` | `dev-api.zed.com` | `dev.zed.com` on ECS | Disabled |
| Staging | `staging` | `staging-api.zed.com` | `staging-fe-ecs.zed.com` on ECS | `staging.zed.com` |
| Production | `prod` | `api.zed.com` | `prod-fe-ecs.zed.com` on ECS | `zed.com` |

Dev and previews are ECS-only. Staging and production retain their parallel
Vercel and `*-fe-ecs.zed.com` paths.
...
`ZED_ENV_JSON`. The frontend uses a separate `zed-<env>-web-env` secret.
Preview uses `zed-preview-web-env`. It excludes Edge Config and Vercel
credentials.
