...
This stack owns the dedicated `zed-dev-web` ECS service and its Cloudflare
...
The stack owns `https://dev.zed.com`. The Deploy Dev workflow also updates
this CNAME before it verifies the deployed frontend. Vercel is disabled for the
`main` branch.

The remote state owns the existing `dev.zed.com` Cloudflare record at
...
record or reintroduce the retired `dev-fe-ecs.zed.com` alias.
