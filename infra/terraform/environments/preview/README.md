...
- one ECS cluster (`zed-preview`);
- one HTTPS ALB with certificates for `*.preview-api.zed.com` and `*.preview.zed.com`;
...
- one task execution role that can read only `zed-preview-env` and `zed-preview-web-env`;
...
region `us-west-2`. It reads the existing dev VPC, `zed-preview-env` secret,
...
1. `https://pr-<PR>.preview-api.zed.com/v1/health` reports
   `environment=preview` and this PR's full commit SHA.
2. `https://pr-<PR>.preview.zed.com/api/health` reports the same SHA.
...
```bash
terraform import aws_ecs_cluster.preview zed-preview
terraform import aws_iam_role.execution zed-preview-exec
terraform import aws_iam_role.task zed-preview-task
terraform import aws_iam_role.github_preview_deploy zed-gha-preview-deploy
terraform import aws_cloudwatch_log_group.preview /ecs/zed-preview
```
...
The backend containers receive `zed-preview-env`. That secret contains
...
`zed-pr-N` service name. Never delete a resource without matching its PR tag
and name.

Before shared-root rollback, remove `preview` from every PR and verify zero
`zed-pr-*` services. Then run and review `terraform plan -destroy`. Destroy
