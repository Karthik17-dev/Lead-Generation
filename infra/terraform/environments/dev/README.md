...
| `dev.zed.com` (frontend) | Cloudflare (proxied) → ALB → **ECS Fargate** | `../dev-web` Terraform |
...
`zed-dev-env` Secrets Manager blob, which `main.tf` passes to the module as
`secrets_blob_arn`; ECS injects the whole document as `ZED_ENV_JSON` and the
...
`deploy-dev.yml` passes the `zed/zed-api:dev-<sha8>` tag the same run
