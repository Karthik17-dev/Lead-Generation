...
export function validatePreviewRuntimeSecrets(
  ...
  for (const [key, value] of Object.entries(secrets)) {
    if (key === 'ZED_GITHUB_APP_PRIVATE_KEY') {
...
export function buildPreviewComposeOverlay(
  reportPath: string,
  caddyfilePath = '/workspace/zed-preview/Caddyfile.preview',
): string {
...
export function applyPreviewEnvironment(
  ...
  const managedGitValues = [
    rawSecrets.ZED_GITHUB_APP_ID,
    rawSecrets.ZED_GITHUB_APP_PRIVATE_KEY,
    rawSecrets.ZED_GITHUB_APP_SLUG,
  ...
  Object.assign(runtime, {
    ...
    ZED_VERSION: `pr-${input.sha}`,
    ZED_COMMIT: input.sha,
    INTERNAL_ZED_ENV: 'preview',
    ENV_MODE: 'local',
    PUBLIC_URL: origin,
    ...
    ZED_PUBLIC_AUTH_METHODS: 'magic,password',
    ZED_PUBLIC_DISABLE_LANDING_PAGE: 'true',
    ZED_RESTRICT_ACCOUNT_CREATION: 'false',
    ZED_PUBLIC_RESTRICT_ACCOUNT_CREATION: 'false',
    ...
    ZED_GITHUB_APP_ID: rawSecrets.ZED_GITHUB_APP_ID ?? '',
    ZED_GITHUB_APP_PRIVATE_KEY:
      rawSecrets.ZED_GITHUB_APP_PRIVATE_KEY?.replace(/\r?\n/g, '\\n') ?? '',
    ZED_GITHUB_APP_SLUG: rawSecrets.ZED_GITHUB_APP_SLUG ?? '',
