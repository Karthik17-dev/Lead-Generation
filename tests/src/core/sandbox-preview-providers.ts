...
async function replaceExistingPlatinumPreview(
  ...
  const existing = (await allPlatinumPreviewSandboxes(api)).filter(
    (sandbox) =>
      sandbox.name === name &&
      sandbox.metadata?.owner === 'zed-preview' &&
...
export function platinumPreviewIdempotencyKey(input: {
  ...
  return `zed-preview-${input.prNumber}-${input.sha}-${input.runId}`;
}
...
export async function deployPlatinumPreview(
  ...
  try {
    ...
    await execPlatinum(api, sandboxId, ['bash', '-lc', 'mkdir -p /workspace/zed-preview']);
    await api.write(
      `${sandboxId}:/workspace/zed-preview/runtime-secrets.json`,
      `${JSON.stringify(input.secrets)}\n`,
      '0600',
    );
    await api.write(
      `${sandboxId}:/workspace/run-zed-preview.sh`,
    ...
    const launch = await execPlatinum(api, sandboxId, [
      'bash',
      '-lc',
      'setsid -f /workspace/run-zed-preview.sh >/workspace/zed-preview/bootstrap.log 2>&1 </dev/null',
    ]);
    ...
    const exitCode = await observePlatinumWorker({
      ...
      checkExitCode: async () => {
        const status = await statPlatinum(api, sandboxId, '/workspace/zed-preview/zed-preview.exit', 1);
        if (!status) return null;
        const bytes = await api.read(
          sandboxId,
          '/workspace/zed-preview/zed-preview.exit',
          undefined,
          undefined,
          1,
        );
      ...
      statLog: () => statPlatinum(api, sandboxId, '/workspace/zed-preview/zed-preview.log', 1),
      readLog: (offset, limit) =>
        api.read(
          sandboxId,
          '/workspace/zed-preview/zed-preview.log',
          offset,
...
async function replaceExistingDaytonaPreview(
  ...
  if (!existing) return;
  if (
    existing.labels?.['zed-preview'] !== 'true' ||
    existing.labels?.['zed-preview-pr'] !== String(prNumber)
  ) {
  ...
  try {
    ...
    sandbox = await waitForDaytonaSandbox(
      ...
      await createDaytonaSandbox(api, {
        ...
        labels: {
          'zed-preview': 'true',
          'zed-preview-pr': String(input.prNumber),
          'zed-preview-repository': input.repository,
          'zed-preview-git-sha': input.sha,
          'zed-preview-run-id': input.runId,
        },
      }),
    );
    const marker = await executeDaytona(
      api,
      sandbox,
      'test -s /workspace/.zed-ci-warm-ready && ! pgrep -x dockerd >/dev/null && ! pgrep -x containerd >/dev/null',
      30,
    );
    ...
    const secretsUpload = await executeDaytona(
      ...
      sandbox,
      encodedFileCommand(
        '/workspace/zed-preview/runtime-secrets.json',
        `${JSON.stringify(input.secrets)}\n`,
      ),
      60,
    );
    ...
    const exitCode = await observePlatinumWorker({
      ...
      statLog: () =>
        statRemoteLog(api, sandbox!, '/workspace/zed-preview/zed-preview.log', 'preview'),
      readLog: (offset, limit) =>
        readRemoteLog(
          api,
          sandbox!,
...
export async function teardownDaytonaPreview(input: {
  ...
  if (!sandbox) return 0;
  if (
    sandbox.labels?.['zed-preview'] !== 'true' ||
    sandbox.labels?.['zed-preview-pr'] !== String(input.prNumber)
  ) {
...
export function daytonaPreviewLabelsFilter(): string {
  return JSON.stringify({ 'zed-preview': 'true' });
}
...
export async function reconcileDaytonaPreviews(input: {
  ...
  const records = sandboxes.map((sandbox) => ({
    id: sandbox.id,
    metadata: {
      owner: sandbox.labels?.['zed-preview'] === 'true' ? 'zed-preview' : '',
      pr_number: sandbox.labels?.['zed-preview-pr'],
      git_sha: sandbox.labels?.['zed-preview-git-sha'],
    },
  }));
