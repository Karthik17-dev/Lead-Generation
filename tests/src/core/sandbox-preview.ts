...
export function buildPreviewBootstrapScript(input: {
  ...
  const state = '/workspace/zed-preview';
  const instanceDir = `${state}/self-host/${instance}`;
  const compose = `docker compose --project-name zed-${instance} --env-file ${instanceDir}/.env -f ${instanceDir}/docker-compose.yml -f ${state}/docker-compose.preview.yml`;
  return `#!/usr/bin/env bash
set -euo pipefail

ROOT=/workspace/suna
STATE=${state}
LOG="$STATE/zed-preview.log"
STATUS="$STATE/zed-preview.exit"
PHASE="$STATE/zed-preview.phase"
SECRETS="$STATE/runtime-secrets.json"
export HOME=/root
export CI=1
export ZED_SELF_HOST_CONFIG_DIR="$STATE/self-host"
...
finish() {
  local code="$1"
  set +e
  tar -czf /workspace/zed-test-results.tar.gz -C "$ROOT" tests/test-results
  printf '%s\n' "$code" > "$STATUS"
}
...
printf 'stack\n' > "$PHASE"
${compose} pull --policy always frontend zed-api llm-gateway preview-edge mailpit
...
export function previewSandboxName(prNumber: number): string {
  ...
  return `zed-preview-pr-${prNumber}`;
}
...
export function selectStalePreviewSandboxIds(
  ...
  return sandboxes
    .filter((sandbox) => sandbox.metadata?.owner === 'zed-preview')
    .filter((sandbox) => {
