...
export function resolveTargetSmokeConfig(
  ...
  const api = normalizedUrl(
    environment.KE2E_API_URL ?? 'https://staging-api.zed.com/v1',
    'KE2E_API_URL',
  );
  const web = normalizedUrl(
    environment.E2E_BASE_URL ?? 'https://staging.zed.com',
    'E2E_BASE_URL',
  );
  const gateway = normalizedUrl(
    environment.KE2E_GATEWAY_URL ?? 'https://gateway-staging.zed.com',
    'KE2E_GATEWAY_URL',
  );
  ...
  if (target === 'preview') {
    ...
    if (api.hostname !== 'staging-api.zed.com' || api.pathname !== '/v1') {
      throw new Error(`target smoke requires https://staging-api.zed.com/v1; received ${api}`);
    }
    if (web.hostname !== 'staging.zed.com' || web.pathname !== '/') {
      throw new Error(`target smoke requires https://staging.zed.com; received ${web}`);
    }
    if (gateway.hostname !== 'gateway-staging.zed.com' || gateway.pathname !== '/') {
      throw new Error(
        `target smoke requires https://gateway-staging.zed.com; received ${gateway}`,
      );
    }
  }
