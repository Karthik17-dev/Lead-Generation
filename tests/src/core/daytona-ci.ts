...
export function daytonaSnapshotName(lockHash: string): string {
  ...
  return `zed-ci-daytona-${DAYTONA_CI_SNAPSHOT_VERSION}-${lockHash.slice(0, 16)}`;
}

export function daytonaBaseSnapshotName(lockHash: string): string {
  if (!/^[a-f0-9]{64}$/i.test(lockHash)) throw new Error(`invalid lockfile hash: ${lockHash}`);
  return `zed-ci-daytona-${DAYTONA_CI_BASE_SNAPSHOT_VERSION}-${lockHash.slice(0, 16)}-base`;
}

export function daytonaWorkerName(runId: string, runAttempt: string): string {
  return `zed-ci-${runId}-${runAttempt}`.slice(0, 64);
}
...
export function buildDaytonaWorkerRequest(input: {
  ...
  return {
    ...
    labels: {
      'zed-ci': 'true',
      'zed-ci-run-id': input.runId,
      'zed-ci-run-attempt': input.runAttempt,
      'zed-ci-repository': input.repository,
      'zed-ci-git-sha': input.sha,
    },
  };
}
...
export function isExactDaytonaWarmBuilder(
  ...
  return (
    ...
    sandbox.labels?.['zed-ci'] === 'true' &&
    sandbox.labels?.['zed-ci-run-id'] === input.runId &&
    sandbox.labels?.['zed-ci-run-attempt'] === input.runAttempt
  );
}
...
async function waitForWarmSnapshotOwner(
  ...
  while (Date.now() < deadline) {
    ...
    if (state !== lastBuilderState) {
      console.log(
        `[daytona-ci] warm_builder=${builderName} owner=${builder.labels?.['zed-ci-run-id'] ?? 'unknown'} state=${state}`,
      );
...
export async function ensureWarmSnapshot(
  ...
  try {
    ...
    const marker = await execute(
      api,
      builder,
      'test -s /workspace/.zed-ci-warm-ready && ! pgrep -x dockerd >/dev/null && ! pgrep -x containerd >/dev/null && test ! -S /var/run/docker.sock && cat /workspace/.zed-ci-warm-ready',
      30,
    );
...
export async function downloadArtifacts(
  ...
  const url = `${toolboxBase(sandbox)}/files/download?path=${encodeURIComponent('/workspace/zed-test-results.tar.gz')}`;
...
export async function runDaytonaCi(input: DaytonaCiInput): Promise<number> {
  ...
  try {
    ...
    const uploaded = await execute(
      api,
      sandbox,
      base64Command(workerScript, '/workspace/run-zed-tests.sh'),
      60,
    );
    if (uploaded.exitCode !== 0)
    ...
    const launched = await execute(
      api,
      sandbox,
      'setsid -f /workspace/run-zed-tests.sh >/workspace/daytona-bootstrap.log 2>&1 </dev/null',
      30,
    );
    if (launched.exitCode !== 0)
    ...
    const exitCode = await observePlatinumWorker({
      ...
      pollMs: POLL_MS,
      checkExitCode: () =>
        readRemoteExitCode(api, sandbox!, '/workspace/zed-test.exit', 'worker'),
      statLog: () => statRemoteLog(api, sandbox!, '/workspace/zed-test.log', 'worker'),
      readLog: (offset, limit) =>
        readRemoteLog(
          api,
          sandbox!,
          '/workspace/zed-test.log',
          offset,
          Math.min(limit, LOG_CHUNK_BYTES),
          'worker',
        ),
...
export function isExactDaytonaCiSandbox(
  ...
  return (
    ...
    sandbox.labels?.['zed-ci'] === 'true' &&
    sandbox.labels?.['zed-ci-run-id'] === runId &&
    sandbox.labels?.['zed-ci-run-attempt'] === runAttempt
  );
}
