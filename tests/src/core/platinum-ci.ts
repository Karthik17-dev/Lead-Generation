...
const WARM_READY_COMMAND =
  'test -s /workspace/.zed-ci-warm-ready && ! pgrep -x dockerd >/dev/null && test ! -S /var/run/docker.sock';
...
export function platinumTemplateName(lockHash: string): string {
  ...
  return `zed-ci-${PLATINUM_CI_TEMPLATE_VERSION}-${lockHash.slice(0, 16)}`;
}

export function platinumBaseTemplateName(lockHash: string): string {
  if (!/^[a-f0-9]{64}$/i.test(lockHash)) throw new Error(`invalid lockfile hash: ${lockHash}`);
  return `zed-ci-${PLATINUM_CI_BASE_TEMPLATE_VERSION}-${lockHash.slice(0, 16)}-base`;
}
...
function platinumWarmEntrypoint(): string {
  return [
    'set -eux',
    'exec >>/workspace/zed-template-warm.log 2>&1',
    'cd /workspace/suna',
    'rm -f /workspace/.zed-ci-warm-ready /var/run/docker.pid /var/run/docker.sock',
    'modprobe overlay',
    'modprobe bridge',
    ...
    'dockerd --host=unix:///var/run/docker.sock >/workspace/zed-template-dockerd.log 2>&1 &',
    ...
    'docker image ls -q | sort -u | wc -l > /workspace/.zed-ci-warm-ready',
    "grep -Eq '^[1-9][0-9]*$' /workspace/.zed-ci-warm-ready",
...
export function buildPlatinumTemplateSpec(input: {
  ...
  return {
    ...
    steps: [
      ...
      { op: 'env', key: 'ZED_PLATINUM_CI_TEMPLATE', value: name },
    ],
...
export function buildPlatinumWorkerRequest(input: {
  ...
  return {
    name: `zed-ci-${input.runId}-${input.runAttempt}`.slice(0, 64),
...
echo "[${provider}-ci] supabase_prestarted=1"`
  ...
  const providerLogs =
    provider === 'daytona'
      ...
      : '/workspace/zed-bootstrap.log /workspace/zed-template-warm.log /workspace/zed-template-dockerd.log';
  return `#!/usr/bin/env bash
set -euo pipefail

ROOT=/workspace/suna
LOG=/workspace/zed-test.log
STATUS=/workspace/zed-test.exit
ARTIFACT=/workspace/zed-test-results.tar.gz
export HOME=/root
export CI=1
${input.skipSdkPackageTests ? 'export ZED_PACKAGE_SKIP_SDK_TESTS=1' : ''}
...
export function platinumWorkerLaunchCommand(): string {
  return 'setsid -f /workspace/run-zed-tests.sh >/workspace/zed-bootstrap.log 2>&1 </dev/null';
}
...
export function selectOutstandingPlatinumSandboxIds(
  ...
  const expectedName = `zed-ci-${runId}-${runAttempt}`.slice(0, 64);
  return sandboxes
    .filter(
      (sandbox) =>
        sandbox.name === expectedName &&
        sandbox.metadata?.owner === 'zed-ci' &&
...
export async function ensureTemplate(
  ...
  const queued = await api.json<PlatinumTemplate>('/v1/templates/from-spec', {
    method: 'POST',
    headers: { 'idempotency-key': `zed-ci-template-${spec.name}` },
...
export async function ensureWarmTemplate(
  ...
  const derived = await api.json<PlatinumTemplate>(`/v1/templates/${base.id}/derive`, {
    method: 'POST',
    headers: { 'idempotency-key': `zed-ci-template-${name}` },
...
export async function waitForWarmSandbox(
  ...
  while (Date.now() < deadline) {
    try {
      if (await stat(api, sandboxId, '/workspace/.zed-ci-warm-ready', 1)) {
        ...
        const marker = new TextDecoder().decode(
          await api.read(sandboxId, '/workspace/.zed-ci-warm-ready', undefined, undefined, 1),
        ).trim();
  ...
  try {
    warmLog = new TextDecoder().decode(
      await api.read(sandboxId, '/workspace/zed-template-warm.log', undefined, undefined, 1),
    );
  } catch {
...
async function readWorkerExitCode(
  ...
  const result = await exec(api, sandboxId, [
    'bash',
    '-lc',
    'if [[ -f /workspace/zed-test.exit ]]; then cat /workspace/zed-test.exit; else exit 3; fi',
  ], true);
...
async function readWorkerExitCodeFile(
  ...
  const status = await stat(api, sandboxId, '/workspace/zed-test.exit', 1);
  if (!status) return null;
  const bytes = await api.read(sandboxId, '/workspace/zed-test.exit', undefined, undefined, 1);
...
async function streamWorker(
  ...
  return observePlatinumWorker({
    ...
    statLog: () => stat(api, sandboxId, '/workspace/zed-test.log', 1),
    readLog: (offset, limit) =>
      api.read(sandboxId, '/workspace/zed-test.log', offset, limit, 1),
  });
}
...
export async function downloadArtifacts(
  ...
  if (!(await stat(api, sandboxId, '/workspace/zed-test-results.tar.gz'))) {
    throw new Error('Platinum worker did not produce the required test-results artifact');
  }
  const bytes = await api.read(sandboxId, '/workspace/zed-test-results.tar.gz');
...
export async function runPlatinumCi(input: PlatinumCiInput): Promise<number> {
  ...
  try {
    ...
    const created = await api.json<PlatinumSandbox>(
      ...
      {
        method: 'POST',
        headers: { 'idempotency-key': `zed-ci-${input.runId}-${input.runAttempt}` },
    ...
    await api.write(`${sandboxId}:/workspace/run-zed-tests.sh`, workerScript, '0755');
