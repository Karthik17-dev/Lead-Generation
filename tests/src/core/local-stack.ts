...
export const LOCAL_TEST_PROFILE_HEADER = "x-zed-local-test-profile";
...
export function localMigrationPlan(
  ...
  return {
    command: ["pnpm", "--filter", "@zed/db", "migrate:local"],
    cwd: topology.root,
    env: {
      ...process.env,
  ...
  const web = Bun.spawn(
    ["pnpm", "--filter", "Zed-Computer-Frontend", "dev"],
    {
      ...
      env: {
        ...
        ZED_API_PROXY_TARGET: topology.apiUrl.replace(/\/v1$/, ""),
        NEXT_PUBLIC_BACKEND_URL: topology.apiUrl,
        ZED_PUBLIC_BACKEND_URL: topology.apiUrl,
        BACKEND_URL: topology.apiUrl,
        SUPABASE_URL: API_URL,
        NEXT_PUBLIC_SUPABASE_URL: API_URL,
        ZED_PUBLIC_SUPABASE_URL: API_URL,
        SUPABASE_ANON_KEY: ANON_KEY,
        NEXT_PUBLIC_SUPABASE_ANON_KEY: ANON_KEY,
        ZED_PUBLIC_SUPABASE_ANON_KEY: ANON_KEY,
        NEXT_PUBLIC_APP_URL: webUrl,
        ZED_PUBLIC_APP_URL: webUrl,
  ...
  const api = apiWasHealthy
    ...
    : Bun.spawn(["bun", "--no-env-file", "run", "src/index.ts"], {
        ...
        env: {
          ...
          ZED_LOCAL_DEV: "1",
          ZED_LOCAL_TEST_PROFILE: "1",
          PORT: String(apiPort),
          ZED_APPS_LOCAL: "true",
          ZED_APPS_LOCAL_PORT: String(apiPort),
          ZED_URL: topology.apiUrl.replace(/\/v1$/, ""),
          NEXT_PUBLIC_BACKEND_URL: topology.apiUrl,
          ZED_PUBLIC_BACKEND_URL: topology.apiUrl,
          BACKEND_URL: topology.apiUrl,
          ...
          ...(options.supabase.MAILPIT_URL
            ...
            : {}),
          ZED_MARKETPLACE_EXTERNAL_ENABLED: "0",
          ZED_MODEL_CATALOG_LIVE_ENABLED: "0",
          ZED_MODEL_PRICING_LIVE_ENABLED: "0",
        },
        stdin: "ignore",
        stdout: "inherit",
        stderr: "inherit",
  ...
  const gateway = gatewayWasHealthy
    ...
    : Bun.spawn(["bun", "run", "src/main.ts"], {
        ...
        env: {
          ...
          ZED_API_URL: topology.apiUrl.replace(/\/v1$/, ""),
          GATEWAY_INTERNAL_TOKEN: gatewayToken,
