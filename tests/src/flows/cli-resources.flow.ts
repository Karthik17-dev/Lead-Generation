...
flow(
  ...
  async (ctx) => {
    ...
    try {
      await ctx.step('zed projects ls --json lists the run-owned project', async () => {
      ...
      await ctx.step('zed projects info <id> returns the exact project', async () => {
      ...
      await ctx.step('zed projects link and unlink write and remove .zed/link.json', async () => {
        requireExit(
        ...
        requireExit(await sandbox.run(['projects', 'link', project.id]), 0, 'zed projects link');
        const link = JSON.parse(sandbox.readFile('.zed/link.json')) as { project_id: string };
        if (link.project_id !== project.id) throw new Error(`link points at ${link.project_id}`);
        requireExit(await sandbox.run(['projects', 'unlink']), 0, 'zed projects unlink');
        if (sandbox.exists('.zed/link.json')) throw new Error('projects unlink left link.json behind');
      });

      await ctx.step('zed projects open prints the dashboard URL without changing state', async () => {
        const result = await sandbox.run(['projects', 'open', project.id]);
        requireExit(result, 0, 'zed projects open');
        if (!result.stdout.includes(`/projects/${project.id}`)) {
          throw new Error(`projects open printed the wrong URL: ${result.stdout}`);
        }
      });

      await ctx.step('zed projects rm archives the project through the real DELETE route', async () => {
        const result = await sandbox.run(['projects', 'rm', project.id, '--yes']);
        requireExit(result, 0, 'zed projects rm');
...
flow(
  ...
  async (ctx) => {
    ...
    try {
      await ctx.step('zed secrets set stores one value and secrets ls returns metadata only', async () => {
        requireExit(
      ...
      await ctx.step('zed env pull writes a names-only dotenv skeleton', async () => {
        requireExit(
          await sandbox.run([
            'env',
            'pull',
      ...
      await ctx.step('zed env push uploads every non-empty dotenv pair', async () => {
      ...
      await ctx.step('zed secrets unset removes both values', async () => {
        requireExit(
          await sandbox.run([
            'secrets',
            'unset',
...
flow(
  ...
  async (ctx) => {
    ...
    try {
      let sessionId = '';
      await ctx.step('zed sessions new creates a session and returns its id', async () => {
      ...
      await ctx.step('zed sessions ls and info read the same session', async () => {
      ...
      await ctx.step('zed sessions open prints the exact dashboard session URL', async () => {
        const result = await sandbox.run(['sessions', 'open', sessionId, '--project', project.id]);
        requireExit(result, 0, 'zed sessions open');
        if (!result.stdout.includes(`/sessions/${sessionId}`)) {
          throw new Error(`sessions open printed the wrong URL: ${result.stdout}`);
        }
      });

      await ctx.step('zed sessions restart calls the real restart route', async () => {
        const result = await sandbox.run(['sessions', 'restart', sessionId, '--project', project.id], {
          timeoutMs: 120_000,
        });
        requireExit(result, 0, 'zed sessions restart');
      });

      await ctx.step('zed sessions rm deletes the session and the next info fails', async () => {
        requireExit(
        ...
        requireExit(missing, 1, 'zed sessions info after delete');
      });
    } finally {
      sandbox.dispose();
    }
...
flow(
  ...
  async (ctx) => {
    ...
    try {
      await ctx.step('zed triggers ls and info return the server trigger', async () => {
      ...
      await ctx.step('zed triggers fire starts or queues the configured trigger', async () => {
        const result = await sandbox.run(['triggers', 'fire', slug, '--project', project.id], {
          timeoutMs: 120_000,
        });
        requireExit(result, 0, 'zed triggers fire');
        if (!/(Fired|Queued)/.test(result.stdout)) {
          throw new Error(`triggers fire output: ${result.stdout}`);
        }
      });

      await ctx.step('zed triggers enable and disable edit the local manifest source of truth', async () => {
        requireExit(
          await sandbox.run(['init', 'local-trigger', '-y', '--no-git']),
          0,
          'zed init for local trigger',
        );
        ...
        requireExit(await sandbox.run(['triggers', 'enable', 'daily']), 0, 'zed triggers enable');
        requireExit(await sandbox.run(['triggers', 'disable', 'daily']), 0, 'zed triggers disable');
        const manifest = sandbox.readFile('zed.yaml');
