 ...
 *
 * These routes are how a long-lived sandbox stops running a `zed` CLI older
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('the manifest alone is enough to decide whether to download', async () => {
      ...
      // apps/cli/dist/zed must null it rather than fail the whole manifest.
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('authed download → 200 with the overlay files and the manifest hash', async () => {
      ...
      // The overlay is what teaches every agent the platform. If zed-system
      // is missing, a sandbox that reconciles is worse off than one that did not.
      if (!body.files.some((f) => f.path === 'zed-system/SKILL.md')) {
        throw new Error('the overlay must carry zed-system/SKILL.md');
      }
      for (const f of body.files) {
        if (!f.path.startsWith('zed-')) {
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step('a converged sandbox transfers nothing — matching ETag → 304', async () => {
      ...
      if (sha === null) {
        // A local profile that never built apps/cli/dist/zed. The contract in
    ...
    await ctx.step('a stale ETag gets the binary, with a length and a digest header', async () => {
      ...
      if (r.header('x-zed-cli-sha256') !== body.cli_sha256) {
