...
describe('Platinum CI worker plan', () => {
  ...
  test('uses one content-addressed template for one lockfile', () => {
    ...
    expect(platinumTemplateName(lockHash)).toBe('zed-ci-v14-bbbbbbbbbbbbbbbb');
    expect(platinumBaseTemplateName(lockHash)).toBe('zed-ci-v11-bbbbbbbbbbbbbbbb-base');
    const spec = buildPlatinumTemplateSpec({
      lockHash,
      repository: 'karthikxa/AI-Management-System',
      cacheSha: sha,
    });
    ...
    expect(spec.entrypoint).toContain('.zed-ci-warm-ready');
    ...
    expect(buildPlatinumWarmTemplateRequest(lockHash)).toEqual({
      ...
      capture_condition: {
        cmd: 'test -s /workspace/.zed-ci-warm-ready && ! pgrep -x dockerd >/dev/null && test ! -S /var/run/docker.sock',
        timeoutSec: 2_700,
      },
      default_cpu: 8,
    ...
    expect(
      selectOutstandingPlatinumSandboxIds(
        [
          {
            id: 'exact',
            name: 'zed-ci-31289428402-1',
            metadata: { owner: 'zed-ci', run_id: '31289428402' },
          },
          {
            id: 'other-attempt',
            name: 'zed-ci-31289428402-2',
            metadata: { owner: 'zed-ci', run_id: '31289428402' },
          },
          {
            id: 'not-ci-owned',
  ...
  test('propagates the full-run SDK de-duplication flag into the package worker', () => {
    ...
    expect(fullPackageScript).toContain('export ZED_PACKAGE_SKIP_SDK_TESTS=1');
    ...
    expect(standalonePackageScript).not.toContain('export ZED_PACKAGE_SKIP_SDK_TESTS=1');
  });

  test('detaches the worker with the Platinum-supported setsid contract', () => {
    const command = platinumWorkerLaunchCommand();
    expect(command).toContain('setsid -f /workspace/run-zed-tests.sh');
  ...
  test('reuses the exact ready or building content-addressed template', () => {
    expect(selectReusablePlatinumTemplate([
      { id: 'failed', name: 'zed-ci-v2-other', state: 'failed' },
      { id: 'ready', name: 'zed-ci-v2-target', state: 'ready' },
    ], 'zed-ci-v2-target')?.id).toBe('ready');
    expect(selectReusablePlatinumTemplate([
      { id: 'failed', name: 'zed-ci-v2-target', state: 'failed' },
    ], 'zed-ci-v2-target')).toBeNull();
  });
