...
describe('Daytona CI worker plan', () => {
  ...
  test('uses one content-addressed warm snapshot for one lockfile', () => {
    ...
    expect(daytonaSnapshotName(lockHash)).toBe('zed-ci-daytona-v4-bbbbbbbbbbbbbbbb');
    expect(daytonaBaseSnapshotName(lockHash)).toBe('zed-ci-daytona-v3-bbbbbbbbbbbbbbbb-base');
  });
  ...
  test('pre-pulls local-stack images before capturing the warm snapshot', () => {
    ...
    expect(script).toContain('.zed-ci-warm-ready');
  ...
  test('creates an explicitly disposable Daytona worker', () => {
    expect(
      buildDaytonaWorkerRequest({
        snapshot: 'zed-ci-daytona-v1-hash',
        target: 'us',
        repository: 'karthikxa/AI-Management-System',
        sha,
        runId: '31320717706',
      ...
      labels: {
        'zed-ci': 'true',
        'zed-ci-run-id': '31320717706',
        'zed-ci-run-attempt': '2',
        'zed-ci-repository': 'karthikxa/AI-Management-System',
        'zed-ci-git-sha': sha,
      },
    });
  });
  ...
  test('gives one shared warm builder an exact lane owner', () => {
    const request = buildDaytonaWarmBuilderRequest({
      snapshot: 'zed-ci-daytona-v1-base',
      target: 'us',
      repository: 'karthikxa/AI-Management-System',
      sha,
      runId: '31320717706-browser',
      runAttempt: '2',
      builderName: 'zed-ci-daytona-v1-bbbbbbbbbbbbbbbb-builder',
    });

    expect(request.name).toBe('zed-ci-daytona-v1-bbbbbbbbbbbbbbbb-builder');
    expect(request.labels).toMatchObject({
      'zed-ci-run-id': '31320717706-browser',
      'zed-ci-run-attempt': '2',
    });
    expect(
  ...
  test('reconciles a lost sandbox create response by exact name', async () => {
    ...
    expect(requests).toEqual([
      ...
      'GET https://app.daytona.io/api/sandbox/zed-ci-run-1',
    ]);
  });
  ...
  test('post cleanup deletes only the exact labelled worker', async () => {
    ...
    vi.stubGlobal('fetch', async (input: string | URL | Request, init?: RequestInit) => {
      ...
      return Response.json({
        id: 'sandbox-id',
        name: 'zed-ci-31320717706-1',
        state: 'started',
        labels: {
          'zed-ci': 'true',
          'zed-ci-run-id': '31320717706',
          'zed-ci-run-attempt': '1',
        },
      });
    });
    ...
    expect(requests).toEqual([
      'GET https://app.daytona.io/api/sandbox/zed-ci-31320717706-1',
