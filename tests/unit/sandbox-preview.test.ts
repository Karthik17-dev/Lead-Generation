...
describe('provider-neutral preview lifecycle', () => {
  it('uses one stable sandbox name per pull request', () => {
    expect(previewSandboxName(6337)).toBe('zed-preview-pr-6337');
  });
  ...
  it('uses a new Platinum idempotency key for each deployment run', () => {
    ...
    ).toBe(`zed-preview-6337-${'a'.repeat(40)}-31431634153`);
    expect(
      platinumPreviewIdempotencyKey({
        prNumber: 6337,
        sha: 'a'.repeat(40),
  ...
  it('encodes the Daytona preview ownership filter as JSON', () => {
    expect(daytonaPreviewLabelsFilter()).toBe('{"zed-preview":"true"}');
  });
  ...
  it('boots the exact self-host distribution and runs the canonical deployed suite', () => {
    ...
    expect(script).toContain('/workspace/zed-test-results.tar.gz');
    expect(script).toContain('zed-preview.exit');
  ...
  it('selects only stale or unlabeled preview sandboxes for teardown', () => {
    const sandboxes = [
      { id: 'keep', metadata: { owner: 'zed-preview', pr_number: '10', git_sha: 'a' } },
      { id: 'stale', metadata: { owner: 'zed-preview', pr_number: '10', git_sha: 'b' } },
      { id: 'closed', metadata: { owner: 'zed-preview', pr_number: '11', git_sha: 'c' } },
      { id: 'ci', metadata: { owner: 'zed-ci', pr_number: '11', git_sha: 'c' } },
    ];
