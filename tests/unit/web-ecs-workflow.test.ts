...
describe('web ECS migration', () => {
  it('maps --service web to the dedicated cluster, container, and secret', () => {
    const output = execFileSync(
      ...
      [
        '-c',
        'source infra/scripts/ecs-deploy.sh; SERVICE_PREFIX=zed-dev; SECRET_NAME=zed-dev-env; configure_service_coordinates web; printf "%s|%s|%s|%s|%s" "$CLUSTER" "$SERVICE" "$CONTAINER" "$SECRET_NAME" "$VERSION_ENV_NAME"',
      ],
      {
        cwd: root,
        encoding: 'utf8',
        env: { ...process.env, ZED_ECS_DEPLOY_LIB: '1' },
      },
    );

    expect(output).toBe(
      'zed-dev-web|zed-dev-web|web|zed-dev-web-env|ZED_PUBLIC_VERSION',
    );
  });
  ...
  it('deploys the immutable frontend image and encrypted Dev profile to ECS', () => {
    ...
    expect(workflow).toContain('NEXT_PUBLIC_ZED_COMMIT: ${{ github.sha }}');
    ...
    expect(workflow).toContain('base=https://dev.zed.com');
    ...
    expect(workflow).toContain('zed:${WEB_PROTECTION_PASSWORD}x');
    expect(workflow).toContain('cookie_only=');
    expect(workflow).toContain('[ "$vercel_headers" = 0 ]');
    expect(workflow).not.toContain('dev-fe-ecs.zed.com');
  ...
  it('defines the Dev web service and canonical DNS record', () => {
    ...
    expect(variables).toContain('Manage the canonical dev.zed.com CNAME');
  ...
  it('defines isolated staging and production services with environment-specific capacity', () => {
    ...
    expect(staging).toContain('name = "zed-staging-web"');
    expect(staging).toContain('name = "zed-staging-web-env"');
    expect(staging).toContain('staging-fe-ecs');
    expect(staging).toContain('min_capacity     = 1');
    expect(staging).toContain('max_capacity     = 4');
    expect(stagingVariables).toContain('never manages staging.zed.com');

    expect(prod).toContain('name = "zed-prod-web"');
    expect(prod).toContain('name = "zed-prod-web-env"');
    ...
    expect(prodVariables).toContain('never manages zed.com');
  });
  ...
  it('deploys staging and production ECS frontends without replacing canonical Vercel hosts', () => {
    ...
    expect(staging).toContain('zed/zed-frontend:staging-${SHA8}');
    ...
    expect(prod).toContain('https://prod-fe-ecs.zed.com');
    expect(prod).toContain('https://zed.com/api/runtime-config');
    expect(rollback).toContain('zed/zed-frontend:${TARGET}');
    expect(rollback).toContain('--service web');
    expect(rollback).toContain('zed/zed-api:${TARGET}');
    expect(rollback).toContain('zed/zed-gateway:${TARGET}');
  ...
  it('contains no active Kubernetes deployment paths', () => {
    for (const file of [
      ...
      expect(workflow).not.toContain('ops.zed.com');
    }
  });
  ...
  it('builds preview images without credentials and deploys one full self-host sandbox', () => {
    ...
    expect(workflow).not.toContain('ZED_PREVIEW_APPROVED_SHA');
  ...
  it('disables Vercel auto-deploys everywhere except staging', () => {
    ...
    // and every project load failed. zed.com deploys only from
    ...
    expect(ignore).not.toContain('ZED_PREVIEW_APPROVED_SHA');
  ...
  it('carries both Basic auth credentials and the Vercel SSO bypass in QA', () => {
    ...
    expect(config).toContain("username: 'zed'");
