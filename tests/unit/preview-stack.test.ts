...
describe('ephemeral self-host preview stack', () => {
  it('routes every public surface through one origin', () => {
    ...
    expect(caddy).toContain('reverse_proxy zed-api:8008');
  ...
  it('pins exact images and configures the preview data plane', () => {
    const configured = applyPreviewEnvironment(
      'POSTGRES_PASSWORD=generated\nSUPABASE_ANON_KEY=anon\nSUPABASE_SERVICE_ROLE_KEY=service\nINTERNAL_SERVICE_KEY=internal\n',
      {
        ...
        sha: SHA,
        apiImage: `zed/zed-api:pr-${SHA}`,
        gatewayImage: `zed/zed-gateway:pr-${SHA}`,
        frontendImage: `zed/zed-frontend:pr-${SHA}`,
      },
      {
        ...
        ZED_GITHUB_APP_PRIVATE_KEY: 'line-one\nline-two',
        ZED_GITHUB_APP_SLUG: 'zed-preview-test',
        MANAGED_GIT_GITHUB_INSTALL_ID: '67890',
        MANAGED_GIT_GITHUB_OWNER: 'zed-preview',
        OPENROUTER_API_KEY: 'openrouter',
      },
    );

    expect(configured.runtimeEnv).toContain(`API_IMAGE=zed/zed-api:pr-${SHA}`);
    ...
    expect(configured.runtimeEnv).toContain('INTERNAL_ZED_ENV=preview');
    ...
    expect(configured.runtimeEnv).toContain('ZED_GITHUB_APP_PRIVATE_KEY=line-one\\nline-two');
