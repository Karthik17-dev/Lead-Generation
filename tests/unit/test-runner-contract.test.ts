...
describe('local test runner contract', () => {
  ...
  it('builds publishable artifacts once and schedules package tests by load class', () => {
    ...
    expect(source).toContain("'@zed/sdk', 'typecheck'");
    ...
    expect(source).toContain("ZED_API_TEST_WORKERS: '3'");
    expect(source).toContain("ZED_TEST_TIMEOUT_MS: '30000'");
    expect(source).toContain("['@zed/cli', '@zed/sandbox-agent-server']");
    expect(source).toContain("await runWorkspaceTests(['@zed/db'], 1)");
    expect(source).toContain('Promise.allSettled(tasks)');
    expect(source.match(/await runAll\(\[/g)).toHaveLength(5);
    expect(source).toContain("'!zed-api'");
    expect(source).toContain("'!@zed/db'");
    expect(source).toContain("skipSdkTests ? ['!@zed/sdk'] : []");
  });
  ...
  it('runs isolated API test files through a bounded parallel worker pool', () => {
    ...
    expect(source).toContain('api_test_workers="${ZED_API_TEST_WORKERS:-4}"');
  ...
  it('keeps process-heavy package tests on their proven concurrency settings', () => {
    ...
    const agentPackage = JSON.parse(
      readFileSync(resolve(root, 'apps/zed-sandbox-agent-server/package.json'), 'utf8'),
    );
    ...
    expect(cliPackage.scripts.test).toContain(
      'bun test --timeout ${ZED_TEST_TIMEOUT_MS:-15000} --isolate --parallel=4',
    );
