...
const skipSdkTests = process.env.ZED_PACKAGE_SKIP_SDK_TESTS === '1';
...
await runAll([
  run(['pnpm', '--filter', '@zed/sdk', 'typecheck']),
  run(['pnpm', '--filter', '@zed/sdk', 'run', 'smoke:install']),
]);
await runAll([
...
await runAll([
  runWorkspaceTests(['zed-api'], 1, {
    ZED_API_TEST_WORKERS: '3',
  }),
  runWorkspaceTests(['@zed/cli', '@zed/sandbox-agent-server'], 2),
]);
await runAll([
  (async () => {
    await runWorkspaceTests(['@zed/db'], 1);
  ...
  runWorkspaceTests(
    [
      ...
      '!zed-api',
      '!@zed/cli',
      '!@zed/sandbox-agent-server',
      '!@zed/db',
      ...(skipSdkTests ? ['!@zed/sdk'] : []),
    ],
    2,
  ),
]);
