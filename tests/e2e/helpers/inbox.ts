...
function localMailpitInbox(mailpitUrl: string): DisposableInbox {
  const email = `zed-e2e-${Date.now()}-${randomUUID().slice(0, 8)}@example.test`;
...
/** Delete only THIS suite's leaked inboxes (zed-e2e-*), never anyone else's. */
async function purgeE2eInboxes(
  ...
  const mine = (body.inboxes ?? [])
    ...
    .filter((id): id is string => typeof id === "string" && id.startsWith("zed-e2e-"));
  await Promise.all(
    mine.map((id) =>
...
async function agentMailInbox(apiKey: string): Promise<DisposableInbox> {
  ...
  const createBody = JSON.stringify({
    username: `zed-e2e-${unique}`,
    display_name: "Zed E2E",
    client_id: `zed-browser-auth-${unique}`,
  });
  const createOnce = () =>
  ...
  // our OWN leaked inboxes (zed-e2e-*, never anyone else's) and retry once.
