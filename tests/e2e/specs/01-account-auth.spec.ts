...
async function requestEmailAuthentication(page: Page, email: string) {
  ...
  await expect(
    page.getByRole("heading", { name: "Welcome to Zed" }),
  ).toBeVisible();
    ...
    try {
      ...
      if (userId) {
        const rows = await queryDatabaseRows<AccountRow>(
          "select distinct account_id::text from zed.account_members where user_id = $1::uuid",
          [userId],
        ).catch(() => []);
      ...
      for (const accountId of accountIds) {
        await runDatabaseSql(
          "delete from zed.accounts where account_id = $1::uuid",
          [accountId],
        );
      }
      if (userId) {
