...
insert into zed.platform_user_roles (account_id, role)
values ('${synthetic.id}'::uuid, 'super_admin'::zed.platform_role)
...
`);
    ...
    try {
      ...
      if (synthetic) {
        await runDatabaseSql(
          `delete from zed.platform_user_roles where account_id = '${synthetic.id}'::uuid;`,
        );
