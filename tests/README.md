...
2. `@zed/sdk` tests in `packages/sdk`.
...
`.zed-worktree.json`. The primary checkout defaults to web `3000`, API
...
5. The sandbox generates the standard `zed self-host` Compose distribution.
...
against `staging.zed.com` with the Vercel bypass header. A missing external
...
actions do not have separate controls in the Zed web app.
...
the `zed` schema exists. The runner still requires migrations and service
...
ref and verifies its full SHA. It streams `zed-test.log`, downloads
