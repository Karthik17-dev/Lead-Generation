...
zed.com. There is no "image tag" to point at. A FE rollback is instead a
...
```
1. FE promoted to old build       → zed.com = vX.Y.Z FE   ✅
2. merge the backend rollback PR   → push to prod
3. Vercel rebuilds FE from branch source (still the NEW code) → zed.com = new FE ❌ CLOBBERED
```
...
at ~v0.9.72 (image exists from 0.9.69), so there is no `zed-gateway:0.9.68`.
...
for staging deploy + QA, then **Promote to Production** (see zed-release).
...
- **The deploy-prod run on the backend merge goes partly red** (its version-watch
  ...
  independently. Judge success by `api.zed.com/v1/health`.
...
- **Verify after:** `api.zed.com/v1/health` (version), ECS service health, and
