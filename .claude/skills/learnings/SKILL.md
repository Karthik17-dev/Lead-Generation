...
`addPlatformMetaAgent` and appears in no `zed.yaml`, so resolving it through
...
owner running it; `zed sessions ls` / `new` 403'd from turn one onward, and
...
Namespace the name by `INTERNAL_ZED_ENV`: dev, staging and prod share ONE
...
`zed-app-*` snapshots look equally abandoned but are the targets of
...
**When:** shipping any change to `apps/zed-sandbox-agent-server`, or turning
...
`grep -aoE "<expected literal>" /usr/local/bin/zed-agent` in a session
...
OpenCode MCP entry (`apps/zed-sandbox-agent-server/src/opencode.ts`), an
...
command` + exit 2, so the `zed-connectors` MCP server never started for six
...
`ZED_CONNECTORS_MCP_ENABLED` is off everywhere — the flag, not the tests, is
what contained it.
...
`--build-env NEXT_PUBLIC_ZED_VERSION=<version>` on every prod deploy (the
`deploy-web-vercel` job does); the project env `NEXT_PUBLIC_ZED_VERSION` is
...
Ordering-correct is not enough. Diff `zed_migrations.pgmigrations` on prod
...
*Enforcer:* zed-release skill Step 3.6 (checklist); this rule is the reason.
...
A push to `prod` must not auto-deploy zed.com; the frontend deploys only from
...
**When:** changing a timeout in `zed-sandbox-agent-server/src/proxy.ts` or
...
`apps/zed-sandbox-agent-server`. The API half goes live the moment Deploy Dev
...
triggers that build hashes `apps/zed-sandbox-agent-server/src`. Until an
...
either: `ZED_WARM_SNAPSHOT_ENABLED` defaults false, and ~2s is also what
...
- **Check the edge before raising any budget.** Nothing is written downstream
  while a pre-header probe runs, and the live `zed.com` zone reports
...
metric that counted it. `zed.probe_timeout` was left pinned to false on
