---
name: zed-release
description: "How to cut a Zed production release — the versioning philosophy (when patch vs minor vs major) and the exact flow: derive the release title + notes from the FULL git log since the last release, run the Promote workflow, then deploy + verify prod. Load WHENEVER the user wants to release, promote, cut/ship a version, publish a release, or bump the version. The release notes ARE the public /changelog, so they must be 100% accurate to what shipped."
---
...
Pairs with **zed-voice** (how the notes read). The version source of truth is the
...
- **notes** — a readable, grouped summary of what's in the log. Lead with user-facing
  ...
  **zed-voice**: plain language, no hype/banned words, say "open source" and never
...
```bash
curl -fsS https://staging-api.zed.com/v1/health | jq '{environment,version,commit}'
curl -fsS https://staging.zed.com/api/runtime-config | grep 'staging-api.zed.com'
gh run list --repo karthikxa/AI-Management-System --branch staging --limit 10
```

`qa-staging` must target `staging.zed.com` / `staging-api.zed.com`. A green
run against dev is not a staging gate.

### Step 3.6 — cost the pending PROD migrations

Diff prod's `zed_migrations.pgmigrations` against the promoted tree and cost
every pending migration at prod data volume (a tag tree containing a file is
NOT proof it ran — only the ledger is):
```bash
psql "$(aws secretsmanager get-secret-value --secret-id zed-prod-env \
  --query SecretString --output text | jq -r .DATABASE_URL)" \
  -Atc "select name from zed_migrations.pgmigrations order by id desc limit 5;"
```
...
```bash
gh workflow run promote.yml --repo karthikxa/AI-Management-System --ref staging \
...
```bash
gh run list --repo karthikxa/AI-Management-System --workflow build-staging.yml --branch staging --limit 3 \
...
and Vercel deploys `prod` to `zed.com`.

```bash
gh run list --repo karthikxa/AI-Management-System --workflow deploy-prod.yml --limit 1
curl -fsS https://api.zed.com/v1/health | jq '{environment,version,commit}'
gh release view "vX.Y.Z" --repo karthikxa/AI-Management-System --json name,body   # title + notes present
```
...
- **Staging is not dev.** If `staging.zed.com/api/runtime-config` references
  `dev-api.zed.com`, stop and fix staging before releasing.
...
gh release delete vX.Y.Z --repo karthikxa/AI-Management-System           # only if a Release was actually cut
...
```bash
gh workflow run rollback-prod.yml --repo karthikxa/AI-Management-System --ref main \
...
(auto-sync + selfHeal) rolls zed-api + zed-gateway to those images. Then:
- **Frontend:** Vercel → zed.com → Deployments → the `vX.Y.Z` prod deployment →
...
3. title + notes written in zed-voice, accurate to the log.
4. `staging.zed.com` and `staging-api.zed.com` verified on the staging SHA.
5. `promote.yml` run with title + notes; release PR merged into `prod`.
6. deploy-prod ran, EKS green, api.zed.com on the new version.
