...
only). Did NOT touch `apps/zed-sandbox-agent-server/*` or
...
pnpm --filter @zed/sdk run smoke:install
  → "OK: @zed/sdk and @zed/executor-sdk import and construct from
...
`window.Zed`). `public-surface.snapshot.json` (value-level) had NO diff —
...
SDK baseline before edits: pnpm --filter @zed/sdk test
...
SDK after edits (doc-only changes to messages.ts):
  ...
  test       → 2030 pass, 0 fail, 147 files (unchanged from baseline — expected,
               no behavior changed)
  smoke:install → "OK: @zed/sdk and @zed/executor-sdk import and
...
- `markDispatchedForPartIds` and `beginOptimisticPlainTextSend` both promoted
  ...
  so neither is reachable from `@zed/sdk/react` and the public-surface
...
@zed/sdk test`; one `sync-store.test.ts` failure observed on a SEPARATE run
...
$ pnpm --filter @zed/sdk test   (bun test --isolate src)
...
`@zed/sdk/react`), type snapshot +4 (the same three plus `AbortSettlement`).
...
- `sessionPinStorageKey(projectId, sessionId)` → `zed:pin:<projectId>/<sessionId>`.
...
147 files** (`pnpm --filter @zed/sdk test`, no `--isolate` — matches the
...
change for this task, only the SDK's `@zed/sdk` workspace source resolution
...
`ApiError.message`. Every Zed error body pairs a machine slug (`reason`) with
...
`globalThis.fetch` / `configureZed()` singletons when 141 files run together.
...
- Cross-package drift holds: `@zed/api-contract` `FeatureFlagMapSchema`, the
...
matches `@zed/llm-catalog` after adding managed `deepseek-v4-pro-0813`. The
...
No public SDK API changes. The playground's compile-time `MODEL_IDS` union now
matches `@zed/llm-catalog` after adding `grok-4.6`. It also includes the
...
- `pnpm --filter @zed/sdk typecheck`: exit `0` for the package and
  `examples/tsconfig.json`.
- `pnpm --filter @zed/sdk test`: `1862 pass`, `2 skip`, `0 fail`, `7131
...
RED — `pnpm --filter @zed/sdk typecheck`:
...
- `pnpm --filter @zed/sdk test`: `1847 pass`, `2 skip`, `0 fail`, 141 files.
- `pnpm --filter @zed/sdk smoke:install`: packed-install import + construction passed.
...
- `pnpm --filter @zed/sdk test`: `1855 pass`, `0 fail`, `7112 expect()` calls.
- `pnpm --filter @zed/sdk typecheck`: exit `0` for the package and examples.
- `pnpm --filter @zed/sdk smoke:install`: packed-install import and construction passed.
...
- `pnpm --filter @zed/sdk test`: `1848 pass`, `0 fail`, `7093 expect()` calls.
- `pnpm --filter @zed/sdk typecheck`: exit `0` for the package and examples.
- `pnpm --filter @zed/sdk smoke:install`: packed-install import and construction passed.
...
- `pnpm --filter @zed/sdk test`: `1827 pass`, `2 skip`, `0 fail`, `7043 expect()` calls.
- `pnpm --filter @zed/sdk typecheck`: exit `0` for the package and examples.
...
- `pnpm --filter @zed/sdk typecheck`: exit `0` (package + examples).
- `pnpm --filter @zed/sdk test`: `1831 pass`, `2 skip`, `0 fail`, `7049
...
- `react/use-opencode-events/helpers.ts`: new module-internal `realRuntimeTitle`
  + `patchZedSessionTitleMirrors` — on a `session.updated`/`session.created`
  title change, patch the cached Zed session reads (`name` on rows matching
  `opencode_session_id`, `custom_name` untouched) before the existing
  `refetchZedSessionMirrors` reconciliation refetch.
...
RED: 4 new tests in `handle-event.test.ts` (`zed session title mirroring`) —
...
- `pnpm --filter @zed/sdk typecheck`: exit `0` (package + examples).
- `pnpm --filter @zed/sdk test`: `1829 pass`, `2 skip`, `0 fail`, `7044 expect()` calls, 141 files.
- `pnpm --filter @zed/sdk run smoke:install`: packed-install import passed.
...
- Add the four monitor read fields to `ProjectTrigger`, matching
  `TriggerSchema` in `@zed/api-contract` (`run`, `mode`, `interval_seconds`,
...
RED — `pnpm --filter @zed/sdk typecheck` before implementation, with the new
...
- `pnpm --filter @zed/sdk test`: `1852 pass`, `2 skip`, `0 fail`,
...
**Gates, real output:**
- `pnpm --filter @zed/sdk typecheck` — clean (`tsc --noEmit` + examples).
- `pnpm --filter @zed/sdk test` — `1901 pass`, `2 skip`, `0 fail`, `7244
...
**Evidence** — `pnpm --filter @zed/sdk test`: 1922 pass, 0 fail (145 files;
...
**Gates:**
- `pnpm --filter @zed/sdk typecheck`: exit 0.
- `pnpm --filter @zed/sdk test`: session baseline `1940 pass`, `2 skip`,
...
- `pnpm --filter @zed/sdk run smoke:install`: passed (tarball packs,
  installs, imports).
...
**Gates:**
- `pnpm --filter @zed/sdk typecheck`: exit 0.
- `pnpm --filter @zed/sdk test`: session baseline `1952 pass`, `0 fail`,
...
**Gates:**
- `pnpm --filter @zed/sdk typecheck`: exit 0.
- `pnpm --filter @zed/sdk test`: session baseline `1953 pass`, `0 fail`,
...
- New `AbortReason` type + `ABORT_REASONS` (`'user' | 'runtime-disposed' |
  ...
  (`apps/zed-sandbox-agent-server/src/main.ts`) — confirmed it aborts
...
(`zed.ts`'s `session.stream()` and `use-opencode-events/index.ts` both
...
**Gates, real output:**
- `pnpm --filter @zed/sdk typecheck` — clean (`tsc --noEmit` + examples).
- `pnpm --filter @zed/sdk test` — `2011 pass`, `0 fail`, `7501 expect()`
...
- `pnpm --filter @zed/sdk typecheck` — clean, no output (tsc --noEmit +
  examples), exit 0.
- `pnpm --filter @zed/sdk test` — `2102 pass`, `0 fail`, `7686 expect()`
