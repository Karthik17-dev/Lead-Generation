...
// token to the WEB origin (https://dev.zed.com/file/upload), got the Next.js
...
test('copy uploads through platformConfig().fetch, not a bare global fetch', async () => {
  ...
  const injected = mock(async (input: unknown, init: RequestInit = {}) => {
    const url = String(input);
    seen.push({ url, clientHeader: new Headers(init.headers).get('X-Zed-Client') });
...
// BAKED INTO THE SANDBOX IMAGE (`/usr/local/bin/zed-agent`,
...
test('createFile survives an OLD daemon that lands a zero-byte part as "undefined"', async () => {
  routeDaemon((url) => {
    // Exactly what a pre-rebuild zed-agent returns for an empty part.
