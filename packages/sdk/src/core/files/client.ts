 ...
 * (`https://dev.zed.com/file/upload`), which answered with the Next.js 404
...
export async function uploadFile(
  ...
  return uploadWithRetry(
    () => {
      ...
      // `apps/zed-sandbox-agent-server/src/routes/files.ts`).
 ...
 * `X-Zed-Client` header, and `platformConfig().fetch` — so every host that
 ...
 * (`/usr/local/bin/zed-agent`, `apps/sandbox/Dockerfile`). `/v1/runtime-assets`
...
export async function writeFile(
  ...
  const results = await uploadFile(content, parent, `.${name}.zed-write-${token}`, base);
  ...
  const backupPath = `${absPath}.zed-write-backup-${token}`;
  let backedUp = false;
  try {
