  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        //
        // A Zed error body pairs a machine slug with the sentence written for
...
async function postStream(
  ...
  const clientSource = normalizeClientSource(platformConfig().clientSource);
  if (clientSource) headers['X-Zed-Client'] = clientSource;
  if (adminBypassEnabled) headers['x-zed-admin-bypass'] = '1';
