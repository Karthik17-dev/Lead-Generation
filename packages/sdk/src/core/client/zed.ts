...
/**
 * createZed — the single opinionated entry point to the Zed data layer.
 *
 * One client. Every action a method. The host app imports ONLY from `@zed/sdk`
 * — never `@opencode-ai/sdk`, never `backendApi`/`authenticatedFetch` directly.
 *
 *   const zed = createZed({ getToken });
 *   await zed.projects.list();
 *   await zed.project(pid).secrets.upsert({ name, value });
 ...
 * for ergonomics. Reactive data still comes from `@zed/sdk/react` hooks.
 */
import * as F from '../files/client';
import { getClient, getClientForUrl } from '../runtime/client';
import { ApiError } from '../http/api/errors';
import { type ZedPlatformConfig, configureZed, platformConfig } from '../http/config';
...
export function createZed(config: ZedPlatformConfig, opts?: { global?: boolean }) {
  // Wire the platform seam once. All wrapped functions read it.
  //
  // `opts.global === false` (used by `@zed/sdk/server`'s `createScopedZed`)
  // skips the process-wide write entirely — that caller relies solely on the
  // `AsyncLocalStorage` scope `createScopedZed` wraps every method call in,
  // so this returned facade never touches (or is affected by) the module-global
  // singleton other concurrent `createZed()` calls in the same process share.
  configureZed(config, opts);
   ...
   * route (or any same-origin proxy) fronting the real Zed API — legitimately
   * configures it as a relative path like `/api/zed`. `new URL()` throws on
  ...
  function parseBackendUrlForPort(apiBaseUrl: string): URL | null {
    try {
      ...
      throw new ApiError(
        `Zed SDK: backendUrl must be an absolute URL outside the browser (got ${JSON.stringify(apiBaseUrl)}). Relative paths like "/api/zed" only resolve against a page origin — configure an absolute backendUrl for server-side hosts.`,
  ...
  function resolvePreviewOptsForSandbox(sandboxId: string): SubdomainUrlOptions {
    ...
    // `createZed()` time: a host may re-point the seam after creation
    // (calling `configureZed()` again — e.g. the whitelabel app switching
  ...
  const projects = {
    ...
    /** Create a project backed by a brand-new Zed-managed GitHub repo. */
  ...
  function project(projectId: string) {
    ...
    return {
      ...
      /** Project-scoped CLI PATs (auto-minted at session-create as `ZED_TOKEN`; can also be minted by hand). */
      tokens: {
      ...
      /** Validate a `zed.yaml` (or legacy `zed.toml`) manifest's raw text server-side — format is auto-resolved from the project's manifest path (same schema `zed ship`/CR-merge use). */
      ...
      git: {
        ...
        /** Invite a GitHub user as a collaborator on a Zed-managed repo. */
  ...
  function session(projectId: string, sessionId: string) {
    ...
    // Zed `sessionId` — they differ. We resolve+cache it once (including the
     ...
     * current project-session model. Read the persisted Zed session so the
     ...
     * itself yet. This is what lets a brand-new `zed.session(pid, sid)`
    ...
    return {
      ...
      /** This session's live voice-call transcript (spoken turns + ask_zed/run_command calls). */
      ...
      /**
       * Liveness/readiness of THIS session's runtime (`GET /zed/health`).
      ...
      // wires). React hosts use `@zed/sdk/react` hooks instead, which bind to
       ...
       * (`@zed/sdk`'s `openEventStream`, also used verbatim by
       * `@zed/sdk/react`'s `useOpenCodeEventStream`): resolves THIS
       ...
       * Framework-free — safe to call from a server-side "Zed as a
       ...
       * sandbox the top-level `@zed/sdk` `files` export follows. Fixes a
       ...
       * (`@zed/sdk/files`'s exports all take an optional trailing
...
export type Zed = ReturnType<typeof createZed>;
/** The id-bound project handle returned by `zed.project(id)`. */
export type ProjectHandle = ReturnType<Zed['project']>;
/** The id-bound session handle returned by `zed.session(pid, sid)`. */
export type SessionHandle = ReturnType<Zed['session']>;
