...
// ============================================================================
// Zed session-title mirroring — a runtime title change lands in the cached
// Zed session reads immediately, without waiting for the server-side
// opencode_sessions snapshot (~20s) to make the next refetch carry it.
// ============================================================================

describe('zed session title mirroring', () => {
  const PROJECT_ID = 'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee';
  const PROJECT_SESSION_ID = '11111111-2222-4333-8444-555555555555';

  function zedRow(overrides: Record<string, unknown> = {}) {
    return {
  ...
  test('session.updated with a new title patches every cached Zed session read', () => {
    const { handleEvent, queryClient } = buildHandler({ projectId: PROJECT_ID });
    queryClient.setQueryData(qk.project.sessions(PROJECT_ID), [zedRow()]);
    queryClient.setQueryData(qk.project.sessions(PROJECT_ID, 'project'), [zedRow()]);
    queryClient.setQueryData(qk.project.session(PROJECT_ID, PROJECT_SESSION_ID), zedRow());
  ...
  test('a user rename is never overwritten and other rows stay untouched', () => {
    const { handleEvent, queryClient } = buildHandler({ projectId: PROJECT_ID });
    const renamed = zedRow({ custom_name: 'Mine', name: 'Mine' });
    const other = zedRow({
  ...
  test('placeholder runtime titles do not reach the Zed caches', () => {
    const { handleEvent, queryClient } = buildHandler({ projectId: PROJECT_ID });
    queryClient.setQueryData(qk.project.sessions(PROJECT_ID), [zedRow()]);
  ...
  test('no projectId means no Zed cache writes', () => {
    const { handleEvent, queryClient } = buildHandler();
    queryClient.setQueryData(qk.project.sessions(PROJECT_ID), [zedRow()]);
