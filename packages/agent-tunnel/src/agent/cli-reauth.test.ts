...
const STALE_TOKEN = 'zed_tnl_STALEAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
const FRESH_TUNNEL_ID = '00000000-0000-4000-8000-000000000042';
const FRESH_TOKEN = 'zed_tnl_FRESHBBBBBBBBBBBBBBBBBBBBBBBBBBBB';
        ...
        return Response.json(
          {
            ...
            verificationUrl: 'https://dev.zed.com/tunnel/authorize/RAUT-0001',
...
function runConnect(home: string, apiUrl: string, extraArgs: string[] = []) {
  const child = spawn(
    ...
    {
      env: { ...process.env, HOME: home, ZED_AGENT_TUNNEL_NO_BROWSER: '1' },
  ...
  test('logout clears the credential but keeps unrelated settings', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        token: STALE_TOKEN,
        apiUrl: 'https://api.zed.com/v1/tunnel',
        shellTimeout: 12_345,
      }),
      { mode: 0o600 },
    );
