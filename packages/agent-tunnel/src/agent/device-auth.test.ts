...
const VALID_TOKEN = `zed_tnl_${'A'.repeat(36)}`;

function challenge(overrides: Record<string, unknown> = {}) {
  return {
    deviceCode: 'ABCD-1234',
    deviceSecret: 'ABCDEFGHIJKLMNOPQRSTUVWXYZ123456',
    verificationUrl: 'https://zed.com/tunnel/authorize/ABCD-1234',
...
describe('device auth challenge validation', () => {
  test('accepts a well-formed challenge', () => {
    ...
    expect(parsed.verificationUrl).toStartWith('https://zed.com/');
  });
  ...
  test('rejects a URL carrying credentials', () => {
    expect(() => parseDeviceAuthChallenge(challenge({ verificationUrl: 'https://user:pw@zed.com/x' })))
