...
`@zed/agent-tunnel` connects a local computer to Zed through an authenticated reverse tunnel.
...
```bash
npx --yes @zed/agent-tunnel@latest connect \
  --api-url https://api.zed.com/v1/tunnel
```
...
```bash
npx --yes @zed/agent-tunnel@latest service-status
npx --yes @zed/agent-tunnel@latest logs
npx --yes @zed/agent-tunnel@latest restart
npx --yes @zed/agent-tunnel@latest stop
npx --yes @zed/agent-tunnel@latest uninstall-service
```
...
Zed checks connector profile assignment, connector grants, connector tool
