...
describe('zed pgSchema', () => {
  test('declares the zed schema namespace', () => {
    expect(zedSchema.schemaName).toBe('zed');
  });

  test('all sampled tables live in the zed schema', () => {
    const tables = [accounts, projects, sandboxes, zedApiKeys];
    for (const t of tables) {
      expect(getTableConfig(t).schema).toBe('zed');
    }
  });
});
...
describe('projects table', () => {
  ...
  test('manifest_path defaults to zed.yaml', () => {
    const col = getTableConfig(projects).columns.find((c) => c.name === 'manifest_path');
    expect(col?.default).toBe('zed.yaml');
  });
...
describe('zedApiKeys table', () => {
  test('maps to the api_keys table name inside the zed schema', () => {
    const cfg = getTableConfig(zedApiKeys);
    expect(cfg.name).toBe('api_keys');
    expect(cfg.schema).toBe('zed');
  });

  test('uses key_id as its primary key', () => {
    expect(primaryColumn(zedApiKeys)).toBe('key_id');
  });

  test('type defaults to user and status defaults to active', () => {
    const cols = getTableConfig(zedApiKeys).columns;
  ...
  test('enforces a unique public_key index', () => {
    const cfg = getTableConfig(zedApiKeys);
    const unique = cfg.indexes.find((i) => i.config.name === 'idx_zed_api_keys_public_key');
    expect(unique?.config.unique).toBe(true);
  });
});

describe('accountSsoProviders table', () => {
  test('maps to account_sso_providers inside the zed schema', () => {
