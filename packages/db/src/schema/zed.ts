...
export const zedSchema = pgSchema('zed');

export const sandboxStatusEnum = zedSchema.enum('sandbox_status', [
  'provisioning',
  'active',
  'stopped',
  'archived',
...
export const sandboxProviderEnum = zedSchema.enum('sandbox_provider', [
  'daytona',
  'platinum',
  'e2b',
  'local',
 ...
 *            Zed chokepoint attaches the credential.
 ...
 */
export const projectSecretStrategyEnum = zedSchema.enum('project_secret_strategy', [
  'runtime',
  'egress',
  'broker',
  'denied',
...
export const projectSecretConsumerEnum = zedSchema.enum('project_secret_consumer', [
  'sandbox',
  'llm_gateway',
  'connector',
  'git_proxy',
...
export const projectSessionStatusEnum = zedSchema.enum('project_session_status', [
  'queued',
  'branching',
  'provisioning',
  'running',
...
export const sessionLifecycleCommandStatusEnum = zedSchema.enum(
...
export const providerTransitionStatusEnum = zedSchema.enum('provider_transition_status', [
  'pending',
  'building',
  'ready',
  'activating',
...
export const projectRoleEnum = zedSchema.enum('project_role', [
  'manager',
  'editor',
  'member',
  'viewer',
...
export const projectAccessRequestStatusEnum = zedSchema.enum('project_access_request_status', [
  'pending',
  'approved',
  'rejected',
]);

export const apiKeyStatusEnum = zedSchema.enum('api_key_status', [
  'active',
  'revoked',
  'expired',
]);

export const apiKeyTypeEnum = zedSchema.enum('api_key_type', ['user', 'sandbox']);

// ─── Accounts & Members ─────────────────────────────────────────────────────
// Replaces basejump.account_user. Fully zed-native.

export const accountRoleEnum = zedSchema.enum('account_role', ['owner', 'admin', 'member']);

export const accounts = zedSchema.table('accounts', {
...
export const accountMembers = zedSchema.table(
  'account_members',
  {
...
export const accountInvitations = zedSchema.table(
  'account_invitations',
  {
     ...
     *  accepted. Lets a project admin invite a non-Zed user "into
...
export const accountGithubInstallations = zedSchema.table(
...
export const accountGithubInstallationStates = zedSchema.table(
...
export const projects = zedSchema.table(
  'projects',
  {
    ...
    manifestPath: text('manifest_path').default('zed.yaml').notNull(),
    ...
    // Zed-as-a-Backend project is the case that wants 'denied', where an
...
export const projectGitConnections = zedSchema.table(
  'project_git_connections',
  {
     ...
     * Distinct from repoUrl, which is the client-facing Zed git-proxy URL.
    ...
    /** True when Zed provisioned this repo (vs a BYO/linked repo). */
...
export const projectGitCredentials = zedSchema.table(
...
export const projectMembers = zedSchema.table(
  'project_members',
  {
    accountId: uuid('account_id')
      .notNull()
...
export const projectAccessRequests = zedSchema.table(
 ...
 */
export const secretShareScopeEnum = zedSchema.enum('secret_share_scope', [
  'project',
  'restricted',
]);

/** Principal kind for a member/group allow-list grant. See doc comment above. */
export const secretGrantPrincipalEnum = zedSchema.enum('secret_grant_principal', [
  'member',
  'group',
]);
 ...
 */
export const projectSecretScopeEnum = zedSchema.enum('project_secret_scope', [
  'runtime',
  'connector',
]);
...
export interface SecretEgressPolicy {
  /** For `strategy='broker'`: which Zed chokepoint holds the real value. */
 ...
 * `secrets` grant (zed.yaml) references. `name` is the env var KEY injected
 ...
 */
export const projectSecrets = zedSchema.table(
  'project_secrets',
  {
     ...
     *  re-entered through a setup link or `zed env push` can never silently
 ...
 */
export const projectSessionVisibilityEnum = zedSchema.enum('project_session_visibility', [
  'private',
  'project',
  'restricted',
]);
...
export const projectSessionOriginEnum = zedSchema.enum('project_session_origin', [
  'user',
  'trigger',
  'schedule',
  'backend',
...
export const projectSessions = zedSchema.table(
  'project_sessions',
  {
  ...
  (table) => [
    ...
    // Per-END-USER concurrency cap for Zed-as-a-Backend: COUNT of a single
 ...
 * server-owned ZED_SESSION_CONTEXT JSON envelope.
 */
export const projectSessionRuntimeContexts = zedSchema.table(
 ...
 */
export const projectSecretHandleStatusEnum = zedSchema.enum('project_secret_handle_status', [
  'active',
  'superseded',
  'revoked',
]);
 ...
 */
export const projectSessionSecretHandles = zedSchema.table('project_session_secret_handles', {
...
// per-project (each project's own zed.yaml), so without this, two unrelated
// projects sharing an account and an agent name (almost always the conventional
// 'zed') would clobber each other's pin — the row was keyed only on
...
export const accountModelPreferences = zedSchema.table(
...
// only a shape change to `@zed/llm-catalog`'s `GenerationConfig`. Values
...
export const projectLlmRoutingPolicies = zedSchema.table(
 ...
 */
export const projectSessionGrants = zedSchema.table(
...
export const projectSessionPublicShares = zedSchema.table(
 ...
 */
export const projectTriggerRuntime = zedSchema.table(
 ...
 */
export const projectMonitorEvents = zedSchema.table(
 ...
 */
export const projectMonitorBoxes = zedSchema.table(
 ...
 */
export const projectTriggerExecutions = zedSchema.table(
...
export const sessionLifecycleCommands = zedSchema.table(
...
export const chatInstalls = zedSchema.table(
  'chat_installs',
  {
...
export const chatChannelBindings = zedSchema.table(
...
// a Zed session and writes a row here. Follow-up messages in the same
...
export const chatThreads = zedSchema.table(
  'chat_threads',
  {
...
export const chatPendingAuthMessages = zedSchema.table(
...
export const chatThreadParticipants = zedSchema.table(
...
// given workspace) to the Zed user they authenticated as via `/login`. The
// inbound gate resolves the sender through this table and runs the agent as that
// Zed user — so each member acts under their OWN credentials/secrets, never
...
export const chatUserIdentities = zedSchema.table(
...
export const chatTurnStreams = zedSchema.table(
  'chat_turn_streams',
  {
    sessionId: text('session_id').primaryKey(),
    projectId: uuid('project_id').notNull(),
 ...
 * as speech happens, read back by the Zed session through `voice_read`.
 *
 ...
 */
export const voiceCallTurns = zedSchema.table(
  'voice_call_turns',
  {
     ...
     *  'tool' (an ask_zed/run_command call the worker made through the
...
/**
 * The Zed agent's read position in a call's transcript — the state that lets
 ...
 */
export const voiceCallReadCursors = zedSchema.table('voice_call_read_cursors', {
 ...
 */
export const voiceJoinLinks = zedSchema.table(
  'voice_join_links',
  {
...
export const teamsPendingUploads = zedSchema.table(
...
export const chatEventDedup = zedSchema.table(
  'chat_event_dedup',
  {
...
// `zed.*` is 100% Drizzle-owned and the migration engine has one source.
export const workerLeaderLease = zedSchema.table('worker_leader_lease', {
...
// Per-session sandbox runtime row. Decoupled from `zed.sandboxes` (the
...
export const sessionSandboxStatusEnum = zedSchema.enum('session_sandbox_status', [
  'provisioning',
  'active',
  'stopped',
  'error',
...
export const sessionSandboxes = zedSchema.table(
  'session_sandboxes',
  {
    ...
    // zed.session_sandboxes_anchor_guard(), which carries it forward on EVERY
 ...
 */
export const providerEvents = zedSchema.table(
  'provider_events',
  {
    id: uuid('id').defaultRandom().primaryKey(),
 ...
 *
 * Sources of truth:
 *   - zed.yaml `sandbox.templates` entries → upserted into this table on read
 ...
 */
export const sandboxTemplates = zedSchema.table(
  'sandbox_templates',
  {
     ...
     * runtime layer (everything the zed-agent CAS swap does NOT touch). The
    ...
    /** Provider-side snapshot name (e.g. `zed-default-…`, `zed-tpl-…`). */
 ...
 * the hot path.
 */
export const projectSnapshotBuilds = zedSchema.table(
...
export const providerTransitions = zedSchema.table(
...
export const sandboxes = zedSchema.table(
  'sandboxes',
  {
...
export const scopeEffectEnum = zedSchema.enum('scope_effect', ['grant', 'revoke']);

export const sandboxMembers = zedSchema.table(
  'sandbox_members',
  {
...
export const sandboxMemberScopes = zedSchema.table(
...
export const sandboxInvites = zedSchema.table(
  'sandbox_invites',
  {
    inviteId: uuid('invite_id').defaultRandom().primaryKey(),
    sandboxId: uuid('sandbox_id')
...
export const legacySandboxMigrations = zedSchema.table(
...
// not zed.sandboxes.
export const sunaAccountMigrations = zedSchema.table(
...
export const zedApiKeys = zedSchema.table(
  'api_keys',
  {
  ...
  (table) => [
    uniqueIndex('idx_zed_api_keys_public_key').on(table.publicKey),
    index('idx_zed_api_keys_secret_hash').on(table.secretKeyHash),
    index('idx_zed_api_keys_sandbox').on(table.sandboxId),
    index('idx_zed_api_keys_account').on(table.accountId),
  ],
);

// ─── Account Tokens (Personal Access Tokens for the CLI) ────────────────────
// Account-scoped, minted from the dashboard, used as
// `Authorization: Bearer <zed_pat_...>` by the `zed` CLI.
 ...
 * so it's never re-declared. `zedCli`/`connectors` are `"all"` (everything,
...
export const accountTokens = zedSchema.table(
  'account_tokens',
  {
    ...
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    revokedAt: timestamp('revoked_at', { withTimezone: true }),
    /** Per-agent authorization grant for a sandbox session token: which Zed
     *  CLI/API actions + connectors the running agent may use. Resolved
     *  from the zed.yaml `agents` map at session birth. The launching
...
export const oauthClients = zedSchema.table('oauth_clients', {
...
export const oauthAuthorizationCodes = zedSchema.table(
...
export const oauthAccessTokens = zedSchema.table(
  'oauth_access_tokens',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const oauthRefreshTokens = zedSchema.table(
...
export const zedApiKeysRelations = relations(zedApiKeys, ({ one }) => ({
  sandbox: one(sandboxes, {
    fields: [zedApiKeys.sandboxId],
    references: [sandboxes.sandboxId],
  }),
}));
...
export const auditEvents = zedSchema.table(
  'audit_events',
  {
...
export const auditSessionSequences = zedSchema.table('audit_session_sequences', {
  sessionId: text('session_id').primaryKey(),
...
export const usageEvents = zedSchema.table(
  'usage_events',
  {
    eventId: uuid('event_id').defaultRandom().primaryKey(),
    accountId: uuid('account_id')
    ...
    sessionId: text('session_id'),
    /**
     * Zed-as-a-Backend attribution: which of the wrapper's END-USERS this
...
export const gatewayRequestLogs = zedSchema.table(
...
export const gatewayApiKeys = zedSchema.table(
  'gateway_api_keys',
  {
    keyId: uuid('key_id').defaultRandom().primaryKey(),
    accountId: uuid('account_id')
...
export const gatewayBudgetScopeEnum = zedSchema.enum('gateway_budget_scope', [
  'project',
  'member',
]);
export const gatewayBudgetPeriodEnum = zedSchema.enum('gateway_budget_period', [
  'day',
  'week',
  'month',
]);
export const gatewayBudgetActionEnum = zedSchema.enum('gateway_budget_action', [
  'block',
  'warn',
]);

export const gatewayBudgets = zedSchema.table(
  'gateway_budgets',
  {
...
export const billingCustomers = zedSchema.table(
  'billing_customers',
  {
    accountId: uuid('account_id').notNull(),
  ...
  (table) => [index('idx_zed_billing_customers_account_id').on(table.accountId)],
);

export const creditAccounts = zedSchema.table(
  'credit_accounts',
  {
    ...
    // Zed-managed model credentials regardless of tier. false = BYOK only,
  ...
  (table) => [
    index('zed_credit_accounts_account_id_idx').on(table.accountId),
 ...
 */
export const sessionPendingQuestions = zedSchema.table(
...
export const sandboxComputeSessions = zedSchema.table(
...
/**
 * Stable, user-facing Zed Apps. A row owns one hostname and an atomic
 ...
 */
export const apps = zedSchema.table(
  'apps',
  {
...
export const appAccessGrants = zedSchema.table(
  'app_access_grants',
  {
    grantId: uuid('grant_id').defaultRandom().primaryKey(),
    appId: uuid('app_id')
...
export const appArtifacts = zedSchema.table(
  'app_artifacts',
  {
...
export const appDeployments = zedSchema.table(
  'app_deployments',
  {
...
export const appRuntimes = zedSchema.table(
  'app_runtimes',
  {
...
export const appDeploymentEvents = zedSchema.table(
...
export const stripeWebhookEventsProcessed = zedSchema.table(
...
export const yoloMemberTokens = zedSchema.table(
  'yolo_member_tokens',
  {
    userId: uuid('user_id').notNull(),
    accountId: uuid('account_id').notNull(),
...
export const creditLedger = zedSchema.table(
  'credit_ledger',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    accountId: uuid('account_id').notNull(),
  ...
  (table) => [
    unique('zed_unique_stripe_event').on(table.stripeEventId),
    ...
    index('idx_zed_credit_ledger_idempotency')
      .on(table.idempotencyKey)
      .where(sql`${table.idempotencyKey} IS NOT NULL`),
  ],
);

export const creditUsage = zedSchema.table('credit_usage', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
...
export const accountDeletionRequests = zedSchema.table('account_deletion_requests', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  userId: uuid('user_id').notNull(),
...
export const creditPurchases = zedSchema.table('credit_purchases', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  amountDollars: numeric('amount_dollars', { precision: 10, scale: 2 }).notNull(),
...
export const tunnelStatusEnum = zedSchema.enum('tunnel_status', [
  'online',
  'offline',
  'connecting',
]);

export const tunnelCapabilityEnum = zedSchema.enum('tunnel_capability', [
  'filesystem',
  'shell',
  'network',
  'apps',
...
export const tunnelPermissionStatusEnum = zedSchema.enum('tunnel_permission_status', [
  'active',
  'revoked',
  'expired',
]);

export const tunnelPermissionRequestStatusEnum = zedSchema.enum(
...
export const tunnelConnections = zedSchema.table(
  'tunnel_connections',
  {
...
export const tunnelRpcForwards = zedSchema.table(
  'tunnel_rpc_forwards',
  {
    requestId: uuid('request_id').defaultRandom().primaryKey(),
...
export const tunnelPermissions = zedSchema.table(
  'tunnel_permissions',
  {
...
export const tunnelPermissionRequests = zedSchema.table(
...
export const tunnelAuditLogs = zedSchema.table(
  'tunnel_audit_logs',
  {
    logId: uuid('log_id').defaultRandom().primaryKey(),
...
export const tunnelDeviceAuthStatusEnum = zedSchema.enum('tunnel_device_auth_status', [
  'pending',
  'approved',
  'denied',
  'expired',
...
export const tunnelDeviceAuthRequests = zedSchema.table(
...
export const platformRoleEnum = zedSchema.enum('platform_role', [
  'user',
  'admin',
  'super_admin',
]);

export const platformUserRoles = zedSchema.table(
  'platform_user_roles',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    accountId: uuid('account_id').notNull(),
...
export const impersonationGrants = zedSchema.table(
...
export const accessRequestStatusEnum = zedSchema.enum('access_request_status', [
  'pending',
  'approved',
  'rejected',
]);

export const platformSettings = zedSchema.table('platform_settings', {
  key: varchar('key', { length: 255 }).primaryKey(),
  value: jsonb('value').notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
});

export const accessAllowlist = zedSchema.table(
  'access_allowlist',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const accessRequests = zedSchema.table(
  'access_requests',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    email: varchar('email', { length: 255 }).notNull(),
...
// ─── Change Requests ────────────────────────────────────────────────────────
// PR-equivalent for Zed-native git workflows. A change_request proposes
...
export const changeRequestStatusEnum = zedSchema.enum('change_request_status', [
  'open',
  'merged',
  'closed',
]);

export const changeRequests = zedSchema.table(
  'change_requests',
  {
...
export const reviewItemKindEnum = zedSchema.enum('review_item_kind', [
  'change',
  'approval',
  'output',
  'decision',
...
export const reviewItemStatusEnum = zedSchema.enum('review_item_status', [
  'needs_you',
  'waiting',
  'approved',
  'changes_requested',
...
export const reviewItemRiskEnum = zedSchema.enum('review_item_risk', [
  'none',
  'low',
  'medium',
  'high',
...
export const reviewItemSourceEnum = zedSchema.enum('review_item_source', [
  'web',
  'slack',
  'agent',
]);

export const reviewItems = zedSchema.table(
  'review_items',
  {
...
export const accountGroupSourceEnum = zedSchema.enum('account_group_source', [
  'manual',
  'scim',
  'sso',
]);

export const accountGroups = zedSchema.table(
  'account_groups',
  {
...
export const accountGroupMembers = zedSchema.table(
 ...
 */
export const projectGroupGrants = zedSchema.table(
...
export const iamRoles = zedSchema.table(
  'iam_roles',
  {
...
export const iamRoleActions = zedSchema.table(
  'iam_role_actions',
  {
    roleId: uuid('role_id')
      .notNull()
...
export const iamPolicies = zedSchema.table(
  'iam_policies',
  {
 ...
 */
export const iamResourceGrants = zedSchema.table(
  'iam_resource_grants',
  {
    grantId: uuid('grant_id').defaultRandom().primaryKey(),
    accountId: uuid('account_id')
...
export const scimTokens = zedSchema.table(
  'scim_tokens',
  {
    tokenId: uuid('token_id').defaultRandom().primaryKey(),
    accountId: uuid('account_id')
      ...
      .references(() => accounts.accountId, { onDelete: 'cascade' }),
    name: varchar('name', { length: 128 }).notNull(),
    // SHA-256 hex of the plaintext token (zed_scim_*). We never store
    ...
    // ("zed_scim_abcd…"). Display-only; not used for lookup.
...
export const auditWebhooks = zedSchema.table(
  'audit_webhooks',
  {
...
export const auditWebhookDeliveries = zedSchema.table(
...
export const serviceAccounts = zedSchema.table(
  'service_accounts',
  {
    ...
    name: varchar('name', { length: 128 }).notNull(),
    description: text('description'),
    /** SHA-256 hex of the plaintext bearer (zed_sa_*). Plaintext
    ...
    projectId: uuid('project_id').references(() => projects.projectId, { onDelete: 'cascade' }),
    /** The zed.yaml `agents` entry name this SA is the standing identity for.
...
export const accountSessionActivity = zedSchema.table(
...
export const accountSsoGroupMappings = zedSchema.table(
 ...
 * Connectors are DEFINED in zed.yaml (`connectors`) and materialized here
 ...
 */
export const connectorProviderEnum = zedSchema.enum('connector_provider', [
  'pipedream',
  'mcp',
  'openapi',
  'postman',
...
export const connectorStatusEnum = zedSchema.enum('connector_status', [
  'active',
  'disabled',
  'needs_auth',
  'error',
...
export const connectorPolicyActionEnum = zedSchema.enum('connector_policy_action', [
  'always_run',
  'require_approval',
  'block',
]);

export const connectorRiskEnum = zedSchema.enum('connector_risk', [
  'read',
  'write',
  'destructive',
]);

export const connectorCallStatusEnum = zedSchema.enum('connector_call_status', [
  'ok',
  'error',
  'denied',
  'pending_approval',
 ...
 */
export const connectorCredentialModeEnum = zedSchema.enum('connector_credential_mode', [
  'shared',
  'per_user',
]);

export const connectorAuthorizationStrategyEnum = zedSchema.enum(
  'connector_authorization_strategy',
  ['project', 'user'],
);

export const connectors = zedSchema.table(
  'connectors',
  {
...
export const connectorConnectionOwnerTypeEnum = zedSchema.enum(
  'connector_connection_owner_type',
  ['project', 'agent', 'member', 'subject', 'external'],
);

export const connectorConnectionStatusEnum = zedSchema.enum('connector_connection_status', [
  'active',
  'revoked',
  'error',
]);
...
export const connectorConnections = zedSchema.table(
...
export const projectSessionConnectorBindingSourceEnum = zedSchema.enum(
  'project_session_connector_binding_source',
  ['request', 'default'],
);

/** Durable alias -> concrete connection selection for one project session. */
export const projectSessionConnectorBindings = zedSchema.table(
...
export const connectorGrants = zedSchema.table(
  'connector_grants',
  {
    grantId: uuid('grant_id').defaultRandom().primaryKey(),
 ...
 */
export const connectionCredentials = zedSchema.table(
...
export const connectionOAuthApplications = zedSchema.table(
 ...
 */
export const connectionOAuthSessions = zedSchema.table(
...
export const connectorActions = zedSchema.table(
  'connector_actions',
  {
...
export const connectorPolicies = zedSchema.table(
  'connector_policies',
  {
    policyId: uuid('policy_id').defaultRandom().primaryKey(),
    connectorId: uuid('connector_id')
 ...
 */
export const connectionPolicies = zedSchema.table(
  'connection_policies',
  {
    policyId: uuid('policy_id').defaultRandom().primaryKey(),
    connectionId: uuid('connection_id').notNull(),
 ...
 * in zed.yaml. Patterns are fully-qualified (`<slug>.<path>` globs) and apply
 * across ALL connectors in the project; evaluated BEFORE any connector-scoped
 * rule. See docs/specs/connector.md §8.
 */
export const connectorProjectPolicies = zedSchema.table(
...
export const connectorDefaultModeEnum = zedSchema.enum('connector_default_mode', [
  'risk',
  'allow_all',
]);
 ...
 * today). Materialized from `policy` in zed.yaml; missing block = allow_all
 * for back-compat with existing projects.
 */
export const connectorProjectSettings = zedSchema.table('connector_project_settings', {
  projectId: uuid('project_id')
    .primaryKey()
    .references(() => projects.projectId, { onDelete: 'cascade' }),
...
export const connectorCalls = zedSchema.table(
  'connector_calls',
  {
    executionId: uuid('execution_id').defaultRandom().primaryKey(),
    accountId: uuid('account_id')
 ...
 */
export const connectorAttachments = zedSchema.table(
 ...
 */
export const sessionToolApprovals = zedSchema.table(
