...
ALTER TABLE "zed"."tunnel_audit_logs"
  DROP CONSTRAINT IF EXISTS "tunnel_audit_logs_tunnel_id_tunnel_connections_tunnel_id_fk";

COMMENT ON COLUMN "zed"."tunnel_audit_logs"."tunnel_id" IS
...
UPDATE "zed"."tunnel_device_auth_requests"
SET "setup_token" = NULL, "updated_at" = now()
WHERE "setup_token" IS NOT NULL AND "expires_at" < now();

COMMENT ON COLUMN "zed"."tunnel_device_auth_requests"."setup_token" IS
...
DELETE FROM "zed"."tunnel_rpc_forwards"
WHERE "expires_at" < now();

COMMENT ON TABLE "zed"."tunnel_rpc_forwards" IS
