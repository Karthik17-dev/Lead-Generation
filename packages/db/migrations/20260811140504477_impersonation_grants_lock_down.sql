...
-- Take zed.impersonation_grants OUT of the schema-wide default grants.
--
-- The `zed` schema carries standing default privileges from the baseline
-- (`ALTER DEFAULT PRIVILEGES IN SCHEMA zed GRANT SELECT, INSERT, UPDATE ON
...
-- `/rest/v1/impersonation_grants` with `Content-Profile: zed` -- verified on
...
REVOKE ALL ON TABLE "zed"."impersonation_grants" FROM anon;
--> statement-breakpoint
REVOKE ALL ON TABLE "zed"."impersonation_grants" FROM authenticated;
--> statement-breakpoint
-- Belt and braces: if a future blanket `GRANT ... ON ALL TABLES IN SCHEMA
-- zed` re-adds the privilege, RLS with no policy still denies every row to
...
ALTER TABLE "zed"."impersonation_grants" ENABLE ROW LEVEL SECURITY;
