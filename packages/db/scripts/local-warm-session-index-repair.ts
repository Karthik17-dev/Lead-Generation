...
export async function repairLocalWarmSessionIndex(
  ...
  try {
    const {
      ...
      select to_regclass('zed.project_sessions') is not null
         and exists (
           select 1
    ...
    try {
      await client.query('lock table zed.project_sessions in share row exclusive mode');
      ...
      await client.query(`drop index if exists zed.${INDEX_NAME}`);
      await client.query(`
