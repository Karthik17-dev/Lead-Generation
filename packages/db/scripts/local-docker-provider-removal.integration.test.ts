...
const PRE_MIGRATION_SCHEMA = `
  ...
  CREATE VIEW zed.workspace_sessions AS
    SELECT session_id, sandbox_provider FROM zed.project_sessions;
...
let cleanupRuntimeMigrations = () => {};
  ...
  test('removes the enum value when every affected table is empty', () => {
    ...
    expect(psql(`SELECT to_regclass('zed.workspace_sessions');`).output.trim()).toBe('');
  });
