...
describe('backfill-DML guard (centralized_audit_v2 outage class)', () => {
  test('rejects a top-level UPDATE without a backfill-safe sign-off', () => {
    const { errors } = lintMigration(
      GOOD_NAME,
      'ALTER TABLE zed.widgets ADD COLUMN kind text;\nUPDATE zed.widgets SET kind = \'x\' WHERE kind IS NULL;\n',
    );
  ...
  test('rejects a data-modifying WITH ... UPDATE CTE', () => {
    const { errors } = lintMigration(
      GOOD_NAME,
      'WITH ranked AS (SELECT id FROM zed.widgets)\nUPDATE zed.widgets w SET n = 1 FROM ranked r WHERE w.id = r.id;\n',
    );
    expect(errors.some((e) => e.includes('top-level DML'))).toBe(true);
  });
  ...
  test('rejects DML that follows a drizzle statement-breakpoint on the same line', () => {
    const { errors } = lintMigration(
      GOOD_NAME,
      "ALTER TABLE zed.widgets ADD COLUMN kind text;--> statement-breakpoint\nUPDATE zed.widgets SET kind = 'x' WHERE kind IS NULL;--> statement-breakpoint\n",
    );
    expect(errors.some((e) => e.includes('top-level DML'))).toBe(true);
  });
  ...
  test('ignores DML inside a dollar-quoted trigger function body', () => {
    const { errors } = lintMigration(
      GOOD_NAME,
      "CREATE OR REPLACE FUNCTION zed.widget_audit() RETURNS trigger LANGUAGE plpgsql AS $$\nBEGIN\n  INSERT INTO zed.audit_events(action) VALUES ('widget');\n  RETURN NEW;\nEND;\n$$;\n",
    );
    expect(errors).toEqual([]);
  });
