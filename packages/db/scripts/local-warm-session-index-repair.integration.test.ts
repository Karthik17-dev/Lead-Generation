...
const container = `zed-local-warm-index-${crypto.randomUUID().slice(0, 8)}`;
let databaseUrl = '';
...
describe.skipIf(!dockerAvailable)('local warm-session index repair — real PostgreSQL', () => {
  ...
  test('keeps the newest available row and rebuilds a valid unique index', async () => {
    psql(`
      insert into zed.project_sessions values
      ...
      update pg_index
       ...
       where indexrelid = 'zed.idx_project_sessions_one_available_warm'::regclass;
    `);
