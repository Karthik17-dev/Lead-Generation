...
const LIVE_ENV_FILES = [
  process.env.ZED_AGENT_ENV_FILE,
  "/dev/shm/zed/agent-env.sh",
  "/tmp/pt-env",
  "/etc/pt-env",
 ...
 * 3. live agent env file — `/dev/shm/zed/agent-env.sh` or warm-claim env files
 ...
 * (zed-master /env API) take effect immediately — no service restart needed.
...
export function getEnv(key: string): string | undefined {
  ...
  //    zed-master writes here on every /env POST, so values update without restart.
...
/**
 * Base URL for a Zed router-proxied upstream service, derived from
 * ZED_API_URL. The sandbox only ever holds ZED_API_URL + ZED_TOKEN;
 * tools build their proxy endpoint from those two. Normalizes so it works
 * whether ZED_API_URL is a bare origin or already ends in /v1 or /v1/router.
 * Returns null when ZED_API_URL is unset.
 */
export function getZedRouterBase(service: string): string | null {
  const raw = getEnv("ZED_API_URL");
  if (!raw) return null;
  const root = raw
    .replace(/\/+$/, "")
    .replace(/\/v1\/router$/, "")
