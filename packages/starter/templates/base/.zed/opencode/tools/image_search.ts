...
function getSerperImagesUrl(): string {
  const override = getZedRouterBase("serper");
...
async function enrichImages(images: EnrichedImage[]): Promise<EnrichedImage[]> {
  const replicateBaseUrl = getZedRouterBase("replicate") ?? undefined;
  // Route through the Zed router (derived from ZED_API_URL); auth with
  // ZED_SANDBOX_TOKEN (ZED_TOKEN kept as a legacy fallback). Fall back
  // to a raw REPLICATE_API_TOKEN only when unset.
  const replicateToken = replicateBaseUrl
    ? getEnv("ZED_SANDBOX_TOKEN") || getEnv("ZED_TOKEN")
...
export default tool({
  ...
  async execute(args, _context) {
    const serperUrlOverride = getZedRouterBase("serper") ?? undefined;
    // Route through the Zed router (derived from ZED_API_URL); auth with
    // ZED_SANDBOX_TOKEN (ZED_TOKEN kept as a legacy fallback). Fall back
    ...
    if (!apiKey) return serperUrlOverride
      ? "Error: ZED_SANDBOX_TOKEN not set."
