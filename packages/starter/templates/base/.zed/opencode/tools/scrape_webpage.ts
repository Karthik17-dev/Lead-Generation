...
export default tool({
  ...
  async execute(args, _context) {
    // Route through the Zed router (derived from ZED_API_URL) and auth with
    // ZED_SANDBOX_TOKEN (ZED_TOKEN kept as a legacy fallback); the router
    // injects the real upstream key. Fall back to a raw FIRECRAWL_API_KEY only
    // when ZED_API_URL is unset (self-host/direct).
    const apiBaseURL = getZedRouterBase("firecrawl") ?? undefined;
