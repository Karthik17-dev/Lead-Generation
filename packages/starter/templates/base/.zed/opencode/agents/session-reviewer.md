...
1. **Pull the transcript.**
   `zed sessions digest --all --json` and select your session, or use
...
2. **Reconstruct from what persists in git.** The session's branch (the
   ...
   - `zed cr ls --json` — CRs this session opened, and their outcomes
     (merged, rejected, stale).
3. **Work through it turn by turn / commit by commit.** You are looking
   for the failure signatures from the `zed-harness-refinement`
   protocol (full text: `zed skills get zed-harness-refinement`):
   ...
   - executable tools under `.zed/opencode/tools/` that raised
