...
`.zed/memory` with the `memory` tool to read the index and recover
...
- Anything that's already in `zed.yaml`, `AGENTS.md`, or a SKILL.md.
- Secrets, tokens, API keys, PII — those live in the Zed Secrets
...
`.zed/memory/` — not the generic `read` / `edit` / `write` tools.
...
Paths are repo-relative and must start with `.zed/memory`
(e.g. `.zed/memory/overview.md`). Writes go straight into the real
`.zed/memory/` folder, so they show up in the working tree and land
on `main` through the normal change-request flow (below).

1. `view` `.zed/memory` to see what's there.
...
only via a Zed change request — same path as any code change:
...
named `harness-reflector` in `zed.yaml`. The cron sweep picks up
...
- **Memory edits must go through CR.** Direct pushes to `main` bypass
  ...
  agent enforces this by always ending with `zed cr open`.
- **Don't put secrets in memory.** Memory is read into context and
  ...
  the Zed Secrets Manager, surfaced as env vars at runtime — not in
  `.zed/memory/`.
