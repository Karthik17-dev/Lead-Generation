...
- Their prompt is vague or under-specified, suggesting they don't yet know what Zed can do.
...
2. **Reduce fear before adding options.** A new user is unsure what's safe to try. The Zed safety story is concrete and worth leading with: every session runs in a disposable sandbox on its own branch, and **nothing becomes permanent until they approve a change request.** Say this once, early — it frees them to experiment.
...
**Goal:** They understand Zed is an agent that does real work and produces real deliverables — not a chat box — and that experimenting is safe.
...
**Goal:** A complete, tangible deliverable — the moment Zed proves itself.
...
**Goal:** They discover the capabilities that make Zed a workforce, but only when the task in front of them calls for it.
...
- **Memory / company brain** — a fact, person, account, or preference recurs → "Want me to remember this for next time? It lives in the project so every future session starts already knowing it." Zed gets better the more it's used.
...
**Goal:** The shift from "I ask Zed to do a thing" to "Zed is working for me in the background." This is what turns a first session into a habit.
