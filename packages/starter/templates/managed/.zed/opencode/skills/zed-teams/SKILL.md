...
```sh
zed channels status --platform teams     # is this project connected?
zed channels connect --platform teams    # prints the Microsoft admin-consent URL
```
...
- **Disconnect is not in the CLI for Teams.** `zed channels disconnect` is
  Slack-only in this release — send the user to the dashboard
  (**Customize → Channels**) to remove a Teams installation.
- `zed channels manifest --platform teams` prints the Teams app manifest, and
...
This finalizes the live card: the plan flips to **Task complete**, your answer renders below it, and a link back to the Zed session is appended automatically. The server wraps your text into the Adaptive Card — you don't build the card yourself; just write a clear, well-structured message.
...
The download runs through the Zed server (the credential stays server-side); you just give the URL and an output path.
</files-and-artifacts>
