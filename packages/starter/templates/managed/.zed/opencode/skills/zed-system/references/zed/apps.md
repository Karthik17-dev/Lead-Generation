...
Zed Apps deploy static sites and HTTP applications from a project. Each App
...
```sh
zed apps deploy . --slug storefront --name Storefront
```
...
The `apps:` map is available only in `zed_version: 2` YAML manifests.
...
```sh
zed apps deploy --manifest-app storefront
```

When the manifest declares exactly one App, bare `zed apps deploy` selects
...
`ZED_*`, or `OPENCODE_*`. A missing identifier fails deployment with
...
```sh
zed apps access storefront
zed apps access storefront --mode private
zed apps access storefront --mode project
zed apps access storefront --mode restricted --members <member-id> --groups <group-id>
zed apps access storefront --mode public
zed apps access storefront --mode password --password '<value>'
```
...
`zed apps deploy` accepts the same `--access`, `--password`, `--members`,
and `--groups` flags. Never store a password in `zed.yaml` or a source file.
Zed stores only an Argon2id hash.

Zed-authenticated users open a five-minute exchange URL. It creates an
...
```sh
zed apps access-link storefront --json
```
...
The packer always excludes `.git`, `.zed`, and `.env*` at every depth.
...
The App sandbox contains `zed-appd` and Caddy. `zed-appd` owns the user
...
`zed apps stop <app>` suspends compute immediately. The next authorized request
resumes the active runtime, waits for readiness, and then returns the App
response for that same request. `zed apps start <app>` warms the active
...
Browser navigation shows a Zed lifecycle page during queued, validating,
...
Each cold start checks the active `zed-appd` version. If it is old, Zed
...
```sh
zed apps show storefront
zed apps rollback storefront <deployment-id>
```

Rollback accepts only a `ready` deployment. Zed starts and checks the target
...
```sh
zed apps logs storefront
zed apps logs storefront <deployment-id> --after 100 --limit 500
```
