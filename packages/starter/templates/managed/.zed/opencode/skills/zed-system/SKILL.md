---
name: zed-system
description: "Canonical reference for Zed projects, Apps, the CLI, sessions, sandboxes, change requests, triggers, connectors, secrets, system skills, and OpenCode REST. Covers `zed.yaml` versions 1 and 2, serverless App deployments, OpenCode configuration, session identity, credential boundaries, and the complete OpenCode reference. Load when the user asks how Zed works, what Zed can do, how to deploy an App, how an agent discovers platform instructions, how to configure or test OpenCode, how to edit `zed.yaml`, how to use the `zed` CLI, how to land work through a change request, or how to schedule and automate work."
---

<skill name="zed-system">

<live-skills>
The `zed` CLI is the live source of truth for how Zed works. The Zed
**system skills** — `zed-system`, `zed-apps`, `zed-connectors`,
`zed-memory`, `zed-harness-refinement`, `zed-slack`,
`zed-computer`, `zed-voice`, `zed-marketplace` — are
...
- `zed system-skills` — list the Zed system skills.
- `zed system-skills get <name>` — print one skill's current SKILL.md body.
- `zed system-skills get <name> --full` — also include its referenced files.

`zed skills` is a permanent compatibility alias. Optional, non-system
skills are marketplace items. Browse them with
`zed marketplace list --type skill`.

Before answering anything about Zed internals — Apps, connectors and connections,
...
Zed is an AI command center where a workforce of agents does real work —
and the whole thing is **code you own**: a project is a git repo with a
`zed.yaml` at its root; a session is one conversation in its own
...
what Zed *is* rather than what others aren't.
...
**Full reference:** `.zed/opencode/skills/zed-system/references/capabilities.md`
— a worked example and fuller paragraph per capability, plus the complete
"what makes Zed different" framing for comparison questions. Load it
...
- "What can you do?" / "Can you do X?" / "How does Zed work?" / "How do I
  do Y in Zed?" / how Zed compares to other AI tools or assistants
- "Schedule this / remind me later / run this every morning / on a
  schedule" / "recurring task" / "cron job" / "webhook trigger"
- "What does `zed.yaml` do?" / "What is `zed_version`?"
...
- "What's the difference between `zed.yaml` and `opencode.jsonc`?"
- "How do I use or test OpenCode?"
- "How does an agent retrieve the current Zed system instructions?"
- "How do I customize the sandbox image?"
- "How do I deploy a website, Dockerfile, or OCI image?" / "How do Zed Apps work?"
...
- "How do change requests work in Zed?" / "What's `zed cr`?"
...
<cli>
You are running inside a Zed session sandbox. The **`zed` CLI**
is on `$PATH` (`/usr/local/bin/zed`) and pre-authenticated against
this exact project — a project-scoped token is already injected as
`$ZED_CLI_TOKEN`, with `$ZED_API_URL` pointed at the right host.
You can run `zed …` from any shell with zero setup. (Don't reach for
`$ZED_SANDBOX_TOKEN` (the deprecated `$ZED_TOKEN` alias still works too):
...
Zed cloud state — not just files in the repo. Examples:

| The user says… | Use… |
| --- | --- |
| "list / read project secrets" | `zed secrets ls` |
| "set / unset a secret" | `zed secrets set NAME=VALUE`, `zed secrets unset NAME` |
| "pull / push my `.env`" | `zed env pull`, `zed env push --from .env` |
| "what sessions are running right now?" | `zed sessions ls` *(add `--json` to parse)* |
| "show all parallel agents at a glance — what's everyone doing?" | `zed sessions status` *(mission control; `--all`, `--json`)* |
| "what is another agent / session doing right now?" | `zed sessions log <id>` *(read-only peek; `--json`)* |
| "talk to / pick a session to interact with" | `zed sessions chat` *(picker)* · `zed sessions chat <id> --prompt "…"` *(one-shot)* |
| "spawn another session / subagent to do X" | `zed sessions new --prompt "X" --json --wait` *(capture session_id)* |
| "restart / kill session `<id>`" | `zed sessions restart <id>` / `zed sessions rm <id>` |
| "fire the daily-digest trigger" | `zed triggers fire daily-digest` |
| "show open change requests" | `zed cr ls` |
| "who am I? what project is this?" | `zed whoami`, `zed projects info` |

**Everything is scriptable — drive Zed like the dashboard.** Every
...
`zed sessions ls --json` to see what's live, then `zed sessions log
<id>` to read what any one of them is doing right now (read-only — sends
nothing), or `zed sessions chat <id> --prompt "…"` to talk to it.
...
**Token scope reminder.** The CLI's token (`$ZED_CLI_TOKEN`) is
project-scoped — it cannot enumerate other projects or hit account-level
routes. Trying `zed projects ls` from inside the sandbox returns 403;
that's intentional. Use `zed projects info` to inspect **this** project.

**Secret capability discovery.** `$ZED_SECRET_CAPABILITIES` contains a
...
value. An `https_broker` entry must use `zed secrets call IDENTIFIER URL
[options]`; Zed adds the value server-side only after the request matches the
stored host, method, and path policy. A `zed_service` entry is available only
through its named service, such as a connector or the LLM gateway. Never print
or return a secret value or an opaque broker handle. Use `zed secrets ls
...
`zed-connectors` MCP (or `zed secrets request` /
`zed connectors connect`). The human gets a fill-in
...
connector or a secret. `zed channels connect` is the ONE command: it prints
a one-click "Add to Slack" install link (Zed Cloud) — surface that URL and
you're done. No manifest, no bot token, no secret-intake link. Details in the
`zed-slack` skill.

**Full reference:** `.zed/opencode/skills/zed-system/references/zed/zed-cli.md`
— every command, every flag, every env var, common workflows. Load it
when you need exact syntax.
</cli>

<apps>
## Zed Apps — deploy a website or container

An **App** is a project-scoped, serverless deployment with one stable Zed
...
New Apps are private. Use `zed apps access <app>` to select creator-only,
whole-project, restricted member/group, public, or password access. Never store
an App password in `zed.yaml`.

Use the CLI from the source directory:

```sh
zed apps deploy .                         # auto-detect static, bundle, or Dockerfile
zed apps deploy ./dist --type static
zed apps deploy . --type dockerfile --command '["bun","run","start"]' --port 3000
zed apps deploy --image ghcr.io/acme/api:1.4.2 --command '["/app/server"]' --port 8081
zed apps access storefront --mode restricted --members <member-id> --groups <group-id>
zed apps ls --json
```

For a repeatable deployment, declare an `apps:` map in a v2 `zed.yaml`, then
run `zed apps deploy --manifest-app <name>`. A single declared App becomes
the default for bare `zed apps deploy`. The manifest stores non-secret
...
**Full reference:**
`.zed/opencode/skills/zed-system/references/zed/apps.md` — workload
...
<marketplace>
The **Zed Marketplace** is the project skill library and the normal way to
...
zed marketplace show <name> --json
...
agents should not use `zed registry build/validate/publish`; those are
...
`.zed/opencode/skills/...`, updates `registry-lock.json`, and commits the
...
**Full reference:** `.zed/opencode/skills/zed-system/references/zed/marketplace.md`
...
`.zed/opencode/skills/<name>/SKILL.md`. The directory name must equal
...
reuse — `zed-system` itself is built this way. The `description` is
...
`.zed/opencode/skills` is the canonical git source for Zed-managed
skills, and OpenCode's own discovery path. At sandbox boot, Zed force-writes
the latest managed system skills into it, so a stale committed copy never wins.

**Full reference:** `.zed/opencode/skills/zed-system/references/authoring-skills.md`
...
Zed runs work on a schedule through **triggers** — a durable entry in
the project's `zed.yaml` (`triggers:`). When one fires, the platform
...
**Full references:**
- `.zed/opencode/skills/zed-system/references/zed/zed-yaml.md`
...
- `.zed/opencode/skills/zed-system/references/scheduling.md` — the
...
- `.zed/opencode/skills/zed-system/references/zed/zed-cli.md`
  — the `zed triggers ls/info/fire/enable/disable` command reference.
</scheduling>
...
under `.zed/`. Zed treats the harness as continuously improvable
from trajectory evidence, on two loops:

1. **In-session refinement (self-invoked)** — every agent runs the
   four-pass protocol from the `zed-harness-refinement` skill over its
...
costs it twice — load `zed-harness-refinement`
(`zed skills get zed-harness-refinement` if it is not on disk)
for the failure signatures, the four passes, and the guardrails (never
edit managed `zed-*` skills, never merge your own harness CR, no-op
is valid).
</continual-harness>

<change-requests>
**This is the single most important rule for any agent running in a
Zed session: if you want your work to land on `main`, you MUST open
a change request (CR).**
...
4. **Open a CR.** From inside the sandbox the CLI reads
   `$ZED_BRANCH_NAME`, `$ZED_SESSION_ID`, and `$ZED_SANDBOX_TOKEN`
   (deprecated alias: `$ZED_TOKEN`) automatically:
   ```sh
   zed cr open \
...
6. **Surface the CR to the user.** Print the CR number so they can
   ...
   zed cr ls
   ```
7. **Wait.** The user merges via dashboard, CLI (`zed cr merge
...
| ------------- | ----------------------------------------------------------------------------------------- |
| Sandbox       | CR is opened from inside the sandbox via `$ZED_SANDBOX_TOKEN` (deprecated alias: `$ZED_TOKEN`). Branch tip is the session HEAD. |
| Dashboard     | Renders the CR — title, description, diff, merge preview, conflict markers.               |
| CLI           | `zed cr ls / show / diff / open / merge / close / reopen` — full life-cycle locally.   |
| `zed.yaml` | Edits to triggers / env land via CR like any other file.                                  |
| Skills        | New OpenCode skill files reach future sessions **only** after a CR merges. Managed Zed system skills also receive the deployed host overlay. |
| Triggers      | Cron / webhook trigger edits reach the scheduler **only** after the CR merges to `main`.  |

Full reference: `.zed/opencode/skills/zed-system/references/zed/change-requests.md`.
</change-requests>
...
| --- | --- | --- | --- |
| Zed config | Zed | `zed.yaml` + optional custom sandbox files | Zed platform |
| OpenCode native config | OpenCode | `.zed/opencode/` | OpenCode |

Version 2 declares OpenCode's config directory through
`opencode.config_dir`.

Do not duplicate OpenCode config in `zed.yaml`. The manifest owns
...
triggers and env round-trip through `zed.yaml`.
</contract>

<canonical-schema>
## The canonical manifest schema — one URL, always correct

The starter uses `zed_version: 2`. Check the top
`# yaml-language-server: $schema=...` line in `zed.yaml`. That URL is the
public, versioned JSON Schema, generated from `@zed/manifest-schema` (the
same package that backs `zed validate` and the CR-merge gate — one source
of truth, no separate spec to keep in sync by hand):

| URL | Covers |
| --- | --- |
| `https://zed.com/schema/zed.v2.schema.json` | `zed_version: 2` OpenCode governance map |
| `https://zed.com/schema/zed.v1.schema.json` | `zed_version: 1` only (legacy `[[agents]]` array + `[[channels]]`) |
| `https://zed.com/schema/zed.schema.json` | All published versions; dispatches on `zed_version` |

`zed schema` (from any session — the CLI is always pre-authenticated, see
`<cli>` above) prints the same document locally: `zed schema --version 2`,
or `zed schema --url` for just the URL. If you are AUTHORING or EDITING
`zed.yaml` and unsure whether a field/shape is legal, this schema — not
this skill's prose, which can drift — is the authoritative structural spec;
`zed validate` is the authoritative behavioral one (it also catches
...
`enabled`/`sandbox`/`connectors`/`secrets`/`skills`/`zed_cli`/`workspace`. `env` was
...
behavior and lives in that agent's own `.zed/opencode/agents/<name>.md`
frontmatter, joined by name (this project's `zed` and `harness-reflector`
...
grant set (an omitted `connectors`/`secrets`/`skills`/`zed_cli` resolves
to `none`, not `all`).
...
```yaml
agents:
  release-bot:                          # = the agent's .md name (.zed/opencode/agents/release-bot.md)
    sandbox: ml                         # default environment for this agent
    connectors: [github]                # which connectors it may call   (default: none)
    zed_cli: [project.write, project.cr.open]    # what it may do via the Zed CLI/API (default: none)
```
...
| Setting | Lives in |
| --- | --- |
| v2 system prompt, `model`, `mode`, tools, and `permission` | `.zed/opencode/agents/<name>.md` and `opencode.jsonc` |
| connectors, secrets, skills, `zed_cli`, workspace, enabled | manifest `agents:` map |

**How the grant resolves at session start:**
- v2 (`zed.yaml`) is **deny-by-default**: an omitted `connectors`/`secrets`/`skills`/`zed_cli` on a declared agent resolves to `none`, not `all`. `default_agent` is required and must resolve to a declared, enabled agent — give it `connectors: all`, `secrets: all`, `zed_cli: all`, `skills: all` explicitly if it should keep full access.
- v1 (`zed.toml`, legacy) is **backward-compatible** instead: manifest has **no `[[agents]]`** at all → no agent-grant restriction, agents discovered straight from OpenCode. Agent **is listed** → its `connectors`/`zed_cli` (default each = none if omitted). Manifest **has `[[agents]]` but this agent isn't listed** → default-deny for Zed grants. The v1 default agent keeps **full access** only while `[[agents]]` is unadopted — the moment you add `[[agents]]`, declare the default agent too or it falls under the unlisted-deny rule.
...
- Once a project adopts declarative agents, Zed chat inputs, trigger/channel pickers, and other product UI should fetch agents from the server-side Zed registry, not directly from the sandbox OpenCode `/app/agents` result.
...
- New projects use v2 declarative discovery. Older `zed.toml` (v1)
  projects stay in legacy mode until they migrate.

**`zed_cli` — the grantable enum** (project-scoped only; account-level admin actions
...
specific project). Run `zed validate --scopes` to print this list:
...
`zed validate` validates `agents:` (v2) / `[[agents]]` (v1) — rejecting unknown /
account-scoped actions — and prints each agent's resolved scope. Use `zed validate --scopes`
to see the full enum.
</agent-authorization>

<references>

<reference path=".zed/opencode/skills/zed-system/references/capabilities.md">
  ...
  memory, scheduling, channels, subagents, models), "What Zed is," and
  "What makes Zed different" for comparison questions. Load whenever a
  capability question needs more than the one-liner in SKILL.md.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/zed/credentials-and-setup-links.md">
  ...
  (the `request_secret` + `connect` MCP tools, or the `zed secrets request` /
  `zed connectors connect` CLI), what the human sees
...
<reference path=".zed/opencode/skills/zed-system/references/zed/zed-cli.md">
  In-depth `zed` CLI reference. Every subcommand (login, hosts,
  ...
  session sandbox** (where `ZED_CLI_TOKEN` + `ZED_API_URL` are
  pre-injected so `zed sessions ls`, `zed secrets set FOO=bar`,
  `zed cr ls` all work out of the box). Load this when you want to
  drive the Zed cloud from a terminal or agent.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/zed/apps.md">
  Zed Apps deployment and operations reference. Covers static, bundle,
...
<reference path=".zed/opencode/skills/zed-system/references/zed/marketplace.md">
  In-depth Zed Marketplace reference. What the marketplace is, when to use
  ...
  and the difference between consumer `zed marketplace` commands and
  developer-only `zed registry` commands.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/authoring-skills.md">
...
<reference path=".zed/opencode/skills/zed-system/references/zed/zed-yaml.md">
  In-depth `zed.yaml` reference. Covers versions 1 and 2; logical agents;
  ...
  legacy note on the v1 `zed.toml` TOML format. Load this when
  editing or debugging the manifest.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/scheduling.md">
  ...
  `zed-yaml.md` (schema) and `zed-cli.md` (commands). Load whenever
  the user wants something scheduled, recurring, reminded, or
  webhook-triggered.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/zed/change-requests.md">
  Full Zed change-request reference. The data model (the
  ...
  reopen path), the CLI surface (`zed cr ls / show / diff / open /
  ...
  or asks how Zed handles the GitHub-PR gap.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/opencode/overview.md">
  How OpenCode fits into a Zed project — where each primitive lives
  under `.zed/opencode/`, how the same dir drives both the remote
  sandbox and local `opencode` runs — plus the index into the per-feature
  pages mirrored from opencode.ai/docs/.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/opencode/agents.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/skills.md">
  On-demand `SKILL.md` definitions. Discovery paths, frontmatter rules,
  name validation, permission gating. Mirrored from
  <https://opencode.ai/docs/skills/>.
</reference>

<reference path=".zed/opencode/skills/zed-system/references/opencode/commands.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/tools.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/plugins.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/mcp-servers.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/permissions.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/rules.md">
...
<reference path=".zed/opencode/skills/zed-system/references/opencode/models.md">
...
- **The workspace IS global — sessions are not.** A Zed project is
  ...
  sanctioned path is `zed cr open` → user review → merge.
...
- **OpenCode behavior remains OpenCode-native.** Skills, commands, tools,
  ...
  a logical agent in `agents:` is a separate Zed decision.
...
- **Manifest schema is versioned.** `zed_version` lets the platform
