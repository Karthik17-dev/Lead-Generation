# What Zed can do — capabilities reference
...
capability, or the "what makes Zed different" framing for a comparison
question.
...
- For *configuration* questions (the `zed.yaml` manifest, the `zed`
  CLI, change requests, authoring agents/skills/tools), the rest of the
  `zed-system` skill is the canonical reference — this page is about
...
a single git repository with a `zed.yaml` at its root, holding your
...
**Research and the web.** Zed searches the live web and fetches specific
...
form, or renders behind JavaScript — Zed can drive a real browser inside
...
machine. Zed can write and run Python, Node, and shell; install any
...
**Documents.** Zed produces finished, shareable deliverables: **PDF**,
...
**Media.** Zed can generate images and short videos, produce
...
**Websites and apps.** Zed builds websites and web apps directly in the
...
— see the `zed-yaml.md` reference's `apps:` section for the deploy
surface.

**Connectors and connections.** Zed reaches **3,000+ apps** through
...
**Memory.** Zed builds a living, file-based "company brain" — context
...
the more the project is used. Load the `zed-memory` system skill for
specifics.

**Scheduling and triggers.** Zed can run work automatically. **Cron
...
Zed sessions where the team already works — so people kick off and
steer agents without leaving their channel. See the `zed-slack` system
skill.

**Subagents and parallelism.** Zed divides large jobs across
...
**Models.** Zed is model-agnostic: bring your own provider and keys (or
...
When comparing Zed to other tools, frame what Zed *is* — owned,
