...
> run `zed channels connect` — it prints a one-click "Add to Slack" install
...
secret simply appears in your session env (and in `ZED_PROJECT_SECRET_NAMES`).

**Preferred — the `request_secret` tool on the `zed-connectors` MCP:**
...
```sh
zed secrets request APOLLO_API_KEY SMARTLEAD_API_KEY     # several keys, one link
zed secrets request APOLLO_API_KEY --scope connector     # server-side only
```
...
request**: the `add_connector` tool / `zed connectors add <slug> --provider
pipedream --app <app> --apply`. That commits it to
`zed.yaml` on main and syncs the catalog server-side, exactly like the
dashboard's "Add app" — it's live this session. Then mint the connect link.

**Preferred — the `connect` tool on the `zed-connectors` MCP:**
...
```sh
zed connectors connect smartlead  # matches the MCP `connect` tool
```
...
`zed connectors connect` returns the durable, modal-friendly connection URL.
...
3. When a **secret** is submitted while your session is running, the platform
   ...
   - **Secret:** check the name is now present — `zed secrets ls` (or look for
     it in `ZED_PROJECT_SECRET_NAMES`). A fresh `runtime` value is live in the
     session env immediately (it's hot-synced; no restart needed).
   - **Connector:** check it now appears in your usable catalog —
     `zed connectors ls` (the `connectors` MCP tool). Unconnected connectors are
...
- ❌ `zed secrets set NAME=<value>` — you don't *have* the value, and you
  shouldn't.
...
| Goal | MCP tool | `zed` CLI |
| --- | --- | --- |
| Ask the human for a secret value | `request_secret` | `zed secrets request <NAME…>` |
| Get an app connected (Pipedream) | `connect` | `zed connectors connect <slug>` |
| Verify a secret arrived | — | `zed secrets ls` |
| Verify a connector connected | `connectors` | `zed connectors ls` |
...
`zed connectors` CLI exposes the same connector gateway for shell use.
