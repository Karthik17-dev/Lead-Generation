# Zed Change Requests — full reference

A **change request** (CR) is Zed's PR-equivalent. It proposes
merging one branch (`head_ref`) into another (`base_ref`) inside a
single Zed project. The CR layer is **Zed-native** — it works
...
Zed API against whatever backend the project's `repo_url` points
to.
...
2. **Commit on the session branch** (`$ZED_BRANCH_NAME`). Small,
...
to accept `zed cr show 3` — `3` resolves to the row with
...
```
                       open
                        ...
                        │   ◀──────────────────────┘      │
                        │                                 │
                        ├── zed cr merge  ──▶  merged ─┘ (terminal)
```

- `open` is the starting state.
- `closed` is reversible: `zed cr reopen <n>` puts it back to
...
The CLI's `zed cr diff` falls back to no color when stdout isn't a
...
`zed cr show <cr>` calls this automatically for open CRs and
...
3. Commit + push.
4. `zed cr show <cr>` to confirm the preview is now clean.
5. Hand back to the user to merge.

## CLI surface

The full surface is in `zed-cli.md` alongside this doc. Summary:

| Command                                      | What it does                                            |
| -------------------------------------------- | ------------------------------------------------------- |
| `zed cr ls [--status open\|merged\|closed\|all]` | List CRs on the project. Default: `open`.        |
| `zed cr show <cr>`                        | Metadata + merge preview.                              |
| `zed cr diff <cr> [--no-color]`           | Unified patch.                                          |
| `zed cr open --title "..." [--description "..."] [--head <ref>] [--base <ref>]` | Open a CR. |
| `zed cr merge <cr> [--message "..."]`     | Merge it (fast-forward or three-way).                   |
| `zed cr close <cr>`                       | Close without merging.                                  |
| `zed cr reopen <cr>`                      | Reopen a closed CR.                                     |
...
When `zed cr open` runs inside a session sandbox:

- `--head` defaults to `$ZED_BRANCH_NAME` (or `$ZED_HEAD_REF`).
- `--session` defaults to `$ZED_SESSION_ID`, which back-fills
...
```sh
zed cr open --title "Add release-notes skill" \
...
| ------------- | ------------------------------------------------------------------------------------------------ |
| **Sessions**  | A CR's `origin_session_id` is back-filled from `$ZED_SESSION_ID` so the dashboard shows the session that opened it. Cascade is `ON DELETE SET NULL` — closing the session orphans the link, doesn't delete the CR. |
| **Skills**    | New `.zed/opencode/skills/<name>/SKILL.md` files reach future sessions **only** after a CR that contains them merges to `main`. Until then, only the originating session sees them. |
| **Agents**    | Same: new `.zed/opencode/agents/<agent>.md` files need to land via CR.                       |
| **Triggers**  | Edits to `triggers:` in `zed.yaml` only reach the scheduler after the CR merges. The scheduler reads `zed.yaml` on `main`. |
| **Secrets**   | Decoupled. Secrets live in the Zed Secrets Manager, not the manifest; CRs don't move secrets. |
...
- **Merge is `Zed <noreply@zed.ai>`-authored.** If you want the
...
- **The `ZED_*` env vars expected at `cr open` time:**
  `ZED_CLI_TOKEN` (the project-scoped PAT the CLI authenticates with —
  **not** `ZED_TOKEN`, which is the sandbox service key and is rejected
  by the CR routes), `ZED_API_URL`, `ZED_PROJECT_ID`,
  `ZED_BRANCH_NAME` (or `ZED_HEAD_REF`), `ZED_SESSION_ID`.
  All of these are pre-injected by the session bootstrap. If you're
  running `zed cr open` *outside* a session (e.g. on your laptop)
  you'll need to pass `--head` and `--project` explicitly, or be on a
  cwd linked via `zed projects link`.

## See also

- `zed-cli.md` (next to this file) — full CLI surface for `zed
  cr`.
...
- `packages/db/src/schema/zed.ts` — schema source (`changeRequests`
