# `zed.yaml` — in-depth reference

`zed.yaml` is the single source of truth for everything the
Zed platform reads about a project. It lives at the repo root.
Any repo with a valid `zed.yaml` (or, for legacy v1 projects, a
`zed.toml`) at the root is a Zed project.
...
This page documents `zed_version: 2`, which uses OpenCode REST and a
governance-only `agents:` name-to-block map.

The authoritative structural spec is the public JSON Schema:
`https://zed.com/schema/zed.v2.schema.json`, or
`https://zed.com/schema/zed.schema.json` for all published versions.
Use `zed schema --version 2` offline.

> **Legacy note — v1 used TOML.** Projects created before v2 shipped
> may still have a `zed.toml` at the root with `zed_version: 1`
> (or no `zed_version` at all). The platform resolves the manifest
> by trying `zed.yaml`, then `zed.yml`, then falling back to
> `zed.toml` — so a v1 TOML project keeps working as-is; nothing
> breaks. To move a project onto v2, rename `zed.toml` to
> `zed.yaml`, convert its contents to YAML, bump `zed_version` to
> `2`, add the now-required `default_agent`, and rework any `[[agents]]`
> array into the `agents:` map described below (or run `zed migrate`
> once available). `zed_version: 2` manifests must be YAML — TOML
> only supports `zed_version: 1`.

## Version 2 example

```yaml
# yaml-language-server: $schema=https://zed.com/schema/zed.v2.schema.json
zed_version: 2
...
# OpenCode runtime config dir. Defaults to ".zed/opencode" when
# omitted. The agent daemon launches opencode with
# OPENCODE_CONFIG_DIR pointed here. OpenCode-native runtime config
# remains in this directory; Zed-side launchability and grants live
# in the `agents:` map below.
opencode:
  config_dir: .zed/opencode

# ─── Apps ─────────────────────────────────────────────────────────
# Local, repeatable deployment defaults. `zed apps deploy` remains the
...
agents:
  ...
  release-bot:
    sandbox: ml
    connectors: [github]
    zed_cli: [project.write, project.cr.open]    # may OPEN a CR, but not merge it
```
...
`.zed/opencode/` and `opencode.jsonc`; the manifest's `agents:` map
declares which agents Zed should treat as platform-launchable and
what server-side authority each one receives. Keyed by the agent's name
(matches its `.zed/opencode/agents/<name>.md`).

`agents:` is **required** in v2 and is **deny-by-default**: an omitted
`connectors`/`secrets`/`skills`/`zed_cli` on a declared agent
...
| `zed_cli` | What it may do via the Zed CLI/API (project-scoped iam actions). Same shape (default: `none`). |
...
**Grantable `zed_cli` actions** (project-scoped only — account-level admin
actions can never be granted to an agent; run `zed validate --scopes`):
...
give it `connectors: all`, `secrets: all`, `zed_cli: all`,
...
or runtime internals. Zed product UI (chat input, triggers, channels)
...
`zed_version` is the schema version. Version 2 is YAML-only and requires
...
it ensures `zed_version` is the first key, so the file is
...
| Session token mint     | `agents:` (per-agent connectors/secrets/skills/zed_cli scope)     |
...
in the **Zed Secrets Manager** — never inline.
...
**`ZED_*` is only reserved at the Secrets Manager surface**, not
at manifest parse time. The dashboard rejects creating secrets with
that prefix, but you can list `ZED_FOO` in `env:` without an
...
| Key          | Default              | Notes                                                                            |
| ------------ | --------------------- | -------------------------------------------------------------------------------- |
| `config_dir` | `.zed/opencode`   | Repo-relative dir. Same silent-fallback behavior as `sandbox:` paths.            |
...
Do not duplicate those details in `zed.yaml`; use `agents:` only for the
Zed-side decision of which agents are launchable/authorized by the platform.
...
(`ZED_TRIGGER_SCHEDULER_INTERVAL_MS`), so sub-minute precision is
best-effort.
...
- Primary header: `X-Zed-Signature: sha256=<hmac>`. The `sha256=`
...
1. Declare the secret name under `env:`:
   ...
   env:
     ...
     optional:
       ...
       - WEBHOOK_SLACK_SECRET
   ```
2. Set the value in the Zed Secrets Manager (dashboard).
...
- `ZED_*` is reserved **at the Secrets Manager surface** — the
  CRUD endpoint rejects it. The manifest parser does not enforce
  this; declaring `ZED_FOO` in `env:` is accepted but no matching
