...
This page is the **how to think about it** companion to `zed-yaml.md`
(which has the full `triggers:` field-by-field schema) and `zed-cli.md`
(which has the `zed triggers …` command reference). Read this when you're
deciding *whether and how* to schedule work, not just what fields exist.

Zed runs work on a schedule through **triggers** — a small, durable piece
of config in the project's `zed.yaml`. When a trigger fires, the platform
...
codes) lives in `zed-yaml.md`'s `## triggers:` section — this page
assumes you already have that shape and covers the judgment calls around it.

## Cron syntax cheat sheet

Zed uses **croner**, 6-field: `second minute hour day-of-month month
...
After adding a recurring trigger, run `zed triggers fire <slug>` once to
...
etc.) — discover the action (`zed connectors discover "send email"`) and call
it.
...
conversation context" primitive** in Zed. A session turn either
...
- **Stop temporarily:** `zed triggers disable <slug>` (sets
  `enabled: false`).
- **Stop permanently:** remove the `triggers:` entry from `zed.yaml` and
...
- **Stop *all* of a project's triggers at once:** use the project-level
  `triggers_paused` kill-switch (dashboard) — see `zed-yaml.md`'s
...
`X-Hub-Signature-256` is accepted natively — see `zed-yaml.md`'s
...
- [ ] Tested once with `zed triggers fire <slug>` before trusting the
      schedule.
