...
specification** and land it in a Zed project the right way.
...
First, though: **search the Zed marketplace before you build.** A skill
that already exists is better than one you write. Run `zed marketplace
search "<topic>" --json`, inspect close matches with `zed marketplace
...
rebuild on every run. (`zed-system` itself follows this pattern — this
very file is one of its `references/`.)

## Where skills live in Zed
...
The directory name **must equal** the `name` in the frontmatter. Zed-managed
...
- **A new skill reaches future sessions only after a change request
  ...
  open a CR (`zed cr open`). See the `<change-requests>` section above.
...
5. **Create the files.** `.zed/opencode/skills/<name>/SKILL.md`,
...
1. **Spec validation.** If the `agentskills` validator is available, point
   ...
   ```bash
   agentskills validate .zed/opencode/skills/<name>/
   ```
...
- **Persist it in the project.** A skill file on a session branch is gone
  ...
  HEAD`, then `zed cr open --title "Add <name> skill" --description
...
- **For reuse across projects**, move the skill into a marketplace/registry
  source and index it there, so other projects can `zed marketplace
  install` it instead of copy-pasting. (Registry *authoring* commands —
  `zed registry build/validate` — are developer tools, distinct from
  the consumer `zed marketplace` surface described in this skill's
  `<marketplace>` section.)
