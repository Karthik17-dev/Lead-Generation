...
<skill name="zed-harness-refinement">
...
|---|---|---|
| **Prompts** | `.zed/opencode/agents/<name>.md` (body) | Each agent's instructions and strategy |
| **Sub-agents** | `.zed/opencode/agents/*.md` | Specialist agents the orchestrator invokes |
| **Skills** | `.zed/opencode/skills/<name>/SKILL.md` | Reusable routines: text heuristics and guides |
| **Tools** | `.zed/opencode/tools/*.ts` | Executable code: wrappers, scripts, automations |
| **Memory** | `.zed/memory/` | Persistent facts, strategies, observations |
...
- **Exception-raising code** — an executable tool or script in
  `.zed/opencode/tools/` raised; you worked around it instead of
...
**Pass 4 — Memory (ΔM).** Follow the `zed-memory` skill's rubric with
...
1. **Enumerate** every session in the window:
   `zed sessions digest --since 24h --json` (add `--all` for a first
...
3. Push and open (or update) ONE change request per session for harness
   ...
   zed cr open --title "harness: <summary>" \
...
<guardrails>
- **Never edit managed `zed-*` skills.** They are platform-owned and
