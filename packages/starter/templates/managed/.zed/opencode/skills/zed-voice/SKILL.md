...
<the-actions>
The voice channel is the **`zed_voice`** connector. Call it like any connector:

```sh
zed connectors call zed_voice spawn_room '{}'
zed connectors call zed_voice read_transcript '{}'
zed connectors call zed_voice send_prompt '{"text":"The deploy finished — all green."}'
zed connectors call zed_voice end_call '{}'
```

Or, through the connector `call` tool: `{ "connector": "zed_voice", "action": "read_transcript", "args": {} }`.
...
If `zed connectors ls` shows no `zed_voice` at all, voice is not
...
link. Calls run on Zed's own infrastructure, so enabling the feature is the
whole setup. There is also no `zed` CLI command for this toggle; it is a
...
- `role: "agent"`, `speaker: "zed"` — something **you** put into the call (a `send_prompt`, or an answer spoken on your behalf when a turn finished).
...
```sh
zed connectors call zed_voice read_transcript '{"mode":"last","limit":10}'   # newest 10, whatever you have read
zed connectors call zed_voice read_transcript '{"mode":"full"}'              # the entire call
zed connectors call zed_voice read_transcript '{"peek":true}'                # unread, without consuming it
zed connectors call zed_voice read_transcript '{"cursor":41}'                # everything after cursor 41
```
