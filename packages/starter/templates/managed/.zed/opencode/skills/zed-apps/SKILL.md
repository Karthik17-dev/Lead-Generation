---
name: zed-apps
description: "Deploy and operate provider-neutral Zed Apps through the pre-authenticated CLI or SDK. Use when the user asks to publish, host, deploy, preview, inspect, wake, suspend, roll back, debug, or remove HTML/CSS/JavaScript, a static SPA, Vite or React source, Next.js, a Dockerfile service, or an OCI image on a stable Zed URL."
---

# Zed Apps

Zed Apps turns one source tree or image into an immutable serverless
deployment with a stable URL. Zed selects and operates the sandbox provider.
...
2. Run `zed projects info --json` to confirm the selected project. Read its
...
4. Do not create an empty App identity first. `zed apps deploy` creates the
   identity when `--app` is omitted.
5. Never run `zed apps deploy` from an uninspected workspace root. It can
...
```bash
zed apps deploy ./dist --slug storefront --name Storefront --type static --spa
```

Deploy a source bundle:

```bash
zed apps deploy . --slug storefront --type bundle --spa
```

Deploy a Dockerfile service:

```bash
zed apps deploy . --slug api --type dockerfile \
...
```bash
zed apps deploy --image nginx:1.27-alpine --slug nginx \
...
```bash
zed apps access <app>
zed apps access <app> --mode private
zed apps access <app> --mode project
zed apps access <app> --mode restricted --members <member-id> --groups <group-id>
zed apps access <app> --mode password --password '<value>'
zed apps access <app> --mode public
```
...
password into `zed.yaml`, source, logs, or a command shown to another user.
Zed stores only an Argon2id hash. A policy update revokes existing App
browser sessions.

Create a short-lived authenticated browser link without changing the policy:

```bash
zed apps access-link <app> --json
```
...
2. Read the stable URL from `app.url` in the `deploy`, `show`, or `access-link`
   ...
   `zed apps access-link <slug> --json`, follow redirects, and retain the
...
7. Run `zed apps stop <slug> --json`. The command returns only after the
...
8. Re-read `zed apps show <slug> --json`. Confirm the active deployment did
...
9. Reuse a browser profile that is already signed in to Zed as the App user.
...
Use the immutable deployment id from `zed apps show <slug> --json`:

```bash
zed apps logs <slug> <deployment-id> --limit 200
```
...
a deployment id, record the last visible phase and inspect `zed apps ls`.
...
zed apps start <slug>
zed apps stop <slug>
zed apps rollback <slug> <deployment-id>
zed apps delete <slug> --yes
```
...
On every cold start, Zed compares the active deployment's App supervisor
version with the current platform version. Zed queues at most one immutable
