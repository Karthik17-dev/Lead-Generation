...
import { getEnv, getZedRouterBase } from "./lib/get-env";

// People search is powered by an Apify LinkedIn-profile-search actor. Like
// web_search, it runs in two modes: through the Zed router by default (Zed
// injects the APIFY_TOKEN and bills the account — zero config for the user), or
// against a raw APIFY_TOKEN when ZED_API_URL is unset (self-host / opt-in).
//
...
export default tool({
  ...
  async execute(args, _context) {
    const routerBase = getZedRouterBase("apify");
    const zedToken = getEnv("ZED_TOKEN");
    const apifyToken = getEnv("APIFY_TOKEN");

    let url: string;
    let bearer: string;
    if (routerBase && zedToken) {
      url = `${routerBase}${RUN_PATH}`;
      bearer = zedToken; // the router validates this and injects the real APIFY_TOKEN
    } else if (apifyToken) {
      ...
      return JSON.stringify(
        {
          success: false,
          error:
            "people_search is unavailable: no Zed router (ZED_API_URL / ZED_TOKEN) and no APIFY_TOKEN set.",
        },
        null,
        2,
      );
