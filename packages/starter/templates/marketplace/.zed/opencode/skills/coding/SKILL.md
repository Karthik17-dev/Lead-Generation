...
Zed gives you two delegation paths. Choosing the right one is the whole game.
...
- **An external GitHub repo.** Spawn a session and have it clone the target itself with `gh repo clone <org>/<repo>` (or `git clone`). Open the prompt with: *"Clone https://github.com/org/repo into your workspace, then work there."* Make sure GitHub is connected first — if it isn't, mint a setup link to connect it before spawning (see the `zed-system` credentials reference) rather than handing the session a repo it can't reach.
...
1. **Check project memory** — `view .zed/memory` for the repo, project, or related names.
