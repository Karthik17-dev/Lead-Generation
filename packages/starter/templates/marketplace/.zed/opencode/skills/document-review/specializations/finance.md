...
1. **A finance-data connector (if configured).** If the project has a market-data / financials connector set up through Zed connectors, query it for the metric — it returns standardized, as-reported figures. Resolve the ticker first, pin the fiscal period from the document's date, then fetch.
