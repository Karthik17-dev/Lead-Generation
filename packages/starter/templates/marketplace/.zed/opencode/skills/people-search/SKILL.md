...
```sh
zed connectors ls                # or the `connectors` MCP tool
```
...
**Wiring up a connector (when none exists).** Don't send the user to a dashboard — use the credentials flow (`zed-system` → *Credentials & setup links*):
- Pipedream-backed app (most prospecting tools): `zed connectors add apollo --provider pipedream --app apollo --apply`, then `zed connectors connect apollo` to mint a 1-click link.
- API-key provider (e.g. a People Data Labs key): `zed secrets request PDL_API_KEY --scope connector`.

Surface the URL, end your turn, and verify it landed (`zed connectors ls` / `zed secrets ls`) when the user returns.
