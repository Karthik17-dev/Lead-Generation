...
### Zed Design System (Fallback Palette — ONLY for Zed-branded sites)

This palette is ONLY for sites that are explicitly Zed-branded. For everything else, run the palette generator.

```css
/* ZED DESIGN SYSTEM — warm beige surfaces, teal primary accent */
...
- `color-mix(in oklab, ...)` for opacity adjustments. Custom palettes: replace Zed defaults but keep variable names + both modes.
...
### Zed HSL Equivalents (for Tailwind / shadcn projects)

When using the fullstack webapp template (Tailwind + shadcn), `index.css` uses HSL values in `H S% L%` format (no `hsl()` wrapper). Below are Zed HSL conversions for when Zed is the appropriate fallback (see "Art Direction First" above). For inferred or custom palettes, convert your chosen colors to the same `H S% L%` format:
