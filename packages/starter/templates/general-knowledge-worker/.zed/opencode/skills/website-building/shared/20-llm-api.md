...
video services). Don't reuse a Zed platform credential name for a
...
| --- | --- | --- | --- |
| `shared/llm-api/generate_image.py` | Text-to-image and img2img edits (OpenAI) | `await generate_image(prompt, image_bytes=..., aspect_ratio=...)` | `OPENAI_API_KEY` (optional `OPENAI_BASE_URL` → Zed gateway) |
