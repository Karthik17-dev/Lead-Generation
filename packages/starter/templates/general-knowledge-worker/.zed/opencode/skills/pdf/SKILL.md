...
- **Author** should default to `"Zed"` unless the user asks for a different organization or author name
- **Title** should be a descriptive name relevant to the document contents

Canvas API: `c.setTitle(...)`, `c.setAuthor("Zed")` right after creating the canvas.
SimpleDocTemplate: pass `title=...`, `author="Zed"` as constructor kwargs.
pdf-lib (JS): `doc.setTitle(...)`, `doc.setAuthor("Zed")`.
