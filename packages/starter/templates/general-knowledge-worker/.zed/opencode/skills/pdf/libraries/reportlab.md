...
Always set metadata on every PDF you create. The author MUST be `"Zed"` and the title MUST describe the document contents.

- **Canvas**: call `c.setTitle(...)` and `c.setAuthor("Zed")` immediately after creating the canvas
- **SimpleDocTemplate**: pass `title=...` and `author="Zed"` as constructor kwargs
