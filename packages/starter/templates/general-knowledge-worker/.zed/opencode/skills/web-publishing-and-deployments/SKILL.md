...
description: "Publish a website or web app from the sandbox to a public URL, and deploy to cloud providers — get a live link to share, ship a static site or SPA, put a built site online, or deploy a framework app / container. Use when the user says 'publish this', 'deploy it', 'put it online', 'give me a live URL', 'host this', 'share a preview link', or wants to make a site/app they (or you) just built reachable on the web. Covers the zero-account instant path (Cloudflare) and permanent hosting (Vercel, incl. any Dockerfile). Also owns the publish GUARDRAILS: never publish unprompted, how to honestly take a site down, the runtime-dependency and data-persistence checks, and the mandatory pre-publish security review. Points at zed-marketplace for other providers and deeper provider-specific skills."
...
3. **Flag runtime-only dependencies that won't survive publishing.** API keys
   ...
   deployment, and anything reaching back into the Zed agent runtime or its
...
5. **Use secure cookies behind the preview proxy.** Sites served through the
   sandbox preview proxy sit behind Zed's domain — set `Secure`,
...
## Other providers & going deeper (`zed-marketplace`)
...
use the **`zed-marketplace`** skill to search the catalog and import a
battle-tested one:

```bash
zed marketplace search "netlify deploy" --json
zed marketplace search "cloudflare workers" --json
zed marketplace search "fly.io deploy docker" --json
```

Reach for `zed-marketplace` whenever the user names a provider this skill
