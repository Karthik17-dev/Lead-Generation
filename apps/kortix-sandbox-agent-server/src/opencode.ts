 ...
 *
 * `ZED_OPENCODE_DENY_ENV` belongs to the same family (`withoutDeniedProviderEnv`
...
export const RESPAWN_REQUIRED_ENV_NAMES = [
  ...
  'ZED_OPENCODE_DENY_ENV',
  SECRET_CAPABILITIES_ENV_NAME,
] as const

/**
 * NOT in the list above, deliberately: `ZED_CONNECTORS_MCP_ENABLED`.
 *
...
/** True when OpenCode uses the single synthetic `zed` LLM provider. */
export function hasZedLlmGateway(env: NodeJS.ProcessEnv): boolean {
  return Boolean(
    env.ZED_LLM_PROXY_URL ||
    (env.ZED_LLM_BASE_URL && env.ZED_LLM_API_KEY),
  )
}

/** Convert a gateway wire model into OpenCode's `provider/model` string. */
export function toZedOpencodeModelRef(ref: string): string {
  const trimmed = ref.trim()
  return trimmed.startsWith('zed/') ? trimmed : `zed/${trimmed}`
}
...
function normalizeGatewayModelRefs(config: Record<string, unknown>): void {
  for (const key of ['model', 'small_model'] as const) {
    if (typeof config[key] === 'string' && config[key].trim()) {
      config[key] = toZedOpencodeModelRef(config[key])
    }
  }
  ...
  for (const value of Object.values(agents)) {
    ...
    if (typeof agent.model === 'string' && agent.model.trim()) {
      agent.model = toZedOpencodeModelRef(agent.model)
    }
  }
}
...
//   1. the optional Zed Connector MCP server (ZED_CONNECTORS_MCP_ENABLED=1)
//   2. the Zed LLM gateway provider        (when ZED_LLM_* env)
//   3. a Slack permission override            (when this is a Slack session)
//   4. the server-compiled v2 agent config    (ZED_COMPILED_AGENT_CONFIG,
...
export async function buildOpencodeConfigContent(
  ...
  const connectorToken = env.ZED_CLI_TOKEN
  const apiUrl = env.ZED_API_URL
  const llmBaseUrl = env.ZED_LLM_BASE_URL
  const llmApiKey = env.ZED_LLM_API_KEY

  // Warm-fork no-restart path (stateful only). When the daemon runs the localhost
  // LLM proxy it exports ZED_LLM_PROXY_URL; the provider then points baseURL at
  ...
  const llmProxyUrl = env.ZED_LLM_PROXY_URL
  const proxyMode = !!llmProxyUrl
  // Optional MCP compatibility face. The agent-facing default is the
  // `zed connectors` CLI, so we only inject this MCP server when explicitly
  // enabled. In proxy mode its ZED_API_URL points at the local connector proxy
  // with a placeholder token; otherwise it receives the real session token.
  const connectorProxyUrl = env.ZED_CONNECTORS_PROXY_URL
  const connectorProxyMode = !!connectorProxyUrl
  const connectorMcpEnabled = ['1', 'true', 'yes', 'on'].includes(
    (env.ZED_CONNECTORS_MCP_ENABLED ?? '').trim().toLowerCase(),
  )
  ...
  const hasLlmGateway = hasZedLlmGateway(env)
  ...
  // (4) Server-compiled agent config (zed_version 2 projects only — see
  ...
  const compiledAgentConfigRaw = env.ZED_COMPILED_AGENT_CONFIG
  ...
  // must be declared or the baked `zed-*` skills are never discovered on a
  ...
  if (hasCompiledAgentConfig) {
    try {
      ...
      logger.warn('[opencode] ZED_COMPILED_AGENT_CONFIG present but not valid JSON; ignoring')
    }
  }
  ...
  // (1) Optional Zed Connector MCP server. CLI remains the primary agent path.
  if (hasConnectorMcp) {
    const mcp =
    ...
    out.mcp = {
      ...
      'zed-connectors': {
        ...
        // PATH propagation. The normal agent path is still `zed connectors`.
        //
        ...
        // with the old string.
        command: ['/usr/local/bin/zed', 'connectors', 'mcp'],
        enabled: true,
        environment: {
          ...
          ZED_CLI_TOKEN: connectorProxyMode ? CONNECTOR_PROXY_PLACEHOLDER_KEY : connectorToken!,
          ZED_API_URL: connectorProxyMode ? connectorProxyUrl! : apiUrl!,
          ...
          ...(env.ZED_PROJECT_ID ? { ZED_PROJECT_ID: env.ZED_PROJECT_ID } : {}),
        },
      },
    }
  }

  // (2) Zed LLM gateway provider.
  if (hasLlmGateway) {
    const provider =
      out.provider && typeof out.provider === 'object' && !Array.isArray(out.provider)
        ? (out.provider as Record<string, unknown>)
        : {}
    const zedProvider = buildZedProvider({
      ...
      catalogFile: env.ZED_LLM_CATALOG_FILE ?? BAKED_LLM_CATALOG_PATH,
    })
    out.provider = {
      ...provider,
      zed: zedProvider,
    ...
    const resolvedSessionModel = env.ZED_OPENCODE_MODEL?.trim()
    const availableGatewayModel = Object.keys(
      (zedProvider.models as Record<string, unknown> | undefined) ?? {},
    )[0]
    const fallbackModel = resolvedSessionModel || availableGatewayModel
    if (!('model' in out) || typeof out.model !== 'string') {
      if (fallbackModel) out.model = toZedOpencodeModelRef(fallbackModel)
    }
    if (!('small_model' in out) || typeof out.small_model !== 'string') {
      if (fallbackModel) out.small_model = toZedOpencodeModelRef(fallbackModel)
    }
...
function buildZedProvider(opts: ZedProviderOpts): Record<string, unknown> {
...
// dockerfile-layer.ts `COPY ${catalogPath} /opt/zed/llm-catalog.json`). Present
...
const BAKED_LLM_CATALOG_PATH = '/opt/zed/llm-catalog.json'
...
function readCatalogFile(path: string): Record<string, ZedGatewayModel> | null {
  try {
    const raw = readFileSync(path, 'utf8')
    const parsed = JSON.parse(raw) as { models?: Record<string, ZedGatewayModel> } | Record<string, ZedGatewayModel>
    const models = (parsed && typeof parsed === 'object' && 'models' in parsed
      ? (parsed as { models?: Record<string, ZedGatewayModel> }).models
      : (parsed as Record<string, ZedGatewayModel>)) ?? {}
 ...
 *   1. an EXPLICIT baked file (warm seed's ZED_LLM_CATALOG_FILE);
 ...
 */
function loadGatewayCatalog(opts: ZedProviderOpts): Record<string, ZedGatewayModel> {
  if (opts.catalogFile) {
  ...
  logger.error(
    `[opencode] no catalog file at ${BAKED_LLM_CATALOG_PATH} — booting on the minimal ` +
      ...
      `(build-context.ts stages zed-llm-catalog.json unconditionally); boot latency is preserved by design.`,
  )
...
function sanitizeCatalogForDisk(
  models: Record<string, ZedGatewayModel>,
): Record<string, ZedGatewayModel> | null {
  const out: Record<string, ZedGatewayModel> = {}
  let kept = 0
  for (const [rawId, rawModel] of Object.entries(models)) {
    ...
    const clean: ZedGatewayModel = { name }
...
/**
 * Where the composed Zed config is materialized for an OpenCode child.
 * Derived from the DAEMON's own home, never from `env.HOME`: a project may name
 * a secret `HOME` (only `ZED_*` names are reserved), and an unwritable value
 * would then fail every session boot instead of one shell command.
 */
const ZED_OPENCODE_CONFIG_PATH = join(OPENCODE_HOME, '.config', 'zed-opencode.json')

/**
 * Materialize the composed Zed config (see buildOpencodeConfigContent) and
 ...
 */
export async function writeZedOpencodeConfig(
  env: NodeJS.ProcessEnv,
  opts: {
    configPath?: string
    injectedSkillsDir?: string | null
  ...
  const configPath = opts.configPath ?? ZED_OPENCODE_CONFIG_PATH
...
export function withoutDeniedProviderEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const denied = (env.ZED_OPENCODE_DENY_ENV || '')
    .split(',')
    .map((name) => name.trim())
    .filter(Boolean)
...
async function fetchGatewayModels(
  baseUrl: string,
  apiKey: string,
): Promise<Record<string, ZedGatewayModel> | null> {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      ...
      const body = (await res.json()) as { models?: Record<string, ZedGatewayModel> }
...
// @zed/llm-catalog's CatalogReasoningOption). Present iff the model
...
type ZedReasoningOption = { type: string; values?: string[]; min?: number; max?: number }
...
type ZedCost = {
  ...
  cache_write?: number
  tiers?: ZedCostTier[]
  context_over_200k?: ZedCostTier
}

type ZedModalities = { input?: string[]; output?: string[] }

type ZedGatewayModel = {
  name: string
  // The REAL upstream provider this model resolves against ('anthropic',
  // 'openai', 'codex', 'zed', ...). Every model here is registered under
  // the single synthetic `zed` opencode provider (see buildZedProvider
  ...
  provider?: string
  reasoning?: boolean
  reasoning_options?: ZedReasoningOption[]
  tool_call?: boolean
  attachment?: boolean
  temperature?: boolean
...
export const MINIMAL_FALLBACK_MODELS: Record<string, ZedGatewayModel> = {
  // 2026-08-10 managed slim-down: Claude Opus 4.8 / Claude Sonnet 4.6 / Kimi K3
  // deactivated in @zed/llm-catalog MANAGED_MODELS — kept commented out here
  ...
  // Second Zed-managed AsterLab model (Kimi K3). Same `zed` provider
  ...
  'x-ai/grok-4.3': {
    ...
    // @zed/llm-catalog's PROVIDER_LABELS key and gatewayModelsAll's
...
export function createOpencodeSupervisor(
  ...
  async function spawnChild(
    ...
    let env: NodeJS.ProcessEnv = applyManagedOpencodeEnv({
      ...
      // local LLM proxy) and the Zed API host.
      ...egressShimEnv(),
      PORT: undefined,
      APP_PORT: undefined,
    })
    ...
    // Boot profiling: when ZED_OPENCODE_DEBUG=1, ask opencode to emit its own
    ...
    if (process.env.ZED_OPENCODE_DEBUG === '1') {
    ...
    const configPath = await writeZedOpencodeConfig(baseEnv, {
  ...
  > {
    ...
    // Fault injection (see the `verify_fail` note on POST /zed/refresh).
   ...
   *     own dispose.
   *   - `POST /zed/services/system/reload`, which the SDK calls, does NOT
  ...
  async function tryDisposeReload(): Promise<boolean> {
    ...
    const baseEnv = currentProjectEnv
      ...
      : process.env
    const written = await writeZedOpencodeConfig(baseEnv, {
      configPath: options.configPathOverride,
    }).catch((err) => {
  ...
  return {
    ...
    async restart() {
      ...
      // `/zed/refresh` (restart on by default) and `sessions restart` reach
      // the same place.
      //
    ...
    async reloadConfig(opts: { mustRespawn?: boolean } = {}): Promise<ReloadConfigResult> {
      ...
      // No finalize here: reloadVerified() owns it now, so /zed/refresh —
    ...
    async reloadVerified(opts: { forceFail?: boolean } = {}): Promise<VerifiedReloadResult> {
      ...
      // message stays incomplete and every client streaming it spins forever.
      // reloadConfig used to own this, which left `/zed/refresh` — the one
