...
export const AGENT_ENV_DIR = '/dev/shm/zed'
...
// (NOT all ZED_*): only the session's own identity/context creds, never the
...
function bootSecretNames(bootEnv: NodeJS.ProcessEnv): string[] {
  return (bootEnv.ZED_PROJECT_SECRET_NAMES ?? '')
    .split(',')
    .map((name) => name.trim())
    .filter(Boolean)
}
...
function renderShellEnv(
  ...
  const lines = [
    '# Generated by zed-sandbox-agent-server. Sourced by every sandbox shell',
  ...
  // and NOT ZED_-prefixed: a proxy URL and some file paths. They belong here
  ...
  // LAST so they win over any stale inherited value, and bypass the ZED_*
