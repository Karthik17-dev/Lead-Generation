...
const Schema = z.object({
  ...
  ZED_WORKLOAD: z.string().default(''),
  // The enabled monitors, resolved from zed.yaml BY apps/api and injected as
  ...
  ZED_MONITOR_BOX_EPOCH: z.string().default(''),
})
...
export type Config = {
  ...
  workload: string
  /** Raw `ZED_MONITORS` JSON; parsed by monitor-runner.parseMonitorSpecs. */
  monitorsJson: string
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  const parsed = Schema.parse({
    ...
    ZED_WORKLOAD: env.ZED_WORKLOAD,
    ZED_MONITORS: env.ZED_MONITORS,
    ZED_MONITOR_BOX_EPOCH: env.ZED_MONITOR_BOX_EPOCH,
  })
  ...
  return {
    ...
    workload: parsed.ZED_WORKLOAD.trim(),
    monitorsJson: parsed.ZED_MONITORS,
    monitorBoxEpoch: parsed.ZED_MONITOR_BOX_EPOCH.trim(),
  }
}
