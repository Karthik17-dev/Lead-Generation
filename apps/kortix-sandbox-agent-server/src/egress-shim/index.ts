...
function installSystemTrust(certPem: string): boolean {
  const target = '/usr/local/share/ca-certificates/zed-egress.crt'
  try {
 ...
 *
 * `NO_PROXY` carries the Zed API host so the in-sandbox CLI and the daemon's
...
export function egressShimAgentEnv(port: number, apiUrl: string): Record<string, string> {
  ...
  try {
    ...
    // A malformed ZED_API_URL is not worth failing the shim over; the
...
export function egressShimPort(env: NodeJS.ProcessEnv = process.env): number {
  return Number(env.ZED_EGRESS_SHIM_PORT) || DEFAULT_SHIM_PORT
}
...
export async function startEgressShim(
  ...
  const ca = createEphemeralCa(env.ZED_PROJECT_ID?.slice(0, 8) ?? 'sandbox')
