 ...
 * sends reaches Zed carrying no credential, and the shim itself never holds
...
describe('the shim cannot hold a credential', () => {
  ...
  test('the request it sends Zed carries no injected header of its own', async () => {
...
describe('broker failures reach the agent honestly', () => {
  test("Zed's own refusal is surfaced verbatim, not as a proxy error", async () => {
...
describe('the shim fails closed', () => {
  const base = {
    ZED_SECRET_CAPABILITIES: JSON.stringify({
      version: 1,
      capabilities: [{ identifier: 'DEMO', delivery: 'network', hosts: ['api.example.com'] }],
    }),
    ZED_API_URL: 'https://api.zed.test/v1',
    ZED_PROJECT_ID: 'proj-1',
    ZED_CLI_TOKEN: 'zed_pat_test',
  }

  test('a complete environment yields a config', () => {
    expect(resolveShimConfig(base)).toMatchObject({ projectId: 'proj-1', token: 'zed_pat_test' })
    expect(shimUnavailableReason(base)).toBeNull()
  })

  test('the SANDBOX token does not stand in for the session token', () => {
    // ZED_TOKEN is the daemon's identity and project-scoped routes reject
    // it. Accepting it here would start a shim whose every relay 403s.
    const env = { ...base, ZED_CLI_TOKEN: undefined, ZED_TOKEN: 'zed_sb_daemon' }
    expect(resolveShimConfig(env)).toBeNull()
    expect(shimUnavailableReason(env)).toContain('ZED_CLI_TOKEN')
  })

  test.each(['ZED_API_URL', 'ZED_PROJECT_ID', 'ZED_CLI_TOKEN'])(
  ...
  test('no boundary secrets is silence, not an error', () => {
    ...
    const env = { ...base, ZED_SECRET_CAPABILITIES: undefined }
    expect(resolveShimConfig(env)).toBeNull()
