 ...
 *
 *   agent ──HTTPS──▶ this shim ──HTTPS──▶ Zed broker route ──▶ upstream
...
export interface EgressShimOptions {
  ...
  /** Zed API base, e.g. `https://dev-api.zed.com/v1`. */
 ...
 * an ordinary request into `400 request header is managed by Zed: cookie` —
...
/**
 * Hand the guest's request to Zed, which holds the credential.
 *
 * Deliberately the SAME broker route that `zed secrets call` and the
...
async function relayToBroker(
  ...
  if (!response.ok) {
    // Surface Zed's own refusal verbatim: a denied grant or an out-of-policy
...
export async function createEgressShim(options: EgressShimOptions): Promise<http.Server> {
  ...
  const onTerminatedRequest: http.RequestListener = (req, res) => {
    ...
    if (!binding) {
      res.writeHead(500).end('zed egress shim: no rule bound to connection')
      return
    }
    const chunks: Buffer[] = []
    ...
    req.on('end', () => {
      void relayToBroker(
        ...
        res.end('zed egress shim: broker relay failed')
      })
    })
  }
  ...
  const server = http.createServer((_req, res) => {
    ...
    // request would have Zed add a credential to a connection that then
    // continues in the clear, which is strictly worse than not having the
    // feature.
    res.writeHead(405).end('zed egress shim: HTTPS CONNECT only')
  })
  ...
  server.on('connect', (req: http.IncomingMessage, socket: Duplex, head: Buffer) => {
    ...
    // terminates it, parses the request, and relays it to Zed.
