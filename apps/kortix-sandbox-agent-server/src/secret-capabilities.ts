...
export const SECRET_CAPABILITIES_INSTRUCTION_PATH = '/tmp/zed/secret-capabilities.md'
...
type CatalogEntry = {
  identifier: string
  delivery: 'sandbox' | 'https_broker' | 'zed_service' | 'network'
...
function parseCatalog(raw: string | undefined): CatalogEntry[] {
  ...
  try {
    ...
    return value.capabilities.flatMap((entry): CatalogEntry[] => {
      ...
      if (!['sandbox', 'https_broker', 'zed_service', 'network'].includes(String(item.delivery))) {
        return []
      }
      return [
        {
...
export function renderSecretCapabilitiesInstruction(raw: string | undefined): string {
  ...
  const lines = [
    ...
    '',
    'Zed grants this session only the secret capabilities listed below.',
    ...
    'Use HTTPS broker secrets with `zed secrets call IDENTIFIER URL [options]`.',
    'Use Zed service secrets through the named service. They are never available as plaintext.',
    '',
  ]
  if (entries.length === 0) {
    ...
    for (const entry of entries) {
      if (entry.delivery === 'sandbox') {
        ...
        lines.push(
          `- \`${entry.identifier}\`: HTTPS broker. Use \`zed secrets call ${entry.identifier} <https-url> [options]\`.`,
        )
      } else if (entry.delivery === 'network') {
        const hosts = (entry.hosts ?? []).join(', ')
        lines.push(
          `- \`${entry.identifier}\`: network boundary. Zed adds the \`${entry.header ?? 'authorization'}\`` +
        ...
        lines.push(
          `- \`${entry.identifier}\`: Zed service \`${entry.consumer ?? 'managed'}\`; no sandbox plaintext.`,
        )
      }
    }
  }
