 ...
 *
 * WHY: `GET /zed/health` already exposes `bootState.timeline` (see
 ...
 * HOST side which IS persisted (zed.provider_events.marks, written by
 ...
 * (a seed has no session yet — `ZED_SESSION_ID` is unset there and the
...
async function doRelay(timeline: BootMark[]): Promise<void> {
  ...
  // so the old "prefer ZED_CLI_TOKEN, this route accepts either" order sent
  // the one token that cannot work on a normal session. Canonical name first,
  // legacy ZED_TOKEN alias second for daemons baked before the rename.
  //
  ...
  const token = (process.env.ZED_SANDBOX_TOKEN || process.env.ZED_TOKEN || '').trim()
