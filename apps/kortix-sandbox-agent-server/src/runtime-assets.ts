 ...
 *
 * THE BUG THIS FIXES. `/usr/local/bin/zed` and `/opt/zed/managed-skills`
 ...
 * proxy is up and readiness has been marked, `POST /zed/refresh` schedules it
 * without awaiting, and warm-fork adoption fires it after the session runtime is
 * already live. Nothing waits on this function.
 */

/** The binary every in-sandbox agent invokes as `zed`. */
const DEFAULT_CLI_PATH = '/usr/local/bin/zed'
/** Image-baked managed-skill overlay root; created here when the image had none. */
const DEFAULT_MANAGED_SKILLS_DIR = '/opt/zed/managed-skills'
/** Digest bookkeeping, so a converged box never re-hashes a 100 MB binary. */
const DEFAULT_STATE_PATH = '/opt/zed/runtime-assets-state.json'
...
export function isSafeOverlayPath(path: string): boolean {
  ...
  if (!path.startsWith('zed-')) return false
  return path
    .split('/')
...
async function replaceCli(
  ...
  const tmpPath = join(
    dirname(cliPath),
    `.zed.download.${process.pid}.${Math.random().toString(36).slice(2, 10)}`,
  )
  try {
    ...
    // Atomic on Linux: a `zed` already running keeps its open inode, and no
...
export async function reconcileRuntimeAssets(
  ...
  const rawApiUrl = (options.apiUrl ?? process.env.ZED_API_URL ?? '').trim().replace(/\/+$/, '')
  if (!token || !rawApiUrl) {
      ...
      if (local && local.sha === manifest.cli_sha256) {
        ...
        if (!res.ok) {
          ...
          if (cli === 'updated') {
            ...
            logger.info('[runtime-assets] zed CLI updated from the API', {
 ...
 * `POST /zed/refresh` can land within milliseconds of each other; without
 ...
 */
export function ensureLatestZedAssets(configDir?: string): void {
  if (inFlight) return
