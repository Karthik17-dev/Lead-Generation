 ...
 * /etc/pt-env carries the session's ZED_BRANCH_NAME from the instant the
...
function wantedSessionBranch(): string {
  try {
    const m = readFileSync('/etc/pt-env', 'utf8').match(/^ZED_BRANCH_NAME=(\S+)/m)
    if (m?.[1]) return m[1]
  } catch { /* no env file (local dev) */ }
  return (process.env.ZED_BRANCH_NAME ?? '').trim()
}
...
function sessionWantsRepo(cfgAutoClone: boolean): boolean {
  ...
  try {
    return /^ZED_PROJECT_AUTO_CLONE=1/m.test(readFileSync('/etc/pt-env', 'utf8'))
  } catch {
    return false
  }
}
...
/**
 * The single Zed-owned route on the daemon.
 *
 * Shape:
 *   {
 *     daemon: 'ok',
...
export function createHealthRouter(
  ...
  router.get('/', async (c) => {
    ...
    return c.json({
      ...
      // it (a box whose env says ZED_WORKLOAD=monitor but whose daemon
      // booted the session path can never run monitors).
      workload: process.env.ZED_WORKLOAD === 'monitor' ? 'monitor' : 'session',
      ...
      agent_config_etag: process.env.ZED_COMPILED_AGENT_CONFIG_ETAG || null,
