...
export function createRefreshRouter(cfg: Config, opencode: Opencode): Hono {
  ...
  router.post('/', async (c) => {
    ...
    if (!serviceAuthenticated) {
      const auth = verifyZedUserContext(
        c.req.header(ZED_USER_CONTEXT_HEADER),
        cfg.sandboxToken,
      )
      if (!auth.ok) {
    ...
    // exactly one daemon path, `/zed/env`, and this is not it — so any
    ...
    // ZED_SERVICE_CALL_HEADER: it strips it from every forwarded request, so
    ...
    if (syncBase && !(serviceAuthenticated && c.req.header(ZED_SERVICE_CALL_HEADER) === '1')) {
    ...
    refreshInFlight = (async () => {
      try {
        ...
        // Converge the sandbox's `zed` CLI + managed-skill overlay on this
