...
const TEST_TOKEN = 'files-test-zed-token'
...
/** Mint a valid X-Zed-User-Context header signed with TEST_TOKEN. */
...
function authHeaders(extra: Record<string, string> = {}): Record<string, string> {
  return { [ZED_USER_CONTEXT_HEADER]: signContext(), ...extra }
}
...
describe('daemon file write routes', () => {
  ...
  beforeAll(async () => {
    WORKSPACE = await fs.mkdtemp(path.join(os.tmpdir(), 'zed-files-test-'))
...
describe('daemon file read + list + status + find routes', () => {
  ...
  beforeAll(async () => {
    WS = await fs.mkdtemp(path.join(os.tmpdir(), 'zed-read-test-'))
    git('init', '-q')
    git('config', 'user.email', 'test@zed.ai')
    git('config', 'user.name', 'Zed Test')
