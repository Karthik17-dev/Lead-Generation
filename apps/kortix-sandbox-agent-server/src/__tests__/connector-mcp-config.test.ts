...
const ENV = { ZED_CLI_TOKEN: 'tok-123', ZED_API_URL: 'https://api.zed.test/v1' }
...
const CATALOG_FILE = join(mkdtempSync(join(tmpdir(), 'zed-mcp-catalog-')), 'catalog.json')
...
describe('buildOpencodeConfigContent — injected managed skills', () => {
  test('declares the injected skills dir via skills.paths', async () => {
    const dir = join(tmpdir(), `zed-skills-test-${process.pid}`)
  ...
  test('merges with skills paths already declared by the compiled config', async () => {
    ...
    const content = await buildOpencodeConfigContent(
      { ZED_COMPILED_AGENT_CONFIG: JSON.stringify({ skills: { paths: ['/repo/skills'] } }) },
  ...
  test('loads the generated secret capability guide without replacing project instructions', async () => {
    const file = join(mkdtempSync(join(tmpdir(), 'zed-secret-guide-')), 'capabilities.md')
...
describe('buildOpencodeConfigContent — optional connector MCP server', () => {
  ...
  test('registers the connector MCP server only when explicitly enabled', async () => {
    const raw = await buildOpencodeConfigContent({ ...ENV, ZED_CONNECTORS_MCP_ENABLED: '1' })
    expect(raw).toBeDefined()
    const config = JSON.parse(raw!)
    const server = config.mcp['zed-connectors']
    expect(server).toMatchObject({
      type: 'local',
      enabled: true,
      environment: {
        ZED_CLI_TOKEN: 'tok-123',
        ZED_API_URL: 'https://api.zed.test/v1',
        PATH: '/usr/local/bin:/usr/bin:/bin',
      },
    })
    expect(server.command).toEqual(['/usr/local/bin/zed', 'connectors', 'mcp'])
  })

  test('returns undefined when no contributor applies', async () => {
    expect(await buildOpencodeConfigContent({})).toBeUndefined()
    expect(await buildOpencodeConfigContent({ ZED_CLI_TOKEN: 'tok-123' })).toBeUndefined()
    expect(await buildOpencodeConfigContent({ ZED_API_URL: 'https://api.zed.test/v1' })).toBeUndefined()
    expect(await buildOpencodeConfigContent({ ...ENV, ZED_CONNECTORS_MCP_ENABLED: '0' })).toBeUndefined()
  })
  ...
  test('merges onto pre-existing inline config without clobbering it', async () => {
    ...
    expect(config.mcp['zed-connectors']).toBeDefined()
  })
...
describe('buildOpencodeConfigContent — Zed LLM gateway provider', () => {
  const GATEWAY_ENV = {
    ZED_LLM_BASE_URL: 'https://api.zed.test/v1/llm',
    ZED_LLM_API_KEY: 'kyolo_abc123',
    ZED_LLM_CATALOG_FILE: CATALOG_FILE,
  }

  test('registers the zed provider when gateway env present', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const config = JSON.parse((await buildOpencodeConfigContent(GATEWAY_ENV))!)
    expect(config.provider.zed).toMatchObject({
      npm: '@ai-sdk/openai-compatible',
      name: 'Zed',
      options: {
        baseURL: 'https://api.zed.test/v1/llm',
        apiKey: 'kyolo_abc123',
      },
    })
    expect(Object.keys(config.provider.zed.models).length).toBeGreaterThan(0)
  })

  test('populates the provider models from the baked catalog file', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const config = JSON.parse((await buildOpencodeConfigContent(GATEWAY_ENV))!)
    const models = config.provider.zed.models
    ...
    // `provider` is picker metadata from the Zed catalog, not part of an
  ...
  test('falls back to a minimal catalog immediately when no catalog file exists', async () => {
    ...
    const config = JSON.parse(
      (await buildOpencodeConfigContent({
        ...GATEWAY_ENV,
        ZED_LLM_CATALOG_FILE: join(tmpdir(), 'zed-absent-catalog.json'),
      }))!,
    )
    const models = config.provider.zed.models
  ...
  test('uses the resolved session model as the OpenCode default', async () => {
    ...
    const config = JSON.parse((await buildOpencodeConfigContent({
      ...GATEWAY_ENV,
      ZED_OPENCODE_MODEL: 'codex/gpt-5.6-sol',
    }))!)
    expect(config.model).toBe('zed/codex/gpt-5.6-sol')
    expect(config.small_model).toBe('zed/codex/gpt-5.6-sol')
  })

  test('uses an available gateway model for legacy sessions without a resolved model', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const config = JSON.parse((await buildOpencodeConfigContent(GATEWAY_ENV))!)
    expect(config.model).toBe('zed/anthropic/claude-opus-4.8')
    expect(config.small_model).toBe('zed/anthropic/claude-opus-4.8')
  })

  test('routes a user-set default model through the Zed provider', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    ...
    expect(config.model).toBe('zed/anthropic/claude-sonnet-4.6')
  })
  ...
  test('does not include connector MCP alongside the provider unless explicitly enabled', async () => {
    ...
    expect(config.provider.zed).toBeDefined()
  ...
  test('can include the optional connector MCP alongside the provider', async () => {
    ...
    const config = JSON.parse((await buildOpencodeConfigContent({
      ...ENV,
      ...GATEWAY_ENV,
      ZED_CONNECTORS_MCP_ENABLED: 'true',
    }))!)
    expect(config.provider.zed).toBeDefined()
    expect(config.mcp['zed-connectors']).toBeDefined()
  })
...
describe('buildOpencodeConfigContent — gateway provider allowlist', () => {
  ...
  test('allows only zed when the gateway is active', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const config = JSON.parse((await buildOpencodeConfigContent(GATEWAY_ENV))!)
    expect(config.enabled_providers).toEqual(['zed'])
  })
...
describe('buildOpencodeConfigContent — Slack sessions deny the question tool', () => {
  ...
  test('does NOT touch permissions for a non-Slack (web) session — tool stays native', async () => {
    const config = JSON.parse((await buildOpencodeConfigContent({ ...ENV, ZED_CONNECTORS_MCP_ENABLED: '1' }))!)
...
describe('buildOpencodeConfigContent — server-compiled v2 agent config (ZED_COMPILED_AGENT_CONFIG)', () => {
  const GATEWAY_ENV = {
    ZED_LLM_BASE_URL: 'https://api.zed.test/v1/llm',
    ZED_LLM_API_KEY: 'kyolo_abc123',
    ZED_LLM_CATALOG_FILE: CATALOG_FILE,
  ...
  test('builds a config from the compiled agent config alone (no connector/gateway/Slack)', async () => {
    const config = JSON.parse((await buildOpencodeConfigContent({ ZED_COMPILED_AGENT_CONFIG: COMPILED }))!)
  ...
  test('a v1 project (no ZED_COMPILED_AGENT_CONFIG) is unaffected: still undefined with no other contributor', async () => {
    expect(await buildOpencodeConfigContent({})).toBeUndefined()
  })
  ...
  test('the gateway overlay normalizes compiled top-level and agent models', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const config = JSON.parse(
      (await buildOpencodeConfigContent({ ...GATEWAY_ENV, ZED_COMPILED_AGENT_CONFIG: COMPILED }))!,
    )
    expect(config.model).toBe('zed/anthropic/claude-sonnet-5')
    expect(config.small_model).toMatch(/^zed\//)
    expect(config.agent.support.model).toBe('zed/anthropic/claude-sonnet-5')
  })

  test('the gateway overlay keeps the complete Codex wire model as the Zed model id', async () => {
    stageGatewayCatalog(GATEWAY_CATALOG)
    const compiled = JSON.stringify({
    ...
    const config = JSON.parse(raw)
    expect(config.model).toBe('zed/codex/gpt-5.6-sol')
    expect(config.agent.mike.model).toBe('zed/codex/gpt-5.6-sol')
  })
  ...
  test('malformed ZED_COMPILED_AGENT_CONFIG is ignored, not fatal', async () => {
    const config = await buildOpencodeConfigContent({
      ...ENV,
      ZED_CONNECTORS_MCP_ENABLED: '1',
      ZED_COMPILED_AGENT_CONFIG: 'not json{',
    })
    expect(config).toBeDefined()
    expect(JSON.parse(config!).agent).toBeUndefined()
    expect(JSON.parse(config!).mcp['zed-connectors']).toBeDefined()
  })
})
