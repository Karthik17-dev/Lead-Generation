...
describe('isTransientTurnError', () => {
  ...
  test('user aborts and auth/credit failures are never resumed', () => {
    ...
    expect(isTransientTurnError({ name: 'ProviderAuthError', providerID: 'zed' })).toBe(false);
...
describe('createTurnAutoResumer', () => {
  ...
  test('kill switch ZED_TURN_AUTO_RESUME=0 disables resumes', async () => {
    const h = makeHarness();
    process.env.ZED_TURN_AUTO_RESUME = '0';
    try {
      expect(await h.resumer.maybeResume('ses_root', IDLE_TIMEOUT)).toBe(false);
      expect(h.prompts.length).toBe(0);
    } finally {
      delete process.env.ZED_TURN_AUTO_RESUME;
    }
  });
