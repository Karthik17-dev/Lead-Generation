...
describe('MINIMAL_FALLBACK_MODELS capability metadata', () => {
  ...
  // (managed models -> 'zed', BYOK entries -> their real provider id), the
  ...
  test('every fallback model carries an explicit `provider` field matching its wire id', () => {
    expect(MINIMAL_FALLBACK_MODELS['glm-5.2']?.provider).toBe('zed')
  ...
  // @zed/llm-catalog's PROVIDER_LABELS and the served /v1/models catalog
  ...
  // and fall back to the raw (always "Zed") providerName for this one
  // fallback-only entry — reintroducing the "every provider = Zed" bug
