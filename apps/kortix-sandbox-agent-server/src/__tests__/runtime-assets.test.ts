...
const SKILL_FILES = [
  { path: 'zed-system/SKILL.md', content: '---\ndescription: how zed works\n---\nbody v2\n' },
  { path: 'zed-cli/SKILL.md', content: 'cli skill v2\n' },
]
...
describe('overlay hashing and path safety', () => {
  ...
  test('rejects traversal, absolute, and non-zed overlay paths', () => {
    expect(isSafeOverlayPath('zed-system/SKILL.md')).toBe(true)
    expect(isSafeOverlayPath('zed-system/references/a.md')).toBe(true)
    expect(isSafeOverlayPath('../etc/passwd')).toBe(false)
    expect(isSafeOverlayPath('/etc/passwd')).toBe(false)
    expect(isSafeOverlayPath('zed-system/../../evil')).toBe(false)
...
describe('reconcileRuntimeAssets', () => {
  ...
  test('digest mismatch → binary replaced, mode 0755, overlay written', async () => {
    const ws = await workspace()
    await writeFile(ws.cliPath.replace(/\/zed$/, '/.keep'), '').catch(() => {})
    ...
    expect(await readFile(join(ws.skillsDir, 'zed-system/SKILL.md'), 'utf8')).toContain('body v2')
    expect(await readFile(join(ws.skillsDir, 'zed-cli/SKILL.md'), 'utf8')).toBe('cli skill v2\n')
  ...
  test('overlay payload digest mismatch → keeps the existing overlay', async () => {
    const ws = await workspace()
    await Bun.write(join(ws.skillsDir, 'zed-system/SKILL.md'), 'body v1\n')
    await Bun.write(ws.cliPath, 'NEW-CLI-BYTES')
    // Manifest advertises the real hash; the payload delivers other files.
    const stub = stubFetch({ skillFiles: [{ path: 'zed-system/SKILL.md', content: 'tampered' }] })

    const result = await run(ws, stub)

    expect(result.skills).toBe('failed')
    expect(await readFile(join(ws.skillsDir, 'zed-system/SKILL.md'), 'utf8')).toBe('body v1\n')
  })
  ...
  test('unsafe overlay paths are dropped, safe siblings still land', async () => {
    ...
    await Bun.write(ws.cliPath, 'NEW-CLI-BYTES')
    const files = [
      { path: 'zed-system/SKILL.md', content: 'ok\n' },
    ...
    expect(result.skills).toBe('updated')
    expect(await readFile(join(ws.skillsDir, 'zed-system/SKILL.md'), 'utf8')).toBe('ok\n')
  ...
  test('missing overlay dir is created even when the hash already matches state', async () => {
    ...
    // never baked /opt/zed/managed-skills at all.
