...
const TEST_TOKEN = 'curl-test-zed-token'
...
describe('project env sync curl e2e', () => {
  it('updates running daemon env state through curl without restarting the sandbox', async () => {
    let restarts = 0
    const temp = mkdtempSync(join(tmpdir(), 'zed-env-curl-'))
    ...
    try {
      const first = await curlJson(`http://127.0.0.1:${server.port}/zed/env`, JSON.stringify({
        revision: 'rev-curl-1',
      ...
      const replay = await curlJson(`http://127.0.0.1:${server.port}/zed/env`, JSON.stringify({
        revision: 'rev-curl-1',
        env: { API_KEY: 'new', EXTRA_TOKEN: 'fresh' },
        names: ['API_KEY', 'EXTRA_TOKEN'],
      }))
      ...
      const modelRefresh = await curlJson(`http://127.0.0.1:${server.port}/zed/env`, JSON.stringify({
        revision: 'rev-curl-2',
