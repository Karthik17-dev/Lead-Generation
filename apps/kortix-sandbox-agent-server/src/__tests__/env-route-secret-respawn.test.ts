 ...
 * delta delivered to the daemon's `/zed/env` must force a full opencode
...
const TEST_TOKEN = 'respawn-test-zed-token-32-chars'
const TEST_ENV_DIR = mkdtempSync(join(tmpdir(), 'zed-env-respawn-'))
...
describe('env route — project-secret delta forces respawn, not dispose', () => {
  it('a secret capability catalog change forces a respawn', async () => {
    ...
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRETS_REVISION: 'rev-1',
      ZED_PROJECT_SECRET_NAMES: '',
      ZED_SECRET_CAPABILITIES: '{"version":1,"capabilities":[]}',
  ...
  it('a project-secret CHANGE (value moved) forces a respawn', async () => {
    ...
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRETS_REVISION: 'rev-1',
      ZED_PROJECT_SECRET_NAMES: 'API_KEY',
      API_KEY: 'v1',
    } as NodeJS.ProcessEnv)
    const app = buildTestApp(opencode, store)
  ...
  it('a project-secret REVOCATION (secret dropped) forces a respawn', async () => {
    ...
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRETS_REVISION: 'rev-1',
      ZED_PROJECT_SECRET_NAMES: 'API_KEY,STRIPE_KEY',
      API_KEY: 'v1',
  ...
  it('a pure MODEL change (no project-secret delta) keeps the dispose fast path', async () => {
    ...
    const { status, json } = await postEnv(app, {
      ...
      names: ['API_KEY'],
      refreshModels: true,
      opencodeEnv: { ZED_OPENCODE_MODEL: 'zed/claude-sonnet-4' },
    })
  ...
  it('a gateway deny-list change forces a respawn even with no project-secret delta', async () => {
    // ZED_OPENCODE_DENY_ENV is a RESPAWN_REQUIRED name: a dispose cannot
    ...
    const { status } = await postEnv(app, {
      ...
      names: ['API_KEY'],
      refreshModels: true,
      opencodeEnv: { ZED_OPENCODE_DENY_ENV: 'ANTHROPIC_API_KEY,OPENAI_API_KEY' },
    })
  ...
  it('enabling the connectors MCP face mid-session takes the dispose path', async () => {
    ...
    // `ZED_CONNECTORS_MCP_ENABLED` shapes `out.mcp` inside the config file,
  ...
  it('re-pushing the SAME connectors-MCP value does not reload at all', async () => {
    ...
    const app = buildTestApp(opencode, store)
    process.env.ZED_CONNECTORS_MCP_ENABLED = '1'
    try {
      const { status } = await postEnv(app, {
        revision: 'rev-1',
        env: { API_KEY: 'v1' },
