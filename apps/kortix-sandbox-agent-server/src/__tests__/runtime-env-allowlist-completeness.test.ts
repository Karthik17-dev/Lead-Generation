 ...
 * (routes/env.ts).
 *
 * That Set is the ONLY gate deciding which names a `/zed/env` push may
 ...
 *
 * This test enumerates every `env.ZED_*` (or `env[NAME_CONST]`) READ in
 ...
 *   (b) in `BOOT_ONLY_ZED_ENV_NAMES` below, a short, individually justified
 *       list of names that are session-boot constants and are deliberately
 *       never pushed through `/zed/env`.
 *
 * Adding a NEW `env.ZED_FOO` read to opencode.ts's config path with
...
/**
 * Every `ZED_*` name read off an `env`-shaped object (`env.ZED_FOO` or
 * `env['ZED_FOO']`) anywhere in a source string. Deliberately broad — it is
 * meant to over-collect (e.g. `process.env.ZED_OPENCODE_DEBUG` matches
 ...
 * `BOOT_ONLY_ZED_ENV_NAMES` below, not by narrowing the pattern.
 */
function readZedEnvNames(source: string): Set<string> {
  const names = new Set<string>()
  for (const m of source.matchAll(/\benv(?:\.|\[['"])(ZED_[A-Z0-9_]+)/g)) {
 ...
 * pushes through `/zed/env` — each has its own reason it is not, and must
 ...
 */
const BOOT_ONLY_ZED_ENV_NAMES = new Set([
...
describe('OPENCODE_RUNTIME_ENV_NAMES — allowlist completeness', () => {
  test('every ZED_* name opencode.ts reads is accounted for', () => {
    const allowlist = runtimeEnvAllowlist()
    const consumed = readZedEnvNames(OPENCODE_SRC)
    // `ZED_SECRET_CAPABILITIES` is read via the shared
    // `SECRET_CAPABILITIES_ENV_NAME` constant in secret-capabilities.ts
    // (`env[SECRET_CAPABILITIES_ENV_NAME]`), not as a literal `env.ZED_...`
    ...
    expect(SECRET_CAPABILITIES_SRC).toContain("SECRET_CAPABILITIES_ENV_NAME = 'ZED_SECRET_CAPABILITIES'")
    ...
    consumed.add('ZED_SECRET_CAPABILITIES')

    const unaccounted = [...consumed].filter(
      (name) => !allowlist.has(name) && !BOOT_ONLY_ZED_ENV_NAMES.has(name),
    )

    // A name landing here means opencode.ts (or its secret-capabilities
    // helper) now reads a ZED_* env var that `/zed/env` has no way to
    // update on a running box. Resolve it by adding the name to
    // OPENCODE_RUNTIME_ENV_NAMES in routes/env.ts (it should be
    // live-updatable) or to BOOT_ONLY_ZED_ENV_NAMES above with a one-line
  ...
  test('the two lists do not overlap — a name is EITHER live-updatable or boot-only, never both', () => {
    const allowlist = runtimeEnvAllowlist()
    const overlap = [...BOOT_ONLY_ZED_ENV_NAMES].filter((name) => allowlist.has(name))
  ...
  test('the boot-only list is not stale — every name on it is still actually read', () => {
    ...
    const consumed = readZedEnvNames(OPENCODE_SRC)
    const stale = [...BOOT_ONLY_ZED_ENV_NAMES].filter((name) => !consumed.has(name))
  ...
  test('pins the current fully-enumerated set, so a change here is a deliberate, reviewed diff', () => {
    ...
    expect([...consumed].sort()).toEqual([
      'ZED_API_URL',
      'ZED_CLI_TOKEN',
      'ZED_COMPILED_AGENT_CONFIG',
      'ZED_CONNECTORS_MCP_ENABLED',
      'ZED_CONNECTORS_PROXY_URL',
      'ZED_LLM_API_KEY',
      'ZED_LLM_BASE_URL',
      'ZED_LLM_CATALOG_FILE',
