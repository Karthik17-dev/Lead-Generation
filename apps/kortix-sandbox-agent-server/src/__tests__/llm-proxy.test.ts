...
describe('buildOpencodeConfigContent — proxy mode vs direct mode', () => {
  ...
  writeFileSync(
    catalog,
    JSON.stringify({ models: { 'zed/test-model': { id: 'zed/test-model', name: 'Test' } } }),
  )
  ...
  test('PROXY mode: session-independent provider by default, no connector MCP unless enabled', async () => {
    const json = await buildOpencodeConfigContent({
      ZED_LLM_PROXY_URL: 'http://127.0.0.1:4319',
      ZED_CONNECTORS_PROXY_URL: 'http://127.0.0.1:4320',
      ZED_API_URL: 'https://api.zed.test/v1',
      ZED_LLM_BASE_URL: 'https://gateway.zed.test/v1/llm',
      ZED_LLM_API_KEY: 'real-session-llm-key',
      ZED_CLI_TOKEN: 'real-session-exec-token',
      ZED_LLM_CATALOG_FILE: catalog,
    } as NodeJS.ProcessEnv)
    expect(json).toBeDefined()
    const cfg = JSON.parse(json!)

    // gateway provider points at the proxy with a placeholder — NO real key baked
    expect(cfg.provider.zed.options.baseURL).toBe('http://127.0.0.1:4319')
    expect(cfg.provider.zed.options.apiKey).toBe(LLM_PROXY_PLACEHOLDER_KEY)
    expect(cfg.provider.zed.options.apiKey).not.toBe('real-session-llm-key')
    ...
    expect(Object.keys(cfg.provider.zed.models)).toContain('zed/test-model')
  })
  ...
  test('PROXY mode can opt into session-independent connector MCP compatibility', async () => {
    ...
    expect(cfg.mcp['zed-connectors'].command).toEqual(['/usr/local/bin/zed', 'connectors', 'mcp'])
    expect(cfg.mcp['zed-connectors'].environment.ZED_API_URL).toBe('http://127.0.0.1:4320')
    expect(cfg.mcp['zed-connectors'].environment.ZED_CLI_TOKEN).toBe(CONNECTOR_PROXY_PLACEHOLDER_KEY)
    expect(cfg.mcp['zed-connectors'].environment.ZED_CLI_TOKEN).not.toBe('real-session-exec-token')
  })
  ...
  test('DIRECT mode (cold/Daytona): real key + token baked, unchanged', async () => {
    ...
    expect(json).toBeDefined()
    const cfg = JSON.parse(json!)
    expect(cfg.provider.zed.options.baseURL).toBe('https://gateway.zed.test/v1/llm')
    expect(cfg.provider.zed.options.apiKey).toBe('real-session-llm-key')
    expect(cfg.mcp).toBeUndefined()
  })
})
