...
function authHeaders(): Record<string, string> {
  return { [ZED_USER_CONTEXT_HEADER]: signCtx(), 'Content-Type': 'application/json' }
}
...
async function createPty(port: number, body: Record<string, unknown>): Promise<PtyMeta> {
  const res = await fetch(`http://127.0.0.1:${port}/zed/pty`, {
    method: 'POST',
    headers: authHeaders(),
...
describe('zed-native pty', () => {
  ...
  it('creates, lists, resizes, and deletes a pty over HTTP', async () => {
    ...
    try {
      ...
      const listRes = await fetch(`http://127.0.0.1:${proxy.port}/zed/pty`, { headers: authHeaders() })
      const list = (await listRes.json()) as PtyMeta[]
      expect(list.some((p) => p.id === created.id)).toBe(true)

      const patchRes = await fetch(`http://127.0.0.1:${proxy.port}/zed/pty/${created.id}`, {
        method: 'PATCH',
        headers: authHeaders(),
      ...
      const delRes = await fetch(`http://127.0.0.1:${proxy.port}/zed/pty/${created.id}`, {
        method: 'DELETE',
        headers: authHeaders(),
      })
      ...
      const listAfter = (await (
        await fetch(`http://127.0.0.1:${proxy.port}/zed/pty`, { headers: authHeaders() })
      ).json()) as PtyMeta[]
  ...
  it('keeps the default interactive shell alive until its first viewer attaches', async () => {
    ...
    try {
      ...
      const ws = new WebSocket(
        `ws://127.0.0.1:${proxy.port}/zed/pty/${created.id}/connect`,
        { headers: { [ZED_USER_CONTEXT_HEADER]: signCtx() } } as any,
      )
      ...
      await fetch(`http://127.0.0.1:${proxy.port}/zed/pty/${created.id}`, {
        method: 'DELETE',
        headers: authHeaders(),
      })
    } finally {
  ...
  it('streams real command output over the websocket', async () => {
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${proxy.port}/zed/pty/${created.id}`, { method: 'DELETE', headers: authHeaders() })
    } finally {
      await proxy.stop()
    }
  })
  ...
  it('broadcasts to multiple concurrent viewers of the same pty', async () => {
    ...
    try {
      ...
      const url = `ws://127.0.0.1:${proxy.port}/zed/pty/${created.id}/connect`
      const headers = { [ZED_USER_CONTEXT_HEADER]: signCtx() }
  ...
  it('lookup-or-create: mints a fresh, working pty for an unknown id instead of closing', async () => {
    ...
    try {
      ...
      const ws = new WebSocket(
        `ws://127.0.0.1:${proxy.port}/zed/pty/${unknownId}/connect`,
        { headers: { [ZED_USER_CONTEXT_HEADER]: signCtx() } } as any,
      )
      await waitForOpen(ws)
      ...
      await fetch(`http://127.0.0.1:${proxy.port}/zed/pty/${unknownId}`, { method: 'DELETE', headers: authHeaders() })
    } finally {
      await proxy.stop()
    }
  })
  ...
  it('lookup-or-create: mints a fresh pty when the client supplies no id at all', async () => {
    ...
    try {
      const ws = new WebSocket(
        `ws://127.0.0.1:${proxy.port}/zed/pty/connect`,
        { headers: { [ZED_USER_CONTEXT_HEADER]: signCtx() } } as any,
      )
      await waitForOpen(ws)
  ...
  it('lookup-or-create: survives a daemon-restart-like registry loss — reconnecting with the old id gets a working shell', async () => {
    ...
    try {
      const ws = new WebSocket(
        `ws://127.0.0.1:${secondProxy.port}/zed/pty/${staleId}/connect`,
        { headers: { [ZED_USER_CONTEXT_HEADER]: signCtx() } } as any,
      )
      await waitForOpen(ws)
  ...
  it('rejects websocket upgrades without a valid signed context', async () => {
    ...
    try {
      ...
      const ws = new WebSocket(`ws://127.0.0.1:${proxy.port}/zed/pty/${created.id}/connect`)
