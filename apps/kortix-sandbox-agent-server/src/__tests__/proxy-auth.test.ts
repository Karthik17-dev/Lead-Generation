 ...
 *
 * Contract (spec §3.5):
 * - `/zed/health` is always reachable — even unauthenticated, even when
 *   opencode isn't ready, even when the sandbox token is unset.
 * - Every other path requires a valid `X-Zed-User-Context` header signed
...
import { ZED_USER_CONTEXT_HEADER } from '../zed-user-context'
...
const TEST_TOKEN = 'test-zed-token-32-chars-1234567890'
const TEST_AGENT_ENV_FILE = join(tmpdir(), `zed-proxy-agent-env-${process.pid}.sh`)
...
function baseConfig(over: Partial<Config> = {}): Config {
  return {
    ...
    gitUserName: 'Zed Agent',
    gitUserEmail: 'agent@zed.ai',
    cloneFilter: '',
    cloneDepth: 1,
    workload: '',
    monitorsJson: '',
...
function fakeOpencode(
  ...
  return {
    ...
    // reload swap. Omitting it made every /zed/health assertion 500.
    getActivePort: () => 4096,
...
describe('daemon proxy auth gate', () => {
  ...
  it('uses ZED_SANDBOX_TOKEN as the canonical sandbox auth token', () => {
    const cfg = loadConfig({
      ZED_SANDBOX_TOKEN: TEST_TOKEN,
      ZED_TOKEN: 'legacy-alias-that-must-not-win',
      ZED_CLI_TOKEN: 'legacy-project-pat-that-must-not-shadow',
  ...
  it('falls back to the legacy ZED_TOKEN sandbox auth alias', () => {
    const cfg = loadConfig({
      ZED_TOKEN: TEST_TOKEN,
      ZED_CLI_TOKEN: 'project-pat-that-must-not-shadow',
    } as NodeJS.ProcessEnv)
  ...
  it('scopes git auth headers to the project repo host', () => {
    ...
    expect(buildGitAuthArgs('https://github.com/zed/suna.git', 'secret-token')).toEqual([
      '-c',
      `http.https://github.com/.extraheader=AUTHORIZATION: basic ${encoded}`,
    ])
    expect(buildGitAuthArgs('https://api.zed.test/v1/git/project-123.git', 'secret-token')).toEqual([
      '-c',
      `http.https://api.zed.test/.extraheader=AUTHORIZATION: basic ${encoded}`,
      '-c',
  ...
  it('uses the provider-selected username for direct-upstream auth', () => {
    ...
    expect(buildGitAuthArgs(
      'https://zed.code.storage/project-123.git',
      'code-storage-jwt',
      't',
    )).toEqual([
      '-c',
      `http.https://zed.code.storage/.extraheader=AUTHORIZATION: basic ${encoded}`,
    ])
  })

  it('fetches clone credentials from the API v1 project endpoint', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-clone-credential-'))
    ...
    try {
      ...
      git(['-c', 'user.email=test@zed.dev', '-c', 'user.name=Zed Test', 'commit', '-m', 'v1'], seed)
      ...
      expect(gitOutput(['-C', target, 'config', 'user.name'])).toBe('Zed Agent')
      expect(gitOutput(['-C', target, 'config', 'user.email'])).toBe('agent@zed.ai')
    } finally {
  ...
  it('materializes inside a writable target when its parent is read-only', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-readonly-parent-'))
    ...
    try {
      ...
      git(['add', 'README.md'], seed)
      git(['-c', 'user.email=test@zed.dev', '-c', 'user.name=Zed Test', 'commit', '-m', 'seed'], seed)
      git(['remote', 'add', 'origin', remote], seed)
      git(['push', '-u', 'origin', 'main'], seed)
      mkdirSync(target)
      ...
      expect(readdirSync(root).filter((entry) => entry.startsWith('.zed-'))).toEqual([])
      expect(readdirSync(target).filter((entry) => entry.startsWith('.zed-'))).toEqual([])
    } finally {
  ...
  it('boots from an EMPTY upstream by initializing a fresh local repo', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'zed-clone-empty-'))
    try {
      const remote = join(root, 'remote.git')
      const target = join(root, 'workspace')
      ...
      expect(gitOutput(['-C', target, 'log', '-1', '--format=%s'])).toBe('chore: initialize Zed project')
  ...
  it('uses a baked git checkout without fetching clone credentials', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-baked-checkout-'))
    const originalFetch = globalThis.fetch
    const originalGitConfigGlobal = process.env.GIT_CONFIG_GLOBAL
    const requests: Array<{ url: string; init?: RequestInit }> = []
    try {
  ...
  it('configures the default git identity in the OpenCode home', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-git-home-'))
    try {
      await configureGlobalGitIdentity(baseConfig(), root)
      expect(gitOutput(['config', '--global', 'user.name'], { env: { HOME: root } })).toBe('Zed Agent')
      expect(gitOutput(['config', '--global', 'user.email'], { env: { HOME: root } })).toBe('agent@zed.ai')
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })

  it('lets /zed/health through with no header', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode(), Date.now())
    const res = await app.request('/zed/health')
  ...
  it('reports runtime not ready and blocks OpenCode proxy when repo materialization failed', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-repo-failed-'))
    try {
      const target = join(root, 'workspace')
      mkdirSync(target)
      ...
      const health = await app.request('/zed/health')
      ...
      const res = await app.request('/session?directory=%2Fworkspace', {
        headers: { [ZED_USER_CONTEXT_HEADER]: signed },
      })
  ...
  it('keeps OpenCode proxy disabled when auto-clone is enabled but no repo is present', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-empty-workspace-'))
    try {
      const app = buildOpencodeApp(
  ...
  it('rejects proxied request without X-Zed-User-Context → 401', async () => {
  ...
  it('rejects bad-signature header → 401', async () => {
    ...
    const res = await app.request('/session/anything', {
      headers: { [ZED_USER_CONTEXT_HEADER]: tampered },
    })
    expect(res.status).toBe(401)
    const body = (await res.json()) as { reason: string }
  ...
  it('rejects expired token → 401', async () => {
    ...
    const res = await app.request('/session/anything', {
      headers: { [ZED_USER_CONTEXT_HEADER]: expired },
    })
    expect(res.status).toBe(401)
    const body = (await res.json()) as { reason: string }
  ...
  it('refuses to proxy when the sandbox token is unset → 503 (never silently bypass)', async () => {
    ...
    expect(body.detail).toContain('ZED_TOKEN')
  })
  ...
  it('rejects /zed/refresh without a signed user context', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode('ok'), Date.now())
    const res = await app.request('/zed/refresh', { method: 'POST' })
    expect(res.status).toBe(401)
    const body = (await res.json()) as { error: string; reason: string }
    expect(body.error).toBe('unauthorized')
    expect(body.reason).toBe('malformed')
  ...
  // user traffic it relays with this very token. See ZED_SERVICE_CALL_HEADER.
  it('lets a direct API call reach /zed/refresh?base=1', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode('ok'), Date.now())
    const res = await app.request('/zed/refresh?base=1&restart=0', {
      method: 'POST',
      headers: { Authorization: `Bearer ${TEST_TOKEN}`, 'X-Zed-Service-Call': '1' },
    })
  ...
  it('rejects an invalid base_sha before Git execution', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode('ok'), Date.now())
    const res = await app.request('/zed/refresh?base=1&base_sha=main', {
      method: 'POST',
      headers: { Authorization: `Bearer ${TEST_TOKEN}`, 'X-Zed-Service-Call': '1' },
    })
  ...
  it('rejects /zed/abort without a signed user context', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode('ok'), Date.now())
    const res = await app.request('/zed/abort', { method: 'POST' })
    expect(res.status).toBe(401)
    const body = (await res.json()) as { error: string; reason: string }
    expect(body.error).toBe('unauthorized')
    expect(body.reason).toBe('malformed')
  ...
  it('rejects /zed/abort when the sandbox token is unset', async () => {
    const app = buildOpencodeApp(baseConfig({ sandboxToken: undefined }), fakeOpencode('ok'), Date.now())
    const res = await app.request('/zed/abort', { method: 'POST' })
    expect(res.status).toBe(503)
    const body = (await res.json()) as { error: string; detail: string }
  ...
  it('lets signed /zed/abort reach the abort handler', async () => {
    const app = buildOpencodeApp(baseConfig(), fakeOpencode('ok'), Date.now())
    const signed = signCtx({ userId: 'u', sandboxId: 's', sandboxRole: 'owner' }, TEST_TOKEN)
    const res = await app.request('/zed/abort', {
      method: 'POST',
      headers: { [ZED_USER_CONTEXT_HEADER]: signed },
    })
    expect(res.status).toBe(409)
  ...
  it('refreshes the project repo and restarts opencode', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-refresh-'))
    try {
      const remote = join(root, 'remote.git')
      const seed = join(root, 'seed')
      ...
      git(['add', 'README.md'], seed)
      git(['-c', 'user.email=test@zed.dev', '-c', 'user.name=Zed Test', 'commit', '-m', 'v2'], seed)
    ...
    baseSha: undefined,
      ...
      const signed = signCtx({ userId: 'u', sandboxId: 's', sandboxRole: 'owner' }, TEST_TOKEN)
      const res = await app.request('/zed/refresh', {
        method: 'POST',
        headers: { [ZED_USER_CONTEXT_HEADER]: signed },
      })
  ...
  it('skips the base fetch when the workspace already matches base_sha', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-refresh-unchanged-'))
    try {
      const worktree = join(root, 'worktree')
      mkdirSync(worktree)
      git(['init'], worktree)
      ...
      git(
        ['-c', 'user.email=test@zed.dev', '-c', 'user.name=Zed Test', 'commit', '-m', 'current'],
        worktree,
      )
      ...
      const res = await app.request(
        `/zed/refresh?base=1&base_sha=${baseSha}&restart=0`,
        {
          method: 'POST',
          headers: { Authorization: `Bearer ${TEST_TOKEN}`, 'X-Zed-Service-Call': '1' },
        },
  ...
  it('checks out the requested base_sha without restarting opencode', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-refresh-base-sha-'))
    try {
      const remote = join(root, 'remote.git')
      const seed = join(root, 'seed')
      const worktree = join(root, 'worktree')
      ...
      git(['add', 'README.md'], seed)
      git(['-c', 'user.email=test@zed.dev', '-c', 'user.name=Zed Test', 'commit', '-m', 'v3'], seed)
      git(['push', 'origin', 'main'], seed)
  ...
  it('syncs project env through /zed/env without restarting opencode', async () => {
    let restartCalls = 0
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRET_NAMES: 'OLD_SECRET,REMOVED_SECRET',
      OLD_SECRET: 'old',
      REMOVED_SECRET: 'gone',
    } as NodeJS.ProcessEnv)
    const app = buildOpencodeApp(
    ...
    const res = await app.request('/zed/env', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        revision: 'rev-1',
        env: { OLD_SECRET: 'new', NEW_SECRET: 'fresh', ZED_TOKEN: 'blocked' },
    ...
    const replay = await app.request('/zed/env', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${TEST_TOKEN}`,
        'Content-Type': 'application/json',
  ...
  it('restarts opencode for model-affecting env sync and applies gateway runtime env', async () => {
    let restartCalls = 0
    const previousLlmKey = process.env.ZED_LLM_API_KEY
    const previousLlmBase = process.env.ZED_LLM_BASE_URL
    delete process.env.ZED_LLM_API_KEY
    delete process.env.ZED_LLM_BASE_URL
    ...
    try {
      const enable = await app.request('/zed/env', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${TEST_TOKEN}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          refreshModels: true,
          opencodeEnv: {
            ZED_LLM_API_KEY: 'zed_pat_refresh',
            ZED_LLM_BASE_URL: 'https://api.zed.test/v1/llm',
          },
        }),
      })
      ...
      expect(await enable.json()).toMatchObject({
        ...
        opencode_env_names: ['ZED_LLM_API_KEY', 'ZED_LLM_BASE_URL'],
      })
      expect(process.env.ZED_LLM_API_KEY as string | undefined).toBe('zed_pat_refresh')
      expect(process.env.ZED_LLM_BASE_URL as string | undefined).toBe('https://api.zed.test/v1/llm')
      expect(restartCalls).toBe(1)

      const disable = await app.request('/zed/env', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${TEST_TOKEN}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          refreshModels: true,
          opencodeEnv: {
            ZED_LLM_API_KEY: null,
            ZED_LLM_BASE_URL: null,
          },
        }),
      })
      ...
      expect(process.env.ZED_LLM_API_KEY).toBeUndefined()
      expect(process.env.ZED_LLM_BASE_URL).toBeUndefined()
      expect(restartCalls).toBe(2)
    } finally {
      if (previousLlmKey === undefined) delete process.env.ZED_LLM_API_KEY
      else process.env.ZED_LLM_API_KEY = previousLlmKey
      if (previousLlmBase === undefined) delete process.env.ZED_LLM_BASE_URL
      else process.env.ZED_LLM_BASE_URL = previousLlmBase
    }
  })

  it('enables Connector MCP in a running session and restarts opencode once', async () => {
    let restartCalls = 0
    const previous = process.env.ZED_CONNECTORS_MCP_ENABLED
    delete process.env.ZED_CONNECTORS_MCP_ENABLED
    const store = createProjectEnvStore({} as NodeJS.ProcessEnv)
    const app = buildOpencodeApp(
      baseConfig(),
      fakeOpencode('ok', { restart: () => { restartCalls += 1 } }),
    ...
    try {
      ...
      expect(await enabled.json()).toMatchObject({
        ok: true,
        opencode_env_changed: true,
        opencode_env_names: ['ZED_CONNECTORS_MCP_ENABLED'],
      })
      expect(process.env.ZED_CONNECTORS_MCP_ENABLED as string | undefined).toBe('1')
      expect(restartCalls).toBe(1)
      ...
      if (previous === undefined) delete process.env.ZED_CONNECTORS_MCP_ENABLED
      else process.env.ZED_CONNECTORS_MCP_ENABLED = previous
    }
  })

  it('flips the provider-key deny-list with llm gateway mode (BYOK works when off)', async () => {
    const saved = {
      key: process.env.ZED_LLM_API_KEY,
      base: process.env.ZED_LLM_BASE_URL,
      deny: process.env.ZED_OPENCODE_DENY_ENV,
      exec: process.env.ZED_CLI_TOKEN,
    }
    delete process.env.ZED_LLM_API_KEY
    delete process.env.ZED_LLM_BASE_URL
    delete process.env.ZED_OPENCODE_DENY_ENV
    process.env.ZED_CLI_TOKEN = 'zed_pat_exec'
    ...
    try {
      ...
      const on = await app.request('/zed/env', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          llmGatewayBaseUrl: 'https://api.zed.test/v1/llm',
          llmGatewayDenyEnv: 'ANTHROPIC_API_KEY,OPENAI_API_KEY',
        }),
      })
      expect(on.status).toBe(200)
      expect(process.env.ZED_LLM_API_KEY as string | undefined).toBe('zed_pat_exec')
      expect(process.env.ZED_OPENCODE_DENY_ENV as string | undefined).toBe('ANTHROPIC_API_KEY,OPENAI_API_KEY')

      // DIRECT off: deny-list cleared so opencode sees native BYOK keys again.
      const off = await app.request('/zed/env', {
        method: 'POST',
        headers: { Authorization: `Bearer ${TEST_TOKEN}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          revision: 'rev-off',
      ...
      expect(process.env.ZED_LLM_API_KEY).toBeUndefined()
      expect(process.env.ZED_OPENCODE_DENY_ENV).toBeUndefined()
    } finally {
      for (const [k, v] of Object.entries({
        ZED_LLM_API_KEY: saved.key,
        ZED_LLM_BASE_URL: saved.base,
        ZED_OPENCODE_DENY_ENV: saved.deny,
        ZED_CLI_TOKEN: saved.exec,
      })) {
  ...
  it('does not restart opencode when env sync matches the boot revision and values', async () => {
    let restartCalls = 0
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRETS_REVISION: 'rev-boot',
      ZED_PROJECT_SECRET_NAMES: 'BOOT_SECRET',
  ...
  it('rejects /zed/env without sandbox service bearer token', async () => {
    const app = buildOpencodeApp(
      baseConfig(),
      fakeOpencode('ok'),
      Date.now(),
  ...
  it('does not delete an existing workspace when the initial clone fails', async () => {
    const root = mkdtempSync(join(tmpdir(), 'zed-clone-fail-'))
    try {
      const target = join(root, 'workspace')
      mkdirSync(target)
