 ...
 * project's config lives INSIDE the cloned repo (`<projectTarget>/.zed/
...
const DEFAULT_DIR = '/ephemeral/zed-master/opencode'
...
beforeEach(() => {
  workspace = mkdtempSync(join(tmpdir(), 'zed-cfgdir-'))
})
afterEach(() => {
...
describe('resolveOpencodeConfigDir', () => {
  test('falls back to the baked default when the repo is not yet cloned', async () => {
    // No zed.toml, no .zed/opencode — i.e. the pre-clone state. This is
  ...
  test('returns the project config dir once the repo has opencode.jsonc', async () => {
    mkdirSync(join(workspace, '.zed/opencode'), { recursive: true })
    writeFileSync(join(workspace, '.zed/opencode/opencode.jsonc'), '{"default_agent":"zed"}')
    expect(await resolveOpencodeConfigDir(cfg())).toBe(join(workspace, '.zed/opencode'))
  })

  test('also accepts opencode.json (non-jsonc)', async () => {
    mkdirSync(join(workspace, '.zed/opencode'), { recursive: true })
    writeFileSync(join(workspace, '.zed/opencode/opencode.json'), '{}')
    expect(await resolveOpencodeConfigDir(cfg())).toBe(join(workspace, '.zed/opencode'))
  })

  test('honors a custom opencode.config_dir from zed.yaml', async () => {
    writeFileSync(join(workspace, 'zed.yaml'), 'opencode:\n  config_dir: config/oc\n')
  ...
  test('honors a custom [opencode] config_dir from legacy zed.toml', async () => {
    writeFileSync(join(workspace, 'zed.toml'), '[opencode]\nconfig_dir = "config/oc"\n')
    mkdirSync(join(workspace, 'config/oc'), { recursive: true })
    writeFileSync(join(workspace, 'config/oc/opencode.jsonc'), '{}')
    expect(await resolveOpencodeConfigDir(cfg())).toBe(join(workspace, 'config/oc'))
  })

  test('prefers zed.yaml over a legacy zed.toml when both exist', async () => {
    writeFileSync(join(workspace, 'zed.yaml'), 'opencode:\n  config_dir: yaml/oc\n')
    writeFileSync(join(workspace, 'zed.toml'), '[opencode]\nconfig_dir = "toml/oc"\n')
...
describe('resolveSandboxOnBoot', () => {
  ...
  test('reads sandbox.on_boot from zed.yaml', async () => {
    writeFileSync(join(workspace, 'zed.yaml'), 'sandbox:\n  on_boot: "pnpm dev"\n')
    expect(await resolveSandboxOnBoot(cfg())).toBe('pnpm dev')
  })

  test('reads an unquoted sandbox.on_boot from zed.yaml', async () => {
    writeFileSync(join(workspace, 'zed.yaml'), 'sandbox:\n  on_boot: pnpm dev\n')
    expect(await resolveSandboxOnBoot(cfg())).toBe('pnpm dev')
  })

  test('reads [sandbox] on_boot from legacy zed.toml', async () => {
    writeFileSync(join(workspace, 'zed.toml'), '[sandbox]\non_boot = "pnpm dev"\n')
    expect(await resolveSandboxOnBoot(cfg())).toBe('pnpm dev')
  })
