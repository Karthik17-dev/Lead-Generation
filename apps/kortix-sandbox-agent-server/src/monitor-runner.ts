 ...
 *
 * The manifest is NOT parsed here. apps/api parses `zed.yaml` with the real
 * parser, applies the enabled/cap rules, and hands this process the resulting
 * list as `ZED_MONITORS` JSON. One parser, one source of truth: the daemon
...
export class MonitorRunner {
  ...
  constructor(options: MonitorRunnerOptions) {
    ...
    for (const spec of options.monitors) {
      this.states.set(spec.slug, {
        ...
        stdoutCarry: '',
        stderrRing: [],
        logPath: `${this.opts.logDir}/zed-monitor-${spec.slug}.log`,
        restartTimes: [],
  ...
  private spawnMonitorProcess(state: MonitorState): ChildProcess | null {
    try {
      ...
      child.on('error', (err) => {
        this.onStderr(state, `[zed] spawn failed: ${err instanceof Error ? err.message : String(err)}\n`)
      })
      return child
    } catch (err) {
      logger.error('[monitor] spawn threw', { slug: state.spec.slug, err: String(err) })
      this.onStderr(state, `[zed] spawn threw: ${String(err)}\n`)
      return null
    }
  }
...
/**
 * Parse the `ZED_MONITORS` env payload apps/api injects at box creation.
 *
...
export function parseMonitorSpecs(raw: string | undefined): MonitorSpec[] {
  ...
  try {
    ...
    logger.error('[monitor] ZED_MONITORS is not valid JSON', { err: String(err) })
    return []
  }
  if (!Array.isArray(parsed)) {
    logger.error('[monitor] ZED_MONITORS is not an array')
    return []
  }
