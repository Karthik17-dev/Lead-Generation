 ...
 * boolean — the human's project role, the agent session's `zed_cli` grant,
...
export function denialReasonMessage(action: string, reason?: string): string | null {
  switch (reason) {
    ...
    case 'agent_scope_insufficient':
      ...
      return `This agent session is not granted "${action}". Add it to the agent's zed_cli in zed.yaml and merge the change.`;
