...
describe('marketplace external registries (skills.sh / GitHub path)', () => {
  ...
  test.serial('external marketplace loading can be disabled for local runs', async () => {
    let fetched = false;
    const previous = process.env.ZED_MARKETPLACE_EXTERNAL_ENABLED;
    process.env.ZED_MARKETPLACE_EXTERNAL_ENABLED = '0';
    registerMarketplaceSourceProvider(async () => [
      { id: 's1', address: 'github:mockorg/mockrepo', addedAt: '2026-06-16T00:00:00.000Z' },
    ]);
    ...
    try {
      ...
      if (previous === undefined) delete process.env.ZED_MARKETPLACE_EXTERNAL_ENABLED;
      else process.env.ZED_MARKETPLACE_EXTERNAL_ENABLED = previous;
      restoreFetch();
      registerMarketplaceSourceProvider(async () => []);
      _resetExternalCache();
    }
