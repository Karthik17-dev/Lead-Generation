...
import { tunnelConnections, tunnelRpcForwards } from '@zed/db';
...
async function createConnectedTunnel() {
  ...
  const oldHash = hashSecretKey(`zed_tnl_${crypto.randomUUID()}`);
...
describe('live relay credential invalidation', () => {
  test('token rotation on another replica blocks the next local RPC and closes the socket', async () => {
    ...
    await db
      ...
      .set({ setupTokenHash: hashSecretKey(`zed_tnl_${crypto.randomUUID()}`) })
