...
describe('serializeSession ⇄ ProjectSessionSchema', () => {
  ...
  test('runtime snapshot root title wins over the Zed auto title', () => {
  ...
  test('placeholder or blank runtime titles fall back to the Zed auto title', () => {
    ...
    expect(ProjectSessionSchema.strict().parse(placeholder).name).toBe('Real Zed Title');
    ...
    expect(ProjectSessionSchema.strict().parse(blank).name).toBe('Real Zed Title');
  });
