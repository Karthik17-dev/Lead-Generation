...
describe('[[triggers]] — type = "monitor"', () => {
  ...
  test('a monitor survives a full manifest round-trip unchanged', () => {
    ...
    const reparsed = extractTriggers(parseManifestString(rewritten, 'yaml', 'zed.yaml')).specs[0]!;
  ...
  test('a stream monitor with an explicit fresh mode round-trips that mode', () => {
    ...
    expect(extractTriggers(parseManifestString(rewritten, 'yaml', 'zed.yaml')).specs[0]).toEqual(
      original,
    );
  });
});
