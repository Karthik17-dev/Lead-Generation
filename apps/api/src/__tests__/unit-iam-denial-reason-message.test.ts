...
// project role, the agent session's `zed_cli` grant, and an activated service
...
describe('denialReasonMessage', () => {
  test('agent_scope_insufficient names the agent grant, not the role', () => {
    ...
    expect(message).toBe(
      'This agent session is not granted "project.session.start". Add it to the agent\'s zed_cli in zed.yaml and merge the change.',
    );
