  ...
  test('creates GitHub session branches through the refs API without git fetch', async () => {
    ...
    const result = JSON.parse(bunEval(`
      ...
      globalThis.fetch = async (input, init = {}) => {
        ...
        if (method === 'GET' && url.endsWith('/repos/karthikxa/AI-Management-System/git/ref/heads%2Fmain')) {
          return new Response(JSON.stringify({ object: { sha: '${baseSha}', type: 'commit' } }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          });
        }
        if (method === 'POST' && url.endsWith('/repos/karthikxa/AI-Management-System/git/refs')) {
      ...
      await createRemoteSessionBranch(
        {
          ...
          repoUrl: 'https://github.com/karthikxa/AI-Management-System.git',
          defaultBranch: 'main',
