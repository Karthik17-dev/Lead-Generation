...
// the whole point of the route is what lands in zed.yaml.
...
const GOVERNED_YAML = `zed_version: 2
...
const ALL_GRANT_YAML = `zed_version: 2
default_agent: support
agents:
  support:
    secrets: all
...
const UNGOVERNED_YAML = `zed_version: 2
default_agent: support
project:
  name: demo
`;
...
function governedManifest(): ParsedManifest {
  return parseManifestString(GOVERNED_YAML, 'yaml', 'zed.yaml', 'sha-governed');
}
...
describe('POST /v1/projects/:projectId/secrets/:identifier/grant', () => {
  ...
  test('merges into a declared agent and preserves every other governance field', async () => {
    ...
    expect(support.zed_cli).toEqual(['project.cr.open']);
    expect(support.skills).toBe('all');
    // The other agent's block is untouched.
    expect(agentsOf(commits[0]!.raw).scout).toEqual({ zed_cli: 'all' });
  });
  ...
  test('reports adopted_governance on the manifest that gains its FIRST agents block', async () => {
    manifest = parseManifestString(UNGOVERNED_YAML, 'yaml', 'zed.yaml', 'sha-ungoverned');
  ...
  test('reports adopted_governance when the repo carries no manifest at all', async () => {
    // What loadManifestForEdit synthesizes for a blank project: a `zed`
  ...
  test("expands `secrets: all` instead of narrowing the agent to one identifier", async () => {
    manifest = parseManifestString(ALL_GRANT_YAML, 'yaml', 'zed.yaml', 'sha-all');
  ...
  test('a v1 manifest is a 400, never a 500', async () => {
    manifest = parseManifestString(V1_TOML, 'toml', 'zed.toml', 'sha-v1');
    ...
    expect(body.error).toContain('zed.yaml');
    expect(commits).toHaveLength(0);
  });
