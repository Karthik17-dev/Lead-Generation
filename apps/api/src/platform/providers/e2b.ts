 ...
 * Zed's own config defaults E2B_DOMAIN to `e2b.dev`. Leaving it off pointed
...
export class E2BProvider implements SandboxProvider {
   ...
   * billed as if Zed had allocated it.
   */
  readonly appMachineSupport: AppMachineSupport = { cpu: true, memoryGb: true, diskGb: false };

  /**
   * Relaunch zed-appd if it died. This used to be a no-op on the reasoning
  ...
  async ensureAppRuntimeStarted(externalId: string): Promise<void> {
    ...
    const envVars = await loadRuntimeEnv(sandbox);
    if (envVars.ZED_WORKLOAD_TYPE !== 'app') return;
