 ...
 *
 * The capability is a ROW (`zed.impersonation_grants`), never a token. The
...
// and a dozen unit suites mock `@zed/db` with a partial shape. A static
...
async function grantsTable() {
  const { impersonationGrants } = await import('@zed/db');
...
export const IMPERSONATION_HEADER = 'x-zed-impersonate';
...
const IMPERSONATION_FORBIDDEN_ROUTES: ForbiddenRoute[] = [
  ...
  // Membership. `POST /v1/accounts/:id/members` adds an EXISTING Zed user
  ...
  // commit durable state into `zed.yaml`: widening what an agent may read
