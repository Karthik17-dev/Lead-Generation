 ...
 *
 * THE PROBLEM THIS EXISTS FOR. A sandbox's `zed` CLI and its managed-skill
 ...
 * every one of those sandboxes started 404ing on `zed connectors …` and had
 ...
 * `zed_pat_` and an in-sandbox daemon holding the platform-injected
 * `ZED_CLI_TOKEN` session PAT. Content is identical for every caller — this
...
runtimeAssetsApp.openapi(
  createRoute({
    ...
    description:
      'Identifies the `zed` CLI binary and managed-skill overlay this API was built with. ' +
...
async function serveCli(c: {
  ...
  c.header('X-Zed-Cli-Sha256', manifest.cli_sha256);
  if (manifest.cli_version) c.header('X-Zed-Cli-Version', manifest.cli_version);
...
runtimeAssetsApp.openapi(
  createRoute({
    ...
    path: '/cli',
    tags: ['runtime-assets'],
    summary: 'GET /runtime-assets/cli — the `zed` binary this deploy bakes into sandboxes',
    description:
    ...
    responses: {
      200: {
        description: 'The compiled zed CLI',
...
runtimeAssetsApp.openapi(
  createRoute({
    ...
    path: '/managed-skills',
    tags: ['runtime-assets'],
    summary: 'GET /runtime-assets/managed-skills — the managed `zed-*` skill overlay',
    description:
      'Every file of the overlay a sandbox writes to /opt/zed/managed-skills, byte-identical ' +
