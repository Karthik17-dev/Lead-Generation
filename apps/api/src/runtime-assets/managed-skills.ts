/**
 * The managed `zed-*` skill overlay, as data.
 *
 ...
 *     exact set into the META image's `/opt/zed/managed-skills`,
 ...
 * baked `/opt/zed/managed-skills`, so an ordinary project sandbox had nothing
 ...
 * 'general-knowledge-worker' })` sources, same `isZedManagedSkillName` filter
...
export interface ManagedSkillOverlayFile {
  /** Path relative to the overlay root, e.g. `zed-system/SKILL.md`. */
  path: string;
  content: string;
}
...
export function managedSkillOverlayFiles(): ManagedSkillOverlayFile[] {
  ...
  for (const file of files) {
    ...
    if (!name || !isZedManagedSkillName(name)) continue;
