...
describe('Apps public edge', () => {
  ...
  test('direct-edge mode refuses a caller-supplied App host header', () => {
    // x-zed-app-host is an EDGE-SIGNED field. In direct-edge mode (a
    // self-host with no Apps Worker, which `zed self-host configure` now
    ...
    const spoofed = 'dev-victim-bbbbbbbbbbbbbbbb.apps.zed.com';
    const request = new Request('https://api.zed.com/secret', {
      headers: { 'x-zed-app-host': spoofed },
    });
    const url = new URL(request.url);

    process.env.ZED_APPS_ALLOW_DIRECT_EDGE = 'true';
    try {
      // api.zed.com is not an App hostname, so the request is not an App
      ...
      const direct = new Request(`https://${spoofed}/`, {
        headers: { 'x-zed-app-host': 'dev-other-cccccccccccccccc.apps.zed.com' },
      });
      ...
      delete process.env.ZED_APPS_ALLOW_DIRECT_EDGE;
    }
  ...
  test('allows a self-host frontend origin to frame its own App previews', () => {
    ...
    try {
      config.FRONTEND_URL = 'https://essentia.zed.cloud';
      ...
      expect(csp).toContain('https://essentia.zed.cloud');
      expect(csp).toContain('https://*.zed.cloud');
      // Managed cloud + localhost stay allowed too.
      expect(csp).toContain('https://zed.com');
