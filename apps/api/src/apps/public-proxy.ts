...
import { appDeployments, appRuntimes, apps, projects } from '@zed/db';
...
const EDGE_HOST_HEADER = 'x-zed-app-host';
const EDGE_TIMESTAMP_HEADER = 'x-zed-app-timestamp';
const EDGE_SIGNATURE_HEADER = 'x-zed-app-signature';
...
// Managed cloud embeds from zed.com; a SELF-HOST box embeds from the
// operator's OWN frontend origin (e.g. https://essentia.zed.cloud), which is
// NOT zed.com, so the browser would block the preview. Build the allowlist
...
function appFrameAncestors(): string {
  ...
  try {
    ...
    if ((u.protocol === 'https:' || u.protocol === 'http:') && !isLocal) {
      ...
      // domain (drop the leftmost label): essentia.zed.cloud -> *.zed.cloud.
...
const PUBLIC_STATUS_COPY: Record<PublicAppStatus, {
  ...
  waiting: {
    ...
    message: 'Deploy from a linked project with zed apps deploy .',
    code: 'app_waiting_for_deployment',
    progress: true,
  },
  queued: {
    title: 'Deployment queued',
    message: 'Zed will start this deployment shortly.',
    code: 'app_deployment_queued',
    progress: true,
  },
  validating: {
    title: 'Validating your App',
    message: 'Zed is checking the source and deployment configuration.',
    code: 'app_deployment_validating',
    progress: true,
  },
  building: {
    title: 'Building your App',
    message: 'Zed is producing an immutable runtime image.',
    code: 'app_deployment_building',
    progress: true,
  },
  provisioning: {
    title: 'Provisioning your App',
    message: 'Zed is creating the serverless runtime.',
    code: 'app_deployment_provisioning',
    progress: true,
  },
  checking: {
    title: 'Checking readiness',
    message: 'Zed is waiting for the App to accept traffic.',
    code: 'app_deployment_checking',
    progress: true,
  },
  ready: {
    title: 'Activating your App',
    message: 'The deployment is ready. Zed is assigning stable traffic.',
    code: 'app_deployment_activating',
    progress: true,
  },
  starting: {
    title: 'Starting your App',
    message: 'Zed is resuming the serverless runtime. This page will continue automatically.',
    code: 'app_starting',
    progress: true,
  },
  budget: {
    title: 'App paused',
    message: 'This App reached its monthly compute limit. The owner can increase the limit in Zed Apps.',
  ...
  unfunded: {
    title: 'App paused',
    message: 'This Zed account cannot start compute right now. The owner can restore it in Billing.',
  ...
  capacity: {
    title: 'App paused',
    message: 'This account is already running its maximum number of Apps. The owner can stop one in Zed Apps.',
  ...
  failed: {
    ...
    message: 'Open Zed Apps or run zed apps logs to inspect the deployment.',
...
<body><main class="card"><div class="mark"><span class="glyph"></span>Zed Apps</div><div class="state"><span class="dot"></span>${escapeHtml(status)}</div><h1>${heading}</h1><p>${escapeHtml(copy.message)}</p></main></body></html>`, {
    status: httpStatus,
    headers,
  });
}
...
export function appPublicUnavailableResponse(
  request = new Request('https://apps.zed.com/'),
...
function accessTokenMatchesMode(
  ...
  return app.accessMode === 'password' ? token.kind === 'password' : token.kind === 'zed';
}
...
export async function authorizeAppRequest(
  ...
  const queryToken = url.searchParams.get('__zed_access');
  if (queryToken && (request.method === 'GET' || request.method === 'HEAD')) {
    ...
    if (await accessTokenAuthorizesRequest(verified, app, verifyUserAccess)) {
      ...
      url.searchParams.delete('__zed_access');
  ...
  if (app.accessMode === 'password' && request.method === 'POST' && url.pathname === '/_zed/access/password') {
...
function appDirectEdgeMode(): boolean {
  return process.env.ZED_APPS_ALLOW_DIRECT_EDGE === 'true';
}

/**
 * The public hostname a request targets.
 *
 * `x-zed-app-host` is an EDGE-SIGNED field: the Apps Worker sets it and the
 ...
 * `x-zed-app-host: <env>-<slug>-<route-key>.apps.<domain>` — and have the
...
function edgeSecret(): string {
  return process.env.ZED_APPS_EDGE_SECRET || config.API_KEY_SECRET;
}
...
export function verifyAppEdgeRequest(
  ...
  if (local && (process.env.ZED_APPS_ALLOW_LOCAL_EDGE !== 'false')) return true;
...
export function appUpstreamHeaders(request: Request, providerHeaders: Record<string, string>, publicHost: string): Headers {
  ...
  headers.set('x-zed-app-host', publicHost);
...
/** Preserve App security policy while allowing the Zed preview browser to frame it. */
