...
// switch: it resolves "the default" to a concrete name (e.g. `zed`) for
...
function isConcreteAgentSwitch(requestedAgent: string | null, sessionAgent: string): boolean {
  ...
  // and really did have the token re-minted to its connector/Zed-CLI grant —
    ...
    try {
      ...
      if (shouldSyncProjectEnvBeforeProxy(port, method, remainingPath)) {
        ...
        if (refusal) {
          ...
          // /zed/env, never to opencode's session endpoints, and it runs
