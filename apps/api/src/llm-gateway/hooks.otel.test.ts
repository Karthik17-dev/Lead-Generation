...
describe('emitGatewayGenAiSpan', () => {
  ...
  test('emits the candidate failure chain and fallback outcome attributes', async () => {
    ...
    const span = lastSpan();
    expect(attrScalar(span, 'zed.failure_count')).toBe('2');
    expect(attrScalar(span, 'zed.failure_codes')).toBe(
      'context_length_exceeded,empty_completion',
    );
    expect(attrScalar(span, 'zed.context_rejected')).toBe(true);
    // `zed.probe_timeout` is gone — a probe timeout commits the stream
    // rather than failing it, so the attribute could only ever report false.
    expect(attrScalar(span, 'zed.probe_timeout')).toBeUndefined();
    expect(attrScalar(span, 'zed.fallback_recovered')).toBe(false);
    ...
    expect(attrScalar(lastSpan(), 'zed.fallback_recovered')).toBe(true);
  });
