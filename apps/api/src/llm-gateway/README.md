...
A gateway `502` means Zed exhausted the configured route before it received a
...
OpenTelemetry spans expose `zed.failure_count`, `zed.failure_codes`,
`zed.context_rejected`, `zed.probe_timeout`, and
`zed.fallback_recovered`. Use `request_id` to correlate the client error,
...
`zed.com` zone reports `proxy_read_timeout = 125` seconds with
