  ...
  try {
    ...
    void emitOtelSpan({
      ...
      attributes: {
        ...
        'zed.ok': trace.ok,
        'zed.failure_count': attemptFailures.length,
        'zed.failure_codes': failureCodes.join(','),
        'zed.context_rejected': failureCodes.includes('context_length_exceeded'),
        // `zed.probe_timeout` removed: the gateway's first-byte deadline
        ...
        // false on every span.
        'zed.fallback_recovered': trace.ok && attemptFailures.length > 0,
