...
describe('toWireModel / toOpencodeModelRef', () => {
  ...
  test('re-prefixes a bare managed id to the opencode ref; leaves BYOK/codex alone', () => {
    ...
    expect(toOpencodeModelRef('deepseek-v4-flash')).toBe('zed/deepseek-v4-flash');
...
describe('degradeUnservableDefault — stale default guard', () => {
  ...
  test('managed default is trusted without a probe (bare id and zed/ ref)', async () => {
    ...
    expect(
      await degradeUnservableDefault('zed/deepseek-v4-flash', { hasProject: true }, neverProbe),
    ).toBe('zed/deepseek-v4-flash');
  });
