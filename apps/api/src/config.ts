...
const envSchema = z.object({
  ...
  // (e.g. https://essentia.zed.cloud) used to rewrite signed URLs on the way
   ...
   * `zed secrets call` and the connector MCP, so if a provider ever
   ...
   */
  ZED_SANDBOX_EGRESS_PIN_ENFORCED: optBoolTrue,
  ...
  //
  // Whether a session's sandbox gets the `zed-connectors` OpenCode MCP
  // server (ZED_CONNECTORS_MCP_ENABLED in the guest). It exposes the
...
export const config = {
  ...
  ZED_SANDBOX_EGRESS_PIN_ENFORCED: env.ZED_SANDBOX_EGRESS_PIN_ENFORCED,
