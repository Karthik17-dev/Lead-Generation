...
/** Names are `zed-meta-<env>-<hash16>`; see `metaSnapshotName`. */
const META_SNAPSHOT_PREFIX = 'zed-meta';
 ...
 *
 * Older builds used `zed-meta-<hash16>` with no namespace. Those names do
...
export function metaSnapshotName(contentHash: string): string {
  return `${META_SNAPSHOT_PREFIX}-${config.INTERNAL_ZED_ENV}-${contentHash.slice(0, 16)}`;
}
 ...
 * superseded it. Measured 2026-08-12: 118 `zed-meta-*` snapshots, all under
...
export async function reapSupersededMetaSnapshots(
  ...
  try {
    const mine = `${META_SNAPSHOT_PREFIX}-${config.INTERNAL_ZED_ENV}-`;
