 ...
 * `zed-meta-*` snapshots, all under 14 days old, ~8/day, against a
...
describe('meta snapshot naming', () => {
  test('is namespaced by environment', () => {
    ...
    expect(name).toBe(`zed-meta-${config.INTERNAL_ZED_ENV}-${'a'.repeat(16)}`);
  });
...
describe('meta snapshot reap', () => {
  test("never touches another environment's images", async () => {
    ...
    const env = config.INTERNAL_ZED_ENV;
    const others = ['dev', 'staging', 'prod', 'preview'].filter((e) => e !== env);
    const foreign = others.map((e) => `zed-meta-${e}-${'f'.repeat(16)}`);
