...
export type SecretCapability =
  ...
  | {
       ...
       *  - `redact`  Zed replaces the value with `[REDACTED]` and returns
...
const SHARED_BOUNDARY_NOTES: readonly string[] = [
  'Zed adds these credentials outside the sandbox, at the network boundary.',
...
export const NETWORK_BOUNDARY_NOTES_REDACT: readonly string[] = [
  ...
  'Requests are relayed through Zed, so responses are not streamed: no SSE, no websockets, and large bodies are capped.',
  'A listed host that answers 401 means the header did not arrive. Report that; do not invent a credential.',
]
