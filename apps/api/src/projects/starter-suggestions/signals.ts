...
import { connectorConnections, connectors, projectSessions } from '@zed/db';
...
/**
 * `.zed/memory/MEMORY.md` first, then the remaining `.md` files under
 * `.zed/memory/` (alphabetical), reading only as many as fit the
...
async function readMemoryFiles(project: GitBackedProject): Promise<Array<{ path: string; content: string }>> {
  ...
  try {
    entries = await listRepoFiles(project, project.defaultBranch, '.zed/memory');
  } catch {
    // `.zed/memory` doesn't exist, or the listing failed — no memory signal.
    return [];
  }
