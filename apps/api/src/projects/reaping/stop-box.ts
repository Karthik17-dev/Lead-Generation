...
import { encodeZedUserContext, ZED_USER_CONTEXT_HEADER } from '../../shared/zed-user-context';
...
/** The daemon's control port; zed-sandbox-agent-server owns `/zed/abort`. */
 ...
 * `encodeZedUserContext` (shared/zed-user-context.ts), the same trio
 ...
 * expiry, the 24h run cap). The daemon's `/zed/abort` only verifies the
...
export async function abortLiveTurnBeforeStop(input: {
  ...
  try {
    ...
    const headers: Record<string, string> = {
      ...
      [ZED_USER_CONTEXT_HEADER]: encodeZedUserContext(
        {
    ...
    const res = await fetch(`${ingress.url.replace(/\/$/, '')}/zed/abort`, {
      method: 'POST',
      headers,
