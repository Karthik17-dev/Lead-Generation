...
// ─── Change Requests ────────────────────────────────────────────────────────
// Zed-native PR layer. The CR is metadata stored alongside the project;
...
// Zed even when the repo is hosted elsewhere.
//
...
// Kept (wired through to the daemon /zed/git/commit-push route) as the
...
projectsApp.openapi(
  ...
  async (c: any) => {
    ...
    // The capability check above is PROJECT-wide, and in Zed-as-a-Backend the
    ...
    const callerSandboxSessionId = callerZedSessionId(c);
    if (
    ...
    // (and the Daytona preview headers), which the daemon's /zed/git route
    // validates against ZED_TOKEN — same contract as /zed/env.
    ...
    try {
      daemonRes = await fetch(`${endpoint.url.replace(/\/$/, '')}/zed/git/commit-push`, {
          method: 'POST',
