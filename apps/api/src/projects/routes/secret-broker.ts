  ...
  async (c) => {
    ...
    if (!pin.ok) {
      ...
      console.warn('[secret-broker] refused an off-sandbox token use', {
        ...
        pinned: pin.pinned,
        seen: pin.seen,
        enforced: config.ZED_SANDBOX_EGRESS_PIN_ENFORCED,
      });
    }
    if (!pin.ok && config.ZED_SANDBOX_EGRESS_PIN_ENFORCED) {
      return c.json(
        {
    ...
    try {
      ...
      // purpose:
      //
      //  - `broker`/`zed_fetch` — the agent calls the route itself
      //    (`zed secrets call`, the `secret_call` MCP tool).
      ...
      const isBrokerSecret =
        shared.strategy === 'broker' && shared.egressPolicy?.backend === 'zed_fetch';
      ...
      // under. A broker secret's snapshot carries `zed_fetch`; a boundary
      // policy carries no backend at all — `networkBoundaryPolicyError` rejects
      // one that does — so demanding `zed_fetch` of both would make a
