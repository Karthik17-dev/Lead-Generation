...
describe('manifest file compare-and-swap', () => {
  test('allows credential-free absolute repositories only in local development', async () => {
    const previous = process.env.ZED_LOCAL_DEV;
    ...
    try {
      delete process.env.ZED_LOCAL_DEV;
      expect(
      ...
      if (previous === undefined) delete process.env.ZED_LOCAL_DEV;
      else process.env.ZED_LOCAL_DEV = previous;
    }
  });
