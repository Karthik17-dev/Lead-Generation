 ...
 * assets (the `zed` CLI and the managed-skill overlay — see
 * apps/zed-sandbox-agent-server/src/runtime-assets.ts).
 *
 ...
 * /zed/refresh` already exists, is already called on warm reuse and on
  ...
  for (const delay of RETRY_DELAYS_MS) {
    ...
    try {
      ...
      const response = await deps.fetch(
        `${ingress.url.replace(/\/+$/, '')}/zed/refresh?restart=0`,
        {
          method: 'POST',
