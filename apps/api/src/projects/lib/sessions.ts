...
export async function buildSessionSandboxEnvVars(input: {
  ...
  return {
    ...runtimeSecrets.env,
    // Fleet default for the `zed-connectors` OpenCode MCP server. Set here
    ...
    // explicit contract.
    ...(config.CONNECTORS_MCP_ENABLED ? { ZED_CONNECTORS_MCP_ENABLED: '1' } : {}),
    ...channelEnv,
    ...sessionContextEnv,
...
/**
 * Liveness check for a quick-tunnel ZED_URL. The static loopback check
...
export async function sandboxCallbackDeadTunnelReason(
  ...
  try {
    ...
    if (!res.ok) {
      reason =
        `The dev tunnel at ${config.ZED_URL} answered ${res.status} to a health probe. ` +
    ...
    reason =
      `The dev tunnel at ${config.ZED_URL} is not answering. Cloud sandboxes cannot ` +
