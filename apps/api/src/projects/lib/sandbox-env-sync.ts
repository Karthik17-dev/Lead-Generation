 ...
 * `refreshModels: true` unconditionally. The daemon's `/zed/env` already
 ...
 *   - `snapshot.capabilitiesJson` — pushed as `ZED_SECRET_CAPABILITIES`,
 *     which is on the daemon's `RESPAWN_REQUIRED_ENV_NAMES` list.
 *   - the LLM-gateway triple (`enabled`, `baseUrl`, `denyEnv`) — the daemon
 *     maps these onto `ZED_LLM_API_KEY` / `ZED_LLM_BASE_URL` /
 *     `ZED_OPENCODE_DENY_ENV`, the model/token/key values the task calls
 *     out by name.
 *   - `args.opencodeEnv` — an explicit runtime-env push a caller asked this
 *     same call to carry (e.g. a channel follow-up's `ZED_CONNECTORS_MCP_ENABLED`,
