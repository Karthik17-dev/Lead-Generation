...
export function monitorOverCapError(cap = MONITOR_MAX_ENABLED): string {
  return `monitor disabled: this project already runs the maximum of ${cap} enabled monitors (selected in slug order). Disable another monitor in zed.yaml to make room.`;
}
...
/** One entry of the `ZED_MONITORS` payload the daemon reads at boot. */
