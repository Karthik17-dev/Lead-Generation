...
describe('refreshSandboxRuntimeAssets', () => {
  test('posts /zed/refresh?restart=0 with the sandbox service key', async () => {
    const d = deps();
    const outcome = await refreshSandboxRuntimeAssets('sess-1', d.deps);

    expect(outcome).toBe('refreshed');
    expect(d.calls.length).toBe(1);
    expect(d.calls[0].url).toBe('http://localhost:8008/v1/p/sbx-1/8000/zed/refresh?restart=0');
