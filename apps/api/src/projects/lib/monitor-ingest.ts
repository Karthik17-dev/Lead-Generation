 ...
 */
import { projectMonitorBoxes, projectMonitorEvents, projectTriggerRuntime } from '@zed/db';
  ...
  await db
    ...
    .set({
      ...
      ...(suppression.autoDisable
        ? {
            enabled: false,
            lastError: `monitor auto-disabled after ${suppression.suppressionCount} event-rate suppressions in 24h — reduce its output or raise its filter, then re-enable it in zed.yaml`,
            lastAttemptAt: now,
          }
        : {}),
    })
