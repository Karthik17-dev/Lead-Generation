 ...
 *  - **No session branch.** `ZED_BRANCH_NAME` is deliberately omitted, so
...
async function probeBoxWorkload(
  ...
  try {
    ...
    const res = await fetch(`${endpoint.url.replace(/\/$/, '')}/zed/health`, {
...
async function observeMonitorBox(
  ...
  if (status === 'running') {
    ...
    // env said ZED_WORKLOAD=monitor, health said opencode/session).
