...
import { projectMonitorBoxes, projectTriggerRuntime } from '@zed/db';
...
import { deriveZedApiBase, proxyGitUrl } from './sessions';
...
async function buildMonitorBoxEnv(input: {
  ...
  const envVars = {
    ...secrets.env,
    ZED_SANDBOX_TOKEN: input.sandboxToken,
    // Back-compat alias, same as a session gets.
    ZED_TOKEN: input.sandboxToken,
    ZED_WORKLOAD: 'monitor',
    ZED_MONITORS: buildMonitorEnvPayload(input.monitors),
    ZED_MONITOR_BOX_EPOCH: input.boxEpoch,
    ZED_PROJECT_ID: input.project.projectId,
    ZED_API_URL: deriveZedApiBase(),
    ZED_FRONTEND_URL: sandboxFrontendBaseUrl(),
    ZED_SERVICE_PORT: '8000',
    // Clone through the Zed git proxy, authenticated with the box's own
    ...
    ZED_REPO_URL: proxyGitUrl(input.project.projectId),
    ZED_DEFAULT_BRANCH: input.gitProject.defaultBranch,
    ZED_BASE_REF: input.gitProject.defaultBranch,
    ZED_PROJECT_AUTO_CLONE: '1',
    // No ZED_BRANCH_NAME on purpose — the daemon then leaves the checkout on
 ...
 * `zed triggers info` and the trigger list instead of surfacing as a
 ...
 * narrowing is visible in `zed triggers info` instead of failing silently.
 *
...
async function resolveMonitorBoxSecrets(
  ...
  // box's own runtime env (ZED_*, PATH, …).
