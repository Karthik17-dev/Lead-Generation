 ...
 *  hand-written `secrets:` list in zed.yaml may use any case, and all three
...
export function grantSecretToAgentV2(
  ...
  if (manifest.schemaVersion !== 2) {
    return {
      ...
      unsupportedV1: true,
      error:
        'This project uses a zed_version 1 manifest (zed.toml). Upgrade to zed_version 2 (zed.yaml) to grant a secret to an agent.',
    };
  }
  const rawAgents = manifest.raw.agents;
  if (
