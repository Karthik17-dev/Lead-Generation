...
# Version stamp baked into the binary (`zed --version`, `zed version`, and
...
# value it bakes as ZED_VERSION (apps/api/Dockerfile, stage `sandbox-cli`), so
...
define_args=()
if [ -n "${ZED_CLI_VERSION:-}" ]; then
  define_args+=(--define "process.env.ZED_CLI_VERSION=\"${ZED_CLI_VERSION}\"")
fi
...
compile_with_retry() {
  ...
  while true; do
    ...
    if bun build --compile --target="$target" ${define_args[@]+"${define_args[@]}"} --outfile=dist/zed src/index.ts; then
      return 0
    fi
...
printf '%s\n' "${ZED_CLI_VERSION:-dev}" > dist/zed.version
size="$(stat -f%z dist/zed 2>/dev/null || stat -c%s dist/zed)"
echo "Built dist/zed for ${target} (${size} bytes; version ${ZED_CLI_VERSION:-dev}; attestation ${attestation_final})"
