...
// session-scoped, carrying that agent's `zed_cli` grant from zed.yaml.
...
// (apps/zed-sandbox-agent-server/src/agent-env-file.ts). So a 403 like
 ...
 * `zed connectors` import closure — every file in that closure is hashed
...
export interface TokenIdentity {
  ...
  /** The agent's `zed_cli` grant: 'all', an explicit list, or null (ungated). */
  zedCli: string[] | 'all' | null;
  userId: string;
  userEmail: string;
}
...
/** A grant can change under a running session (a CR-merged zed.yaml, an
...
/** Same directory as the config + update-check cache (~/.config/zed), so a
 *  test that redirects ZED_CONFIG_FILE redirects this too. */
 ...
 *
 *   - The input is a machine-generated bearer token (`zed_pat_…`), never a
