/**
 * The Connector's data plane, shared by both faces of `zed connectors`:
 *   - the CLI subcommands (`zed connectors call …`)
 *   - the stdio MCP server (`zed connectors mcp`)
 * plus the `@zed/sdk` project client, which this module uses directly.
 *
 * Two project surfaces live here:
 *   1. `@zed/sdk`'s project Connector data plane — runs connector tool calls.
 *      It acts as the launching user via ZED_CLI_TOKEN. The gateway resolves
 ...
 *      (`ZED_CLI_TOKEN` + `ZED_PROJECT_ID`).
 */
import type { ConnectorCallResult, Zed } from '@zed/sdk';
 ...
 * identically:
 *   - in-sandbox: `ZED_CLI_TOKEN` + `ZED_API_URL` are
 *     injected and win;
 *   - on a laptop: falls back to the host you `zed login`'d.
 * The project comes from ZED_PROJECT_ID / `.zed/link.json` / `--project`.
 * When a project is known we hit the project-explicit gateway routes (which
 * accept a plain user token), so `zed connectors` is the SAME locally and in
 * the cloud. Without a project we fall back to the legacy flat routes, which
 * need a scoped session token (the in-sandbox case).
 */
export type ConnectorClient = Zed['connectors'];

export function connectorClient(projectOverride?: string): ConnectorClient {
  const auth = loadAuth();
  if (!auth?.token) {
    throw new CliError(
      'not authenticated — run `zed login` (or set ZED_CLI_TOKEN in a sandbox).',
      'MISSING_ENV',
    );
  }
  // --project > ZED_PROJECT_ID > .zed/link.json (resolveProjectId order).
  const projectId = resolveProjectId(projectOverride);
  const zed = zedFromAuth(auth);
  return projectId ? zed.project(projectId).connectors : zed.connectors;
}

/**
 * The project-scoped zed API client (NOT the gateway) — for connector
 * management + setup-link minting. Resolves the sandbox env-token host
 * (`activeHost()` in api/config.ts) + ZED_PROJECT_ID.
 */
export function connectorProjectContext(projectOverride?: string): { client: ApiClient; projectId: string } {
  const auth = loadAuth();
  if (!auth?.token) {
    throw new CliError(
      'not authenticated — ZED_CLI_TOKEN is missing.',
      'MISSING_ENV',
    );
  }
  const projectId = resolveProjectId(projectOverride);
  if (!projectId) throw new CliError('ZED_PROJECT_ID not set.', 'MISSING_ENV');
 ...
 * only the upstream response. Same route the `zed secrets call` CLI uses
...
/**
 * Add (or update) a connector on the project NOW — committed to zed.yaml on
...
/** Remove a connector from the project (zed.yaml on main + catalog). */
