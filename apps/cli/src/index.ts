...
async function main(argv: string[]): Promise<number> {
  ...
  // `zed` must ALWAYS land here — the interactive session picker is an
  // explicit verb (`zed connect`), never the front door.
  ...
  // Singular `session` is a permanent alias — `zed session new` is typed
