...
/**
 * Picker rows for `zed connect` with no session id: running sessions first
...
/**
 * Interactive session selection for `zed connect` when no id was given:
    ...
    if (stage === 'failed' || stage === 'stopped') {
      process.stderr.write(
        `${status.err(`Session did not start (${stage}${reason ? `: ${reason}` : ''}).`)}\n` +
          `  ${C.dim}Try ${C.reset}${C.cyan}zed sessions restart ${sessionId}${C.reset}${C.dim}, then ${C.reset}${C.cyan}zed connect${C.reset}${C.dim} again.${C.reset}\n`,
      );
      return false;
    }
  }
