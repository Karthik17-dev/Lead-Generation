...
async function configureConnections(env: SelfHostEnv, flags: GlobalFlags): Promise<void> {
  ...
  // Zed Apps (optional, default skip): Apps publish on a wildcard domain
  ...
  // operator is asked rather than surprised. Zed Cloud fronts Apps with a
  ...
  if (shouldPrompt(flags)) {
    const appsMode = await selectFrom(
      'Zed Apps hosting (optional, serves deployed Apps on their own domain): configure/skip',
      ['skip', 'configure'] as const,
      env.ZED_APPS_BASE_DOMAIN ? 'configure' : 'skip',
    );
    if (appsMode === 'configure') {
      env.ZED_APPS_BASE_DOMAIN = await prompt(
        'Apps base domain (needs a *.<domain> DNS record and certificate pointing at this instance)',
        env.ZED_APPS_BASE_DOMAIN,
      );
      env.ZED_APPS_ALLOW_DIRECT_EDGE = 'true';
    }
  }
...
function defaultEnv(flags: GlobalFlags): SelfHostEnv {
  ...
  return {
    ...
    // Zed Apps. Blank base domain = the API derives `apps.<its own domain>`.
    // ZED_APPS_ALLOW_DIRECT_EDGE tells the API that no Cloudflare Apps
    ...
    ZED_APPS_BASE_DOMAIN: '',
    ZED_APPS_ALLOW_DIRECT_EDGE: '',
