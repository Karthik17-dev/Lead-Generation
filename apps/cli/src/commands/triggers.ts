...
zed.yaml (the source of truth); \`zed ship\` applies them. ls/fire/info
 ...
 * `parseMonitorFields` and `@zed/manifest-schema`'s `validateMonitorTrigger`.
 * The CLI writes the manifest, so it must reject exactly what `zed ship`
