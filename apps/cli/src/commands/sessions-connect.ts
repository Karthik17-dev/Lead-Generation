...
the session's server runs (cached under ~/.zed/opencode/<version>/) so the
TUI and server never skew. Set ZED_OPENCODE_BIN to force your own binary.
...
async function runtimeOpencodeVersion(resolved: ResolvedSession): Promise<string | undefined> {
  try {
    const health = unwrapRuntime(
      await withZedScope(resolved.auth, () => resolved.runtime.global.health()),
    );
