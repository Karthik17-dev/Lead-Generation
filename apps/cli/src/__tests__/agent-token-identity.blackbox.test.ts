...
// Black-box reproduction of the in-sandbox agent case: the real `zed`
// process, a real HTTP API, a real `.zed/link.json`, and a real minted
// agent token in the environment.
//
// The original report — `zed sessions restart "$ZED_SESSION_ID"` from
// inside a session — printed:
//
//   host cloud (https://api.zed.com, not logged in) · account … · session …
...
const AGENT_TOKEN = 'zed_pat_minted_for_osp_vision_route_agent';

let tmp: string;
let server: ReturnType<typeof Bun.serve> | null = null;
let requests: string[] = [];

/** Stand-in for the Zed API: it knows the token's identity, and it refuses
...
async function runCli(args: string[]) {
  const proc = Bun.spawn({
    ...
    env: {
      ...
      ZED_CONFIG_FILE: join(tmp, 'config.json'),
      // What the platform injects into a running session.
      ZED_API_URL: `http://127.0.0.1:${server!.port}`,
      ZED_CLI_TOKEN: AGENT_TOKEN,
      ZED_PROJECT_ID: PROJECT_ID,
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zed-agent-identity-'));
  server = startApi();
...
test('a refused session read reports the CLI as authenticated and names the agent', async () => {
  ...
  expect(first.stderr).toContain('agents.osp-vision-route-agent.zed_cli');
...
test('the bare landing screen shows the agent row instead of a logged-out host', async () => {
  ...
  expect(landing.stdout).toContain('zed whoami --token-only');
});
