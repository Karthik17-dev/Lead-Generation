...
beforeEach(() => {
  for (const key of ['ZED_OPENCODE_BIN', 'ZED_OPENCODE_DIR']) {
    savedEnv[key] = process.env[key];
    delete process.env[key];
  }
  scratch = mkdtempSync(join(tmpdir(), 'opencode-bin-test-'));
  process.env.ZED_OPENCODE_DIR = join(scratch, 'managed');
});
...
describe('opencodePlatformPackage', () => {
  ...
  test('throws for unsupported platforms', () => {
    expect(() => opencodePlatformPackage('win32', 'x64')).toThrow(/ZED_OPENCODE_BIN/);
    expect(() => opencodePlatformPackage('linux', 'ia32')).toThrow(/ZED_OPENCODE_BIN/);
  });
});
...
describe('ensureOpencodeBin', () => {
  ...
  test('ZED_OPENCODE_BIN overrides everything', async () => {
    process.env.ZED_OPENCODE_BIN = '/custom/opencode';
  ...
  test('uses an already-managed binary without touching network or PATH', async () => {
    ...
    mkdirSync(join(process.env.ZED_OPENCODE_DIR!, '9.9.9'), { recursive: true });
  ...
  test('fails with install guidance when download fails and PATH has nothing', async () => {
    ...
    ).rejects.toThrow(/ZED_OPENCODE_BIN/);
  });
});
