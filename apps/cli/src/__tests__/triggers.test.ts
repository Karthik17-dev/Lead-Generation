...
/**
 * `zed triggers` as a real process, for the third trigger type
 * (docs/specs/2026-08-12-monitors.md). `add` edits the LOCAL zed.yaml, so
...
const MONITOR_TRIGGER = {
  ...
  path: 'zed.yaml#triggers.checkout-errors',
...
function writeManifest(): void {
  writeFileSync(
    join(tmp, 'zed.yaml'),
    ['zed_version: 2', 'name: monitors-test', 'default_agent: default', ''].join('\n'),
    'utf8',
  );
}

function manifestText(): string {
  return readFileSync(join(tmp, 'zed.yaml'), 'utf8');
}
...
async function runCli(args: string[], configFile: string = config) {
  const env: Record<string, string | undefined> = {
    ...
    // Always a throwaway config — never the developer's real ~/.zed.
...
describe('zed triggers — monitors', () => {
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'zed-triggers-'));
  ...
  test('add writes a poll monitor block to zed.yaml', async () => {
    ...
    expect(result.stdout).toContain('zed ship');
  });
