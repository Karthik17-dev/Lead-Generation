 ...
 * launcher got exit 2 and the `zed-connectors` MCP server never started —
...
async function runMcp(argv: string[], stdin: string) {
  const proc = Bun.spawn({
    ...
    env: {
      ...
      ZED_CLI_TOKEN: 'test-token-not-used-offline',
      ZED_API_URL: 'https://api.zed.invalid/v1',
...
describe('sandbox daemon -> CLI MCP contract', () => {
  test('the argv the daemon registers names a real command', () => {
    ...
    expect(argv[0]).toBe('/usr/local/bin/zed');
  ...
  test('that argv completes an MCP handshake', async () => {
    ...
    expect(response.result.serverInfo.name).toBe('zed-connectors');
  });
