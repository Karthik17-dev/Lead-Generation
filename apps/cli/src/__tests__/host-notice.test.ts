 ...
 *  never runs `zed login` and gets its credential from the environment. */
 ...
 *  workspace is (`.zed/link.json` is committed by `zed init`). */
function enterLinkedProject(host: string, accountId: string, projectId: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'zed-cli-linked-'));
...
function seedTokenIdentity(token: string, agent: string): void {
  const configFile = process.env.ZED_CONFIG_FILE!;
      ...
      entries: {
        [key]: {
          identity: {
            ...
            projectId: 'proj_123',
            sessionId: 'sess_123',
            zedCli: ['project.secret.read', 'project.secret.write'],
            userId: 'user_123',
...
describe('host notice', () => {
  ...
  // Regression: a sandbox workspace ALWAYS carries `.zed/link.json`, whose
  ...
  test('linked directory does not override sandbox env auth', () => {
    ...
    try {
      ...
      process.env.ZED_CLI_TOKEN = 'zed_pat_session';
      process.env.ZED_SESSION_ID = 'sess_ea985b87';
  ...
  test('names the agent a minted session token belongs to', () => {
    ...
    try {
      ...
      process.env.ZED_CLI_TOKEN = 'zed_pat_session';
      process.env.ZED_SESSION_ID = 'sess_ea985b87';
      seedTokenIdentity('zed_pat_session', 'osp-vision-route-agent');

      expect(renderHostNotice(['sessions', 'restart', 'x'])).toContain(
        'agent osp-vision-route-agent',
      );
      // The bare-`zed` hierarchy names it too, on its own row.
