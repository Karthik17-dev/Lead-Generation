...
/**
 * Resolves the `opencode` binary that `zed connect` hands the terminal to.
 *
 ...
 * the sandbox pin lives in @zed/shared runtime-versions.json. So instead of
 * requiring users to install and version-manage OpenCode themselves, the CLI
 * keeps per-version managed binaries under ~/.zed/opencode/<version>/ and
...
function managedRoot(): string {
  return process.env.ZED_OPENCODE_DIR || join(homedir(), '.zed', 'opencode');
}
  ...
  if (!os || (arch !== 'arm64' && arch !== 'x64')) {
    throw new Error(
      `No prebuilt OpenCode binary for ${platform}/${arch}. ` +
        'Install OpenCode yourself and set ZED_OPENCODE_BIN.',
    );
  }
...
/**
 * Resolution order:
 *   1. `ZED_OPENCODE_BIN` — explicit user override, used as-is.
...
export async function ensureOpencodeBin(
  ...
  const envBin = process.env.ZED_OPENCODE_BIN;
  ...
  try {
    ...
    throw new Error(
      `Could not download OpenCode v${version}: ${reason}. ` +
        'Install OpenCode (https://opencode.ai) or set ZED_OPENCODE_BIN.',
    );
  }
}
...
async function downloadOpencode(
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'zed-opencode-'));
  try {
    ...
    // Stage in the destination directory, then rename: a concurrent `zed`
