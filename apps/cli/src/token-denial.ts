...
// `zed whoami --token-only`.
//
// So after a 401/403 the CLI answers it unprompted: the token kind, the agent,
// and that agent's `zed_cli` grant — which is exactly the list a manifest
...
export async function printPermissionDenialIdentity(): Promise<void> {
  ...
  if (identity.agent) {
    lines.push(`  ${C.dim}granted   ${C.reset}${formatGrantList(identity.zedCli)}`);
    lines.push(
      `  ${C.dim}fix       ${C.reset}add the action to ` +
        `${C.cyan}agents.${identity.agent}.zed_cli${C.reset}${C.dim} in zed.yaml, then merge${C.reset}`,
    );
