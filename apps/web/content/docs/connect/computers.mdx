...
description: Connect your machine through the permissioned Zed Agent Tunnel.
---

A computer is your own machine — laptop, desktop, or server — connected to
Zed through a permissioned reverse tunnel. A computer is not a sandbox. A
sandbox is a disposable cloud machine that Zed creates for a
...
2. Run the pairing command shown in the profile, for example
   `npx --yes @zed/agent-tunnel@latest connect --api-url <url>`.
...
`zed.yaml`. Use the profile's Accounts tab to pair, select, inspect, rename,
