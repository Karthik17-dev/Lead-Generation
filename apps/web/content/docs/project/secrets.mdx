---
title: Secrets
description: How Zed stores project credentials and controls where each value is allowed to go.
---
...
Zed stores secrets on the project, never on the account, and encrypts
...
- **Shared** — the project-wide value. Every project member with read
  access sees every shared secret. Zed has no per-member sharing control
  for a single secret.
- **Personal override** — your own value for one name, used instead of the
  shared row for sessions you start. Today Zed uses this only for one
  OAuth login credential.

Zed never lets an LLM provider key, for example `ANTHROPIC_API_KEY`,
become a personal override. The model gateway always reads the shared row.

An agent only receives the secrets its manifest grants. In `zed.yaml`,
...
| **Zed service** | Nothing | The LLM gateway, a connector, Git, or one policy-bound HTTPS request Zed makes for you |
...
```bash
zed secrets delivery STRIPE_API_KEY denied
```
...
in `zed.yaml` never receives one. The Secrets page can write that grant for
...
```bash
zed secrets set STRIPE_API_KEY=sk_live_...
```

Zed saves it as a shared secret for the project. To store more than one
value under the same name, add `--identifier <id>`. Names can't start with
`ZED_` — Zed reserves that prefix for platform values.
</Step>

<Step>
### Or use the dashboard

Open the project's Secrets page, enter the key and value, and save. Zed
...
`secrets` list in `zed.yaml` yourself. A session only receives the secrets
...
this secret**. Choose an agent there and confirm. Zed edits `zed.yaml` and
...
admits the identifier needs no commit, and Zed makes none.

An agent on `secrets: all` is a special case. `all` cannot carry a
session-scoped secret, so Zed writes an explicit list: every identifier the
...
- A project on `zed_version: 1` (`zed.toml`) has no agents map to edit.
  The request fails with `400` and `manifest_v1_unsupported`. Edit the manifest
  by hand, or move the project to `zed_version: 2`.
...
A project with no `agents:` block — or with no `zed.yaml` at all — is
...
Run `zed secrets ls` to see which secrets a project declares and which
...
```bash
zed secrets set STRIPE_API_KEY=sk_live_new...
```
</Step>

<Step>
### Zed pushes it to running sessions

Zed pushes the new value to every sandbox with an active session for the
project, on a best-effort basis. For model or gateway credentials, Zed
...
Run `zed secrets unset STRIPE_API_KEY` (or `unset <identifier>`), or
delete it from the project's Secrets page.

<Callout type="warn" title="Removal is immediate, propagation is not">
Zed deletes a shared secret right away. Push to already-running
sandboxes is best-effort, the same as rotation.
</Callout>

## Share a value without seeing it

Run `zed secrets request STRIPE_API_KEY` to create a link. Anyone with the
...
Network boundary is the strictest delivery mode. Zed registers the value
...
no `agents:` block in `zed.yaml` delivers nothing here until one exists.
...
```bash
zed secrets delivery STRIPE_API_KEY egress \
...
to the same secret, or use a second secret. Zed rejects an unenforceable
policy with `400` when you save it — it never stores a rule it cannot apply.

Use Zed service delivery with the HTTPS broker consumer when you need
wildcards, paths, or method filters. Zed makes that request itself, so it can
...
|---|---|
| `zed secrets ls` | List secrets declared and set for the project |
| `zed secrets set KEY=VALUE [--identifier <id>]` | Create or update a secret. `KEY=-` reads the value from stdin |
| `zed secrets unset IDENTIFIER` | Remove a secret |
| `zed secrets delivery IDENTIFIER runtime\|broker\|egress\|denied` | Set the delivery mode and its policy |
| `zed secrets call IDENTIFIER URL` | Send one HTTPS broker request |
| `zed secrets sync` | Re-push project secrets to this session's sandbox |
| `zed secrets request NAME [--scope runtime\|connector] [--expires <min>]` | Create a link so someone else can enter a value |
| `zed env push --from <path>` | Upload a `.env` file as secrets |
| `zed env pull [--out <path>] [--force]` | Export secret names, not values, to a `.env` file |

Run `zed secrets --help` for every delivery flag.
...
| POST | `/secrets/{identifier}/grant` | Add the identifier to one agent's `secrets` list in `zed.yaml`. Body: `{agent}`. |
...
`POST /secrets` rejects names that start with `ZED_`. It returns `409`
if the `identifier` already exists with a different `name`. It rejects the
exact name `CODEX_AUTH_JSON` with `400` — Zed manages that secret
...
granted, the mode needs no grant, or Zed could not read the manifest.
...
which case Zed commits nothing. It returns `adopted_governance: true` when
...
for a `zed.toml` project and `409` `secret_not_grantable` for a disabled
secret.

## Rotation and propagation

A secret write does not wait for a session restart. Zed pushes the
change to every active sandbox in the project:

1. Zed builds a new environment snapshot, using the running agent's
   `secrets` grant.
2. The sandbox writes the snapshot to the live agent environment. New tool
   calls pick up the change right away.
3. If the changed secret is an LLM provider credential, Zed restarts
   OpenCode.
...
A project on Zed's managed model access needs no key of its own. To bring
