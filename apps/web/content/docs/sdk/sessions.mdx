...
`zed.session(projectId, sessionId)` returns the handle for everything a
...
- `s.voiceTranscript(options?)` — this session's live voice-call transcript (spoken turns plus `ask_zed`/`run_command` worker tool calls). Returns an empty list when the session has no live call, not a 404.
...
OpenCode session in the host. Zed creates and persists the root session.
...
agent persisted on the Zed session. This prevents a snapshot-inherited
...
3. The persisted Zed session default.
...
`s.stream()` is the OpenCode REST compatibility event stream. The Zed API
...
`zed.session(projectId, sessionId)`. The SDK resolves it during
...
await zed.session(projectId, warm.session.session_id).send("Build me a widget");
```
...
Every call through `createZed` rejects with a typed `Error` subclass,
never a plain object. Catch the error, check `instanceof`, and branch on
`.status` or `.code`.

```ts
import { ApiError, BillingError } from '@zed/sdk';
...
| `AuthError`            | `ApiError` | `getToken` returned `null`. Zed never sent the request                      | `code` is always `'NO_SESSION'`                                      |
...
Zed retries some requests before your code sees an error. If a `GET` or `HEAD` request returns `502`, `503`, or `504`, Zed retries it up to 2 times, with a 250ms then 500ms delay. A transient transport failure on a `GET` or `HEAD` — a network error, not a status code — is retried the same way. A retry that succeeds never reaches `onError`. Zed never retries `POST`, `PUT`, `PATCH`, or `DELETE` requests, or a `500` response.

Zed throws `AuthError` on the client, before it sends a request, when `getToken()` returns `null`. `AuthError` extends `ApiError`, so `err instanceof ApiError` still matches. Check `err instanceof AuthError`, or `err.code === 'NO_SESSION'`, to treat "not signed in" as a separate case from a backend failure.

Zed throws `BillingError` for every HTTP `402` response: out of credits, over a plan limit, or another billing gate. `detail.message` holds the reason from the backend.

Zed throws `RequestTooLargeError` for HTTP `431`. This usually means the request carried too many files. `detail.suggestion` holds a ready-to-show hint for the user.
...
```ts
import { SessionNotReadyError } from '@zed/sdk';
...
```ts
import { formatBillingErrorForUI } from '@zed/sdk';

try {
  await zed.session(projectId, sessionId).start();
} catch (err) {
...
`@zed/sdk/react` re-exports `BillingError`, `RequestTooLargeError`, `parseBillingError`, `isBillingError`, and `formatBillingErrorForUI`. It does not re-export `ApiError` or `AuthError` — import those from `@zed/sdk`.
