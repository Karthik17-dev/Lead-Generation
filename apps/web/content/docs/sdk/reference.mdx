---
title: SDK reference
description: The full @zed/sdk API surface — client methods, modules, turns, and distribution.
---

This page is the full `@zed/sdk` API surface: every client method, the
...
`createZed(config)` returns one client. Every method is a typed call to
...
zed.accounts; // account / team operations
zed.accountInvites; // invite accept/decline by token alone
zed.projects; // top-level project operations
zed.connectors; // Connector calls scoped by an agent-minted token
zed.project(id); // id-bound project handle
zed.session(pid, sid); // id-bound session handle → see Sessions
zed.github; // GitHub App install + repo linking
zed.billing; // credits, subscription, tier, transactions
zed.sandboxShares; // public share links for a sandbox port
zed.transcribe; // speech-to-text
zed.connectStatus; // easy-connect (Pipedream) status
zed.marketplace; // public marketplace catalog
zed.validateToken; // pasted-API-key check
zed.config; // platform config in effect
zed.runtime(); // OpenCode REST compatibility client
```
...
`accounts.tokens` mints account-scoped API keys (`zed_pat_...`). See
...
### Account invites — `zed.accountInvites`
...
| `provision(input)` | new project on a new Zed-managed repo, seeded with a starter template — `{ name, account_id?, seed_starter?, starter_template?, marketplace_items?, source_item_id?, idempotency_key? }` |
...
### Sandbox shares — `zed.sandboxShares`
...
### Marketplace catalog — `zed.marketplace`
...
### The project handle — `zed.project(id)`

Binds the project id; every sub-resource hangs off it.

```ts
const p = zed.project(projectId);
await p.detail();
await p.update({ name });
await p.llmCatalog();
```
...
| `validateManifest(raw)` | validate a `zed.yaml` (or legacy `zed.toml`) manifest server-side |
...
Auto-minted at session create as `ZED_CLI_TOKEN`; can also be minted by hand.
...
| `session(sid)` | the session handle (same as `zed.session(id, sid)`) |
...
then sets it as `default_agent` in the project's `zed.yaml`. New sessions
...
`feature` is one of `FEATURE_FLAG_KEYS` (exported from `@zed/sdk`, typed as
...
`zed.runtime()` returns the typed OpenCode REST client for the active
sandbox.
On a client created by `createScopedZed` (`@zed/sdk/server`), it **throws**
...
`zed.session(pid, sid).runtime` (call `ensureReady()` first) instead, which
...
| Tier | Meaning |
| --- | --- |
| Canonical | Import from the root `@zed/sdk`. Use this for all new code. |
| Supported | A dedicated subpath (`@zed/sdk/react`, `@zed/sdk/server`). First-class, not deprecated. |
...
straight from `@zed/sdk`:

```ts
import { files, getSessionHealth, getClient, authenticatedFetch, backendApi } from '@zed/sdk';
```
...
```ts
import { getSessionHealth, isRuntimeReady } from '@zed/sdk';
...
with auth already injected. Prefer `zed.session(pid, sid).runtime`, the
...
```ts
import { authenticatedFetch, getAuthToken } from '@zed/sdk';

const res = await authenticatedFetch(`${runtimeUrl}/zed/health`);
const token = await getAuthToken();
```

The token comes from the `getToken` function you passed to `createZed`.
...
| --- | --- |
| `@zed/sdk/react` | React hooks — see [React hooks](/docs/sdk/react) |
| `@zed/sdk/server` | Request-scoped config for multi-tenant backends |

#### Server-side isolation

`createZed` stores its config, including the token function, in one
...
requests for different users, because the last `createZed` call wins for
every in-flight request. `@zed/sdk/server` fixes this with per-request
isolation:
...
export async function handler(req: Request) {
  ...
  return zed.projects.list();
}
```

`createScopedZed` and `runWithZed` isolate config per request with
Node's `AsyncLocalStorage`. Never import `@zed/sdk/server` from a browser
...
`zed.session(pid, sid).runtime` after `await s.ensureReady()`.
...
root `@zed/sdk` entry already exports. They stay working so no existing
import breaks, but new code should import from the root.

### Internal modules

`@zed/sdk/internal/*` holds the zustand stores apps/web's own runtime uses
...
```ts
import { classifyPart, classifyTurn, toolInfo, toolViewModel } from '@zed/sdk';
```

Import every function from the root `@zed/sdk` entry. The
`@zed/sdk/turns` subpath still works, but it is a deprecated alias. New
...
```ts
import { classifyPart, type ClassifiedPart } from '@zed/sdk';
...
Both functions match tool-name prefixes, so Zed's plugin tool families
...
```ts
import { groupMessagesIntoTurns, collectTurnParts, type TurnLike } from '@zed/sdk';
...
```ts
import { isTextPart, isToolPart, getPartText } from '@zed/sdk';
...
```ts
import { getWorkingState, getTurnStatus, formatDuration } from '@zed/sdk';
...
```ts
import { getTurnError, getChildSessionError, unwrapError } from '@zed/sdk';
...
```ts
import { getTurnCost, getSessionCost, formatCost, formatTokens, COST_MARKUP } from '@zed/sdk';
...
```ts
import { getChildSessionId, getChildSessionToolParts } from '@zed/sdk';
...
```ts
import { getPermissionForTool, getHiddenToolParts, isToolPartHidden } from '@zed/sdk';
...
```ts
import { getFilename, getDirectory, relativizePath, stripAnsi } from '@zed/sdk';
...
```ts
import { sortSessions, childMapByParent, allDescendantIds } from '@zed/sdk';
...
use `@zed/sdk/react`, install both.

```ts
import { createZed } from '@zed/sdk'; // framework-free core
import { useSession } from '@zed/sdk/react'; // optional React layer
import { createScopedZed } from '@zed/sdk/server'; // Node and Bun servers
```

### Entry points and stability

The root entry, `@zed/sdk`, is canonical. It exports the full
framework-free surface and runs in browsers, Node 18+, Bun, and edge
runtimes.

| Entry | Tier | Contract |
| --- | --- | --- |
| `@zed/sdk` | Canonical | Framework-free. Never imports React or `node:*`. |
| `@zed/sdk/react` | Supported | React hooks. The only entry that imports React. |
| `@zed/sdk/server` | Supported | Per-request config isolation for Node and Bun servers, via `node:async_hooks`. Never bundle it into a browser. |
...
| `@zed/sdk/internal/*` | Internal | Used by the Zed web app only. Not a supported API. |

### CDN bundles

The package also ships two browser bundles, built by `tsup`: an ESM bundle
(`dist/zed.esm.min.js`) and an IIFE global (`dist/zed.global.js`) that
defines `window.Zed`.

```html
<script src="https://unpkg.com/@zed/sdk/dist/zed.global.js"></script>
<script>
  const zed = Zed.createZed({
...
`window.Zed` exposes the same root entry as the npm import:
`Zed.createZed`, `Zed.classifyTurn`, `Zed.ApiError`.
