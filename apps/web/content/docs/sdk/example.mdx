...
```ts
import { ApiError, classifyTurn, createZed, narrowChatEvent } from '@zed/sdk';
import type { MessageWithParts } from '@zed/sdk';

async function main() {
  // 1. One client, one auth seam. getToken returns your API key
  //    (zed_pat_…) or a logged-in user's Supabase JWT — nothing else.
  const zed = createZed({
  ...
  const project = projects[0] ?? (await zed.projects.provision({ name: 'sdk-quickstart' }));
  console.log(`using project ${project.name} (${project.project_id})`);

  // 3. Create a session — a cheap platform call. No sandbox exists yet.
  const created = await zed.projects.createSession(project.project_id, {
    name: 'sdk full example',
  });
  const session = zed.session(project.project_id, created.session_id);
  ...
  await session.send('What files are in this repo?', {
    model: { providerID: 'zed', modelID: 'glm-5.2' },
  });
...
```sh
ZED_API_KEY=zed_pat_... npx tsx full-example.ts
```
...
// Managed provider credentials stay on the Zed LLM gateway.
await project.secrets.upsert({
...
const { config } = await zed.projects.detail(projectId);
console.log(
...
wrapper, a Zed-as-a-Backend multi-tenant wrapper, transcript rendering, and
files and secrets.
