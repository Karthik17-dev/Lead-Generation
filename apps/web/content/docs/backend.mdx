---
title: Zed as a Backend
description: Start and manage Zed sessions from your backend with explicit connector, model, context, and secret scope.
---

import { Steps, Step } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

Use a Zed API key to start sessions from your server. Each session has one
Zed owner, one project, and one cost record.
...
Create a personal access token (`zed_pat_…`) or service-account credential
(`zed_sa_…`). Both authenticate a programmatic session-create request. The
API derives `origin: "backend"` from the credential type.

```bash
export ZED_API_URL="https://your-zed-deployment.com/v1"
...
```bash
curl -X POST "$ZED_API_URL/projects/$ZED_PROJECT_ID/sessions" \
...
Use `createScopedZed` when one server process handles concurrent requests.
...
connection. Use `project` connectors for service-account sessions.
...
```ts
const connection = await zed.project(projectId).connectors.connections.reconcile({
...
await zed.project(projectId).connectors.connections.activate(connection.connection_id);
```
...
```ts
const scope = await zed.session(projectId, sessionId).scope();
```

Replace scope with `PUT` or `rescope()`:

```ts
const nextScope = await zed.session(projectId, sessionId).rescope({
  secrets: ['STRIPE_KEY'],
  connector_bindings: {
...
- Session scope replacement cannot select a connection owned by another
  member.
- Store application customer metadata outside Zed.
