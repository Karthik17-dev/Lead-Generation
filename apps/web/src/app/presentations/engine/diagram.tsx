 ...
 * shows the credential resolved on the Zed side of the wall and never drawn
...
const TONE_RING: Record<Tone, string> = {
  ...
  ok: 'border-zed-green/50',
  warn: 'border-zed-orange/50',
  danger: 'border-zed-red/50',
};
...
/* ── 2 · the credential broker ──────────────────────────────────────────────
   The centrepiece. Steps: 0 the shape · 1 the agent asks · 2 Zed resolves ·
   3 the API answers · 4 what never crossed. Grounded in connectors/content.ts
   `broker`: the sandbox carries one ZED_CLI_TOKEN and zero third-party
   secrets; the credential is attached server-side at call time. */

export function BrokerDiagram({ step }: { step: number }) {
  const captions = [
    'A sandbox is a real Linux machine the model can run anything on. So the only credential in it is one Zed token, scoped to the project.',
    'The agent calls a tool by name. It has no URL, no host and no key — it cannot construct the request itself.',
    'Zed checks that this agent may use this connector, applies the policy, and decrypts the credential on its own side of the wall.',
  ...
  return (
    <Stage caption={captions[Math.min(step, captions.length - 1)]}>
      <Row className="gap-0">
        ...
        >
          <div className="mt-3 flex flex-col gap-1.5">
            <Chip tone={step >= 1 ? 'active' : 'idle'}>ZED_CLI_TOKEN</Chip>
  ...
  return (
    <m.span
      ...
      className={cn(
        ...
        tone === 'ok'
          ? 'border-zed-green text-zed-green'
      ...
      <div className="flex flex-col gap-8 py-2">
        ...
        <Reveal on={step >= 3}>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="border-zed-green/50 bg-background rounded-sm border p-4">
              <p className="text-muted-foreground font-mono text-[10px] tracking-widest uppercase">
                a person with project.gitops.merge
              </p>
              <p className="text-zed-green mt-2 text-sm font-medium">may merge</p>
            </div>
            <div className="border-zed-red/50 bg-background rounded-sm border p-4">
              <p className="text-muted-foreground font-mono text-[10px] tracking-widest uppercase">
                every agent, by default
              </p>
              <p className="text-zed-red mt-2 text-sm font-medium">
  ...
  return (
    <Stage caption={captions[Math.min(step, captions.length - 1)]}>
      <div className="flex flex-col gap-3">
        <Row className="gap-0">
          <Box label="principal" title="Person" mono="marko@zed.com" tone="active" className="max-w-[13rem]" />
        ...
        <Reveal on={step >= 2} className="relative flex items-center justify-center py-1">
          <span aria-hidden className="border-zed-red/40 absolute inset-x-0 top-1/2 border-t border-dashed" />
          <span className="border-zed-red/50 bg-card text-zed-red relative rounded-sm border px-2.5 py-1 font-mono text-[10px] tracking-widest uppercase line-through">
...
const LEDGER = [
  ...
  { action: 'iam.role.grant', actor: 'marko@zed.com', risk: 'admin', outcome: 'ran', tone: 'active' as Tone },
  { action: 'cr.merge', actor: 'marko@zed.com', risk: 'write', outcome: 'ran', tone: 'active' as Tone },
];
