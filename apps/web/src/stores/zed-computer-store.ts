...
interface ZedComputerState {
  ...
  // Clear pending tool nav after ZedComputer processes it
...
export const useZedComputerStore = create<ZedComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      focusToolCall: (callId: string) => {
        ...
        const update: Partial<ZedComputerState> = {
      ...
      setIsSidePanelOpen: (open: boolean) => {
        const sessionId = get()._activeSessionId;
        const update: Partial<ZedComputerState> = { isSidePanelOpen: open };
        // Only clear THIS session's own announcement — session B opening its
        // panel must not destroy session A's unseen ready chip.
      ...
      setIsActionPanelOpen: (open: boolean) => {
        const sessionId = get()._activeSessionId;
        const update: Partial<ZedComputerState> = { isActionPanelOpen: open };
      ...
      openSidePanel: () => {
        const sessionId = get()._activeSessionId;
        const update: Partial<ZedComputerState> = { isSidePanelOpen: true };
        // Only clear THIS session's own announcement — session B opening its
        // panel must not destroy session A's unseen ready chip.
        if (get().readyChip?.sessionId === sessionId) update.readyChip = null;
        set(update);
      ...
      reset: () => {
        console.log('[ZedComputerStore] Full reset');
        useFilesStore.getState().reset();
        set(initialState);
      },
    }),
    {
      name: 'zed-computer-store',
      /**
       ...
       * opens.
       */
      partialize: (state: ZedComputerState) => ({ activeView: state.activeView }),
    },
  ),
);
...
// Main view state
export const useZedComputerActiveView = () =>
  useZedComputerStore((state) => state.activeView);

// Individual selectors for pending tool navigation (stable primitives)
export const useZedComputerPendingToolNavIndex = () =>
  useZedComputerStore((state) => state.pendingToolNavIndex);

export const useZedComputerClearPendingToolNav = () =>
  useZedComputerStore((state) => state.clearPendingToolNav);

// Side-panel Actions focus (clicking a tool call in chat)
export const useFocusedToolCallId = () =>
  useZedComputerStore((state) => state.focusedToolCallId);

export const useClearFocusedToolCall = () =>
  useZedComputerStore((state) => state.clearFocusedToolCall);

// Side panel state selectors
export const useIsSidePanelOpen = () => useZedComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useZedComputerStore((state) => state.setIsSidePanelOpen);
...
export const useIsActionPanelOpen = () =>
  useZedComputerStore((state) => state.isActionPanelOpen);

export const useToggleActionPanel = () =>
  useZedComputerStore((state) => state.toggleActionPanel);

/** ⌘I / Ctrl+I — the single right-side toggle. See `toggleRightPanel`. */
export const useToggleRightPanel = () =>
  useZedComputerStore((state) => state.toggleRightPanel);

export const useIsExpanded = () => useZedComputerStore((state) => state.isExpanded);

export const useToggleExpanded = () => useZedComputerStore((state) => state.toggleExpanded);

// Ready chip state selectors
export const useReadyChip = () => useZedComputerStore((state) => state.readyChip);

export const useClearReadyChip = () => useZedComputerStore((state) => state.clearReadyChip);
