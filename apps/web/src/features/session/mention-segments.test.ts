...
describe('buildMentionSegments — regex detection', () => {
  ...
  test('an email address is not a mention', () => {
    // A bare /@(\S+)/ matched mid-token, so `jay@zed.com` rendered
    // `@zed.com` as a file chip pointing at a path that does not exist.
    expect(buildMentionSegments({ text: 'mail me at jay@zed.com' })).toEqual([
      { text: 'mail me at jay@zed.com' },
    ]);
  });
