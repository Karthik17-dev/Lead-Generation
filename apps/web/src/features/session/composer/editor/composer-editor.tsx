...
import type { Agent, Command, Session } from '@zed/sdk/react';
  ...
  function ComposerEditor(
    ...
    const editor = useEditor({
      ...
      editorProps: {
        attributes: {
           ...
           */
          class:
            'outline-none min-h-[24px] max-h-[180px] overflow-y-auto px-0.5 pt-1.5 pb-1.5 text-base sm:text-sm',
        },
        handleKeyDown,
      },
    ...
    return (
      <EditorContent
        editor={editor}
        className={cn(
          'zed-composer-editor w-full text-[15px] sm:text-[14px]',
          disabled && 'opacity-50',
        )}
      />
    );
