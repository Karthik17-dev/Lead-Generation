  ...
  return (
    <div
      className={cn(
        ...
        pendingCount > 0 ? 'border-zed-orange/25' : 'border-border',
      )}
    >
      <div
        className={cn(
          'flex items-center gap-2 border-b px-3 py-2',
          pendingCount > 0 ? 'border-zed-orange/20' : 'border-border',
        )}
      >
        {pendingCount > 0 ? (
          <ShieldAlert className="text-zed-orange size-4" />
        ) : (
          <CheckCircleIcon weight="fill" className="text-zed-green size-4" />
        )}
