import { DEFAULT_MANAGED_MODEL_IDS, PROVIDER_LABELS } from '@zed/llm-catalog';

import type { FlatModel } from './session-chat-input';

const MANAGED_MODEL_IDS = new Set<string>(DEFAULT_MANAGED_MODEL_IDS);

// The gateway exposes its whole catalog through a single `zed` provider, with
// model ids namespaced as `<provider>/<model>`. For the picker we recover the
// REAL provider: platform-managed defaults stay under the "Zed" group, while
// every BYOK model surfaces under its real provider ("Anthropic", "OpenAI", …) —
// so a connected provider reads as its own section, not buried in Zed.
//
// *** BUG THIS FIXES (every model showing under "Zed", even BYOK Anthropic) ***
...
// which is ALWAYS "Zed" — every gateway model is registered under the one
// synthetic `zed` opencode provider). So the group's icon rendered
// correctly (`ProviderLogo` is keyed off the correct `providerID`), but the
// text label next to it always read "Zed" regardless of which provider
...
// (always "Zed" under the gateway, see the bug note above). Prefer the
