...
function action(overrides: Partial<SessionAuditAction> = {}): SessionAuditAction {
  return {
    ...
    risk: 'write',
    acted_by: 'user-1',
    acted_by_email: 'marko@zed.ai',
    resolved_by: null,
    resolved_by_email: null,
    result_summary: {
      args_preview: { subject: 'Weekly report', to: ['marko@zed.ai'] },
      args_preview_complete: true,
    },
    at: '2026-08-09T10:00:00.000Z',
    resolved_at: null,
    approval_url: 'https://dev.zed.com/approve/tok-1',
    ...overrides,
  };
}
...
describe('approvalArgsSummary', () => {
  test('puts the target fields first', () => {
    expect(approvalArgsSummary(action())).toBe('to: marko@zed.ai · subject: Weekly report');
  });
...
describe('approvalRequestFromAction', () => {
  test('maps a pending row to the shared request shape', () => {
    expect(approvalRequestFromAction(action())).toEqual({
      ...
      argsPreview: { subject: 'Weekly report', to: ['marko@zed.ai'] },
      reviewComplete: true,
      resolution: null,
      pending: true,
      status: 'pending_approval',
