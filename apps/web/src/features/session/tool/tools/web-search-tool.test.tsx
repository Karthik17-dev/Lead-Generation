...
describe('WebSearchTool', () => {
  test('renders every source as a flat row — no disclosure groups', () => {
    const html = render(
      <WebSearchTool
        part={completedSearchPart([
          ...
          { title: 'Zed founder', url: 'https://markokraemer.com' },
    ...
    expect(html).toContain('Zed founder');
  ...
  test('two sources on the same domain render as two flat rows, never a "N results" domain group', () => {
    const html = render(
      <WebSearchTool
        part={completedSearchPart([
          { title: 'Zed SDK repo', url: 'https://github.com/zed-ai/sdk' },
          { title: 'Suna repo', url: 'https://github.com/karthikxa/AI-Management-System' },
        ])}
        defaultOpen
      />,
    );
    expect(html).toContain('Zed SDK repo');
