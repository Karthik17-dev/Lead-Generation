 ...
 * `model.providerID`: under the gateway every model is registered as `zed`,
 * so the model's own id would paint the Zed mark on every row.
 */
function ModelRow({
  model,
  groupProviderID,
                  ...
                  {grouped.map((group, groupIndex) => (
                    <Fragment key={group.providerID}>
                      ...
                      {/* A provider heading only earns its row when there is a
                          ...
                          common gateway case — everything is "Zed") the
