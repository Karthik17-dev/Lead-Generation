...
export function buildAgentGitReconciliationPrompt(baseRef: string | null | undefined): string {
  ...
  const target = safeBaseRef
    ? `\`origin/${safeBaseRef}\``
    : 'the remote base branch named by `ZED_BASE_REF` (fall back to `ZED_DEFAULT_BRANCH` only if it is unset)';
...
\`zed sessions reload "$ZED_SESSION_ID" --project "$ZED_PROJECT_ID" --no-repo --force --yes\`
