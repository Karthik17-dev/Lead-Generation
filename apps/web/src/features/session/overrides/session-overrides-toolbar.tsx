...
import type { SessionScope } from '@zed/sdk';
...
export interface SessionOverridesToolbarProps {
   ...
   * default to what this agent's `zed.yaml` allows. Not a rendered row:
...
export function SessionOverridesToolbar({
  ...
  const rows = useMemo(() => {
    ...
    list.push({
      ...
      description:
        'Which secrets reach this session. The default is the agent’s grant — everything its zed.yaml allows — and narrowing it here only ever takes access away.',
