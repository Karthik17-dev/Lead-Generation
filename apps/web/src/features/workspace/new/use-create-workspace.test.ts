...
const OWNER_ACCOUNT: ZedAccount = { account_id: 'acct-owner', name: 'Owner Co', account_role: 'owner' };

function fakeProject(id: string, accountId = 'acct-owner'): ZedProject {
  return {
    project_id: id,
...
describe('fingerprintOf', () => {
  ...
  test('changes when the name changes', () => {
    ...
    const b = fingerprintOf({ ...INITIAL_FORM_STATE, name: 'zed-api' });
    expect(a).not.toBe(b);
  });
...
describe('runCreate: the full create() orchestration', () => {
  ...
  test('on success, writes the last-project cookie with the current user id and primes the cache for the account actually used', async () => {
    ...
    const primeCalls: Array<[string, ZedProject]> = [];
