...
const owner: ZedAccount = { account_id: 'a1', name: 'Owner Co', account_role: 'owner' };
const admin: ZedAccount = { account_id: 'a2', name: 'Admin Co', account_role: 'admin' };
const member: ZedAccount = { account_id: 'a3', name: 'Member Co', account_role: 'member' };
// `account_role` is optional on `ZedAccount` — this account has none at all.
const roleless: ZedAccount = { account_id: 'a4', name: 'No Role Co' };

describe('INITIAL_FORM_STATE', () => {
  test('defaults to a Zed-managed repo on main, with no icon', () => {
...
describe('resolveDefaultCreatableAccountId', () => {
  const team: ZedAccount = {
    account_id: 'a-team',
    name: 'Acme',
    account_role: 'owner',
  };
  const personal: ZedAccount = {
  ...
  test('returns null when there are no creatable accounts', () => {
    expect(resolveDefaultCreatableAccountId([], 'jay@zed.ai')).toBeNull();
  });

  test('prefers the account whose name matches the signed-in email (case-insensitive)', () => {
    expect(resolveDefaultCreatableAccountId([team, personal], 'Jay@Zed.ai')).toBe(
      'a-personal',
    );
  });

  test('falls back to slug match against the email or its local-part', () => {
    const bySlug: ZedAccount = {
      account_id: 'a-slug',
      name: 'Personal',
      slug: 'jay@zed.ai',
      account_role: 'owner',
    };
    expect(resolveDefaultCreatableAccountId([team, bySlug], 'jay@zed.ai')).toBe('a-slug');

    const byLocal: ZedAccount = {
      account_id: 'a-local',
      name: 'Personal',
      slug: 'jay',
      account_role: 'admin',
    };
    expect(resolveDefaultCreatableAccountId([team, byLocal], 'jay@zed.ai')).toBe('a-local');
  });

  test('falls back to is_primary_owner when nothing matches the email', () => {
    const primary: ZedAccount = {
      account_id: 'a-primary',
      name: 'Me',
      account_role: 'owner',
      is_primary_owner: true,
    };
    expect(resolveDefaultCreatableAccountId([team, primary], 'other@zed.ai')).toBe(
      'a-primary',
    );
  });
