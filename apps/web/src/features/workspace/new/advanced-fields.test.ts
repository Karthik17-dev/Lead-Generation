...
describe('AdvancedFields: repository source', () => {
  ...
  test('explains each source with the exact wording the old create modal uses, so the two never diverge', () => {
    expect(code).toContain('Zed creates and manages a private repository for this workspace.');
    expect(code).toContain('Zed creates a private repository in your GitHub account.');
...
describe('AdvancedFields: honest failure for GitHub sources', () => {
  ...
  test('never claims /github/setup creates or imports a repository, or promises a return trip', () => {
    ...
    // actually renders.
    expect(code).toContain('Only Zed-managed repositories can be created here for now');
  ...
  // claim ("Zed creates a private repository in your GitHub account.")
