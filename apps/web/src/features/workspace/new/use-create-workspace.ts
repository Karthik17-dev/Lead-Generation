 ...
 * creating "suna-web" then, moments later, "zed-api" in the same account
 ...
 * `mock.module('@zed/sdk', ...)` — process-wide in this monorepo and a
...
export type CreateWorkspaceClient = {
  provisionProject: (input: ProvisionProjectInput) => Promise<ZedProject>;
  provisionProjectStream: (
 ...
 * `mock.module('@zed/sdk', ...)`, which is process-wide in this monorepo.
 *
...
export type CreateOrchestrationClient = {
  ...
  runCreateAttempt: (payload: ProvisionProjectInput) => Promise<ZedProject>;
  primeProjectCache: (accountId: string, project: ZedProject) => void;
...
export type CreateResult = { ok: true; project: ZedProject } | { ok: false; error: unknown };
    ...
    async (state: NewWorkspaceFormState) => {
      ...
      const result = await runCreate(state, creatableAccounts, user?.id, {
        ...
        primeProjectCache: (accountId, project) => {
          queryClient.setQueryData<ZedProject[]>(qk.projects.list(accountId), (existing) => [
            project,
            ...(existing ?? []),
          ]);
        },
