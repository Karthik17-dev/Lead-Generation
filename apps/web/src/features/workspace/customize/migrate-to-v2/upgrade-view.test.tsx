...
describe('UpgradesViewContent — pane chrome', () => {
  ...
  /**
   * The upgrade row carried `bg-zed-base/[0.06]`, `border-zed-base/30`,
   * `shadow-zed-base/20` and `shadow-md`. The design system reserves
   * `zed-*` for state and shadows for overlays: an in-flow panel gets a
  ...
  test('no brand-tinted, shadowed card — in-flow surfaces are flat with a border', () => {
    ...
    // carries `focus-visible:ring-zed-base`, which is the accent doing its
    // job. Asserting on `zed-base` alone would fail on that and tell nobody
    // anything about this row.
    expect(html).not.toContain('bg-zed-base/');
    expect(html).not.toContain('border-zed-base/');
    expect(html).not.toContain('shadow-zed-base/');
