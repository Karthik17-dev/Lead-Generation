...
import { contract, qk, refreshProjectProviderState, useProjectConfig } from '@zed/sdk/react';
import {
 ...
 *   lock      — managed by Zed, you cannot edit it
...
function SecretMarks({ row }: { row: SecretRow }) {
  ...
  return (
    <span className="flex shrink-0 items-center gap-1">
      {row.system ? (
        <Hint label="Managed by Zed" side="top">
          <Lock
            className="text-muted-foreground size-3.5 shrink-0"
            aria-label="Managed by Zed"
          />
        </Hint>
      ) : null}
      {row.requirement === 'required' ? (
        <Hint label={row.configured ? 'Required' : 'Required — not set'} side="top">
          <Asterisk
            className={cn(
              'size-3.5 shrink-0',
              row.configured ? 'text-muted-foreground' : 'text-zed-orange',
            )}
      ...
      <TableCell className="max-w-[220px] align-middle whitespace-normal">
        <div className="flex flex-col items-start gap-1">
          ...
          {shouldWarnMissingAgentGrant(row.deliveryBlockedReason, row.strategy) && (
            <span className="text-zed-orange text-[11px] font-medium">No agent grant</span>
          )}
        </div>
      </TableCell>
      <TableCell>
                ...
                >
                  ...
                  {grantPlan.candidates.length === 0 && (
                    <span className="text-muted-foreground mt-2 block text-xs text-pretty">
                      This project declares no agent. Add one to zed.yaml first.
                    </span>
                  )}
                  {selectedGrantHint && (
                    <span className="text-muted-foreground mt-2 block text-xs text-pretty">
                  ...
                  {grant.error && (
                    <span className="text-zed-red mt-2 block text-xs text-pretty">
                ...
                <div className="border-border bg-sidebar space-y-4 rounded-md border p-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-xs text-pretty">
                      Zed adds this value to matching HTTPS requests outside the sandbox.
                    </p>
                  </div>
                  ...
                  <Field>
                    ...
                    />
                    <FieldDescription>
                      Zed replaces this header only for the allowed hosts.
                    </FieldDescription>
                  </Field>
                  ...
                  <Field>
                    ...
                    />
                    <FieldDescription>
                      Optional. Include {'{{secret}}'} where Zed inserts the value. Leave it
                  ...
                  {brokerConsumer === 'http_broker' && (
                    <>
                      ...
                      <Field>
                        ...
                        />
                        <FieldDescription>
                          One exact host or leading wildcard per line. Zed denies every other
                          host.
                        </FieldDescription>
                      </Field>
                      ...
                      {injectionKind === 'header' && (
                        <Field>
                          ...
                          />
                          <FieldDescription>
                            Optional. Include {'{{secret}}'} where Zed inserts the value. Leave
