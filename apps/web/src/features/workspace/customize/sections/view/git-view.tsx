 ...
 * the difference between a user pasting `zed env pull` and pasting a line
...
const TONE_DOT: Record<ConnectionTone, string> = {
  connected: 'bg-zed-green',
  attention: 'bg-zed-orange',
 ...
 * `ZedProject.repo_url` is a required column, and a project can carry one
 ...
 * and the two settings that change how Zed uses it.
 *
  ...
  return (
    <section className="space-y-3">
      {connection?.last_error_message ? (
        <InfoBanner tone="warning" icon={WarningIcon} title="Zed can't reach this repository">
          {connection.last_error_message}
        </InfoBanner>
      ) : null}
      ...
      <SettingsRowGroup>
        ...
        <SettingsRow
          ...
          description={
            <>
              The file in your repository that tells Zed how to run this workspace.{' '}
  ...
  return (
    <section className="space-y-3">
      ...
      <ol className="bg-popover divide-border divide-y overflow-hidden rounded-md border">
        <Step
          index={1}
          title="Install the Zed command line"
        ...
        >
          <CommandLine value={`zed projects clone ${projectId}`} label="Clone command" />
        </Step>
        <Step
          index={3}
        ...
        >
          <CommandLine value="zed init --force" label="Setup command" />
          <CommandLine value="zed env pull" label="Secrets command" />
        </Step>
      </ol>
    </section>
  );
 ...
 *
 * This is the old "Zed proxy origin" section. It is behind a disclosure now
 ...
 * unusual about it (no token to create or store), not with how Zed resolves
 * the credential.
 */
          ...
          >
            <span className="min-w-0 flex-1 space-y-0.5">
              ...
              <span className="text-muted-foreground block text-xs font-normal text-pretty">
                Clone with plain <code className="font-mono">git</code> instead of the Zed
                command line.
              </span>
            </span>
            <CaretDownIcon
        ...
        <DisclosureContent variant="outline" contentClassName="border-border border-t">
          <div className="space-y-2 px-4 py-3.5">
            <p className="text-muted-foreground text-xs text-pretty">
              Clone from this address with any Git client. Zed signs the request for you, so
...
export function GitView({ projectId }: { projectId: string }) {
  ...
  // `detail.data.project` is a full `ZedProject`, so `RepositoryGroup` reads
