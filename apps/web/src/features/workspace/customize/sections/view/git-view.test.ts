import type { ProjectGitConnection } from '@zed/sdk';
...
test('states the provider as a sentence, and says Code Storage is stored not hosted', () => {
  ...
  // Code Storage is Zed's own storage, not a third-party host the user has
  // an account with — "Hosted on" would send them looking for a login.
  expect(providerSentence('code-storage')).toBe('Stored in Zed Code Storage.');
});
...
test('the comment-stripped view of the source can still fail', () => {
  ...
  expect(source).toContain('Zed proxy origin');
  expect(code).not.toContain('Zed proxy origin');
});
...
test('does not name internal mechanisms in user-facing copy', () => {
  // "Zed proxy origin" / "resolves the current provider credential just in
...
test('an address on a host with no web page is shown, not linked, and not called unlinked', () => {
  ...
  const out = renderRepositoryValue({
    connection: null,
    repoUrl: 'https://zed.code.storage/9f3a.git',
  });
  expect(out).toContain('https://zed.code.storage/9f3a.git');
  expect(out).not.toContain('<a ');
  expect(out).not.toContain('Not linked yet');
});
...
test('projectRepoFallback never guesses a URL for a host it cannot place', () => {
  expect(projectRepoFallback('https://zed.code.storage/9f3a.git')).toBeNull();
