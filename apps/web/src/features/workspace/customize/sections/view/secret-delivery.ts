...
export type MissingAgentGrantNotice = {
  title: string;
  body: string;
  /** zed.yaml snippet that grants the secret to one agent. */
  manifest: string;
};
...
export function missingAgentGrantNotice(identifier: string): MissingAgentGrantNotice {
  return {
    ...
    body: `List ${identifier} under an agent's secrets in zed.yaml, then run the session with that agent. "secrets: all" does not grant this delivery mode — only an explicit list does.`,
...
/** The zed.yaml edit that grants the secret, for anyone editing the repo by hand. */
...
export function agentGrantConfirmation(identifier: string, agent: string): AgentGrantConfirmation {
  return {
    title: 'Start governing agents in zed.yaml',
    body: `This project declares no agents yet, so every agent receives every project secret. Granting ${identifier} to ${agent} writes the first agents: block. From then on an agent that is not listed receives no project secrets — including the sandbox secrets that work today. Add the other agents to zed.yaml to keep their access.`,
...
export function agentGrantOutcome(result: {
  ...
  if (result.already_granted) {
    return {
      ...
      description: 'zed.yaml was not changed.',
    };
  }
  return {
    tone: 'success',
    ...
    description: result.adopted_governance
      ? 'zed.yaml now declares agents. An agent that is not listed receives no project secrets.'
      : undefined,
  };
}
...
export function agentGrantErrorMessage(error: unknown): string {
  ...
  if (fields.code === 'manifest_v1_unsupported') {
    return 'This project uses a zed_version 1 manifest. Edit zed.toml directly, or upgrade the project to zed_version 2.';
  }
