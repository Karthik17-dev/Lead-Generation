...
/**
 * The Slack hero's cover art — the `Slack × Zed` lockup, on the Zed
 ...
 * real brand SVGs on chips over the Zed gradient with the grain overlay,
...
export function SlackConnectCover({ className }: { className?: string }) {
  return (
    ...
    // just `logos`/`withZed`/`className` — widening them to spread div
    // attributes would change a component the blog renders, for one caller.
    //
    // Decorative, deliberately: the lockup says "Slack and Zed", and the
