...
import { wireToModelKey } from '@zed/sdk/react';
...
describe('the identity a badged row is decided by', () => {
   ...
   * synthetic `zed` provider rather than splitting it. If that ever changes
  ...
  test('wireToModelKey keeps the whole wire id in modelID', () => {
    expect(wireToModelKey('glm-5.2')).toEqual({ providerID: 'zed', modelID: 'glm-5.2' });
  });
