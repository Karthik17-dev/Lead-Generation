...
const TEAMS_MANIFEST_STEPS = [
  'Grant admin consent so the Zed bot can run in your tenant.',
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-1">
        <h3 className="text-foreground text-sm font-medium">Add Zed to Microsoft Teams</h3>
        <p className="text-muted-foreground text-sm leading-relaxed">
          Install the Zed app into your Teams tenant, then bind this project to that tenant.
        </p>
      </div>
      ...
      {managedAvailable ? (
        <div className="flex items-center justify-between gap-4">
          <Label htmlFor="teams-byo" className="text-muted-foreground text-sm font-normal">
            Use your own Azure bot instead of the managed Zed bot
          </Label>
          <Switch id="teams-byo" checked={byo} onCheckedChange={setByo} />
        </div>
      ) : (
        <InfoBanner tone="neutral">
          No managed Zed Teams bot is configured on this server. Register a multi-tenant Azure
