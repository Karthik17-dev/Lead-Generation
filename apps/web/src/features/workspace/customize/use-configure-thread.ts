...
import { writeStartStash } from '@zed/sdk/react';
...
const NEW_PROMPTS: Record<ConfigureKind, string> = {
  agent:
    ...
    '`.zed/opencode/agents/<name>.md` and open a change request so I can review and merge it.',
  skill:
    ...
    '`.zed/opencode/skills/<name>/SKILL.md` and open a change request so I can review and merge it.',
  command:
    'I want to create a new slash command for this project. Ask me what it ' +
    'should do, then add it at `.zed/opencode/commands/<name>.md` and open a ' +
    ...
    (prompt: string) => {
      ...
      newSession({
        onNavigate: (sessionId) => {
          // `sessionId` is the route/Zed session id here, not the OpenCode
