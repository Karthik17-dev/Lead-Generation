 ...
 * collapse toggle.
 *
 * That first control used to be three. A `<Link>` carrying the Zed mark was
 ...
 * design in prose — "a `<Link>` carrying the Zed mark" — so an unstripped
...
describe('project sidebar header', () => {
  ...
  test('no standalone Zed mark button beside it', () => {
    expect(headerCode).not.toContain('<Icon.Zed');
