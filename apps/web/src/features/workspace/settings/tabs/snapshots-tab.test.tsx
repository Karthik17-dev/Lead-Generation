...
import type { ProjectSnapshotBuild, SandboxRuntimeStatus } from '@zed/sdk';
...
describe('every build row can be opened', () => {
  ...
  test('the snapshot name is not on the collapsed row — it lives in the details', () => {
    expect(renderBuildRow('automatic', { status: 'ready' })).not.toContain('zed-tpl-abc123');
    expect(renderBuildDetails('automatic', { status: 'ready' })).toContain('zed-tpl-abc123');
  });
});
