 ...
 *
 * The copy is de-jargoned to match: `source` renders as "Zed platform" /
 * "This dashboard" / `zed.yaml`, not `platform` / `UI` / `zed.yaml`;
 ...
 *  repo can act on. Replaces the raw `platform` / `UI` / `zed.yaml` tag
...
export function describeSource(
  ...
  if (template.source === 'platform')
    return { label: 'Zed platform', icon: ShippingContainerIcon, mono: false };
  if (template.source === 'ui')
    return { label: 'This dashboard', icon: SquaresFourIcon, mono: false };
  return {
    label: manifestVersion === 2 ? 'zed.yaml' : 'zed.toml',
    icon: FileCodeIcon,
    mono: true,
  };
}
...
export function describeBase(template: SandboxTemplate): {
  ...
  return { label: 'Base image', value: 'Zed default', icon: CubeIcon, mono: false };
}
...
export interface SandboxTabViewProps {
   ...
   *  `zed.toml` or `zed.yaml`. `null`/anything else falls back to
   *  `zed.toml`, matching `sandbox-view.tsx`'s own ternary default. */
  manifestVersion?: number | null;
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            <p className="text-muted-foreground text-sm text-pretty">
              ...
              already checked out at <code className="font-mono">/workspace</code>. The Zed
