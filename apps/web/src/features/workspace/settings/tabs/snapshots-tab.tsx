...
/**
 * The Snapshots tab — what Zed prepared for this project, and when.
 *
 * **The problem this layout solves.** The pane used to open on a bare "Build
 * log" of `zed-tpl-…` strings, provider names and raw stderr, with a
 ...
 *
 * `CATEGORY_LABEL` alone names the failure without explaining it: "Zed
...
export const CATEGORY_HELP: Record<SnapshotErrorCategory, string> = {
  ...
  layer:
    'The image built, but Zed could not install its own tools on top of it. This is usually temporary — try building again.',
  git: 'Zed could not read this project’s repository. Check that the repository connection is still authorised, then build again.',
  tunnel:
    'The new machine could not call back to Zed while it was being set up. This is usually temporary — try building again.',
  provider:
  ...
  runtime:
    'The machine was prepared, but a file Zed needs at start-up was missing from it. Build again; if it repeats, the template’s base image is the place to look.',
  unknown: 'The build stopped on an error Zed could not classify. The full log is below.',
};
...
export function BuildRow({
  ...
  return (
    <li>
      ...
      >
        <DisclosureTrigger>
          <div className="focus-visible:ring-zed-base hover:bg-foreground/[0.03] flex w-full cursor-pointer items-center gap-3 px-4 py-2.5 transition-colors outline-none focus-visible:ring-[0.6px]">
            <span
              className={cn(
            ...
            <div className="min-w-0 flex-1">
              ...
              {/* Where `zed-tpl-…` used to be. The snapshot name identifies
...
> = {
  ...
  not_built: {
    ...
    body: 'Zed prepares a machine the first time you start a session here. Nothing needs to be set up in advance.',
...
const HOW_IT_WORKS: readonly { question: string; answer: string }[] = [
  {
    ...
    answer:
      'A snapshot is a machine that has already been set up: this project’s repository, its dependencies, and the Zed tools, all installed ahead of time. Starting a session copies that prepared machine instead of installing everything from scratch, which is why a session is ready in seconds rather than minutes.',
  },
  {
    question: 'Why do new rows keep appearing?',
    answer:
      'Zed prepares a new machine whenever the recipe changes — a new sandbox template, an edited Dockerfile, or a merged change — and each attempt gets its own row. Older rows are kept as a record. They are history, not a list of things to deal with.',
  },
  {
      ...
      <ul className="space-y-2">
        {HOW_IT_WORKS.map(({ question, answer }) => (
          <li key={question}>
            ...
            >
              <DisclosureTrigger>
                <div className="focus-visible:ring-zed-base hover:bg-foreground/[0.03] flex w-full cursor-pointer items-center gap-3 px-4 py-3 transition-colors outline-none focus-visible:ring-[0.6px]">
          ...
          <>
            ...
            <section className="space-y-4">
              <SettingsSubsectionHeader
                title="Build log"
                description="One row for every time Zed prepared a machine for this project. Open a row for the details."
              />
            ...
            {acceleratorBuilds.length > 0 ? (
              <section className="space-y-4">
                ...
                <SettingsSubsectionHeader
                  ...
                  description="An optional head start: Zed clones this repository in advance so a later session opens faster. A missing or failed accelerator never stops a session."
                />
                <ul className="space-y-2">
