import type { SandboxTemplate } from '@zed/sdk';
...
describe('SandboxTabView', () => {
  ...
  test('reads the manifest hint from zed.toml by default, zed.yaml at manifest v2', () => {
    const v1 = renderToStaticMarkup(<SandboxTabView manifestVersion={1} />);
    expect(v1).toContain('zed.toml');
    expect(v1).not.toContain('zed.yaml');

    const v2 = renderToStaticMarkup(<SandboxTabView manifestVersion={2} />);
    expect(v2).toContain('zed.yaml');
    expect(v2).not.toContain('zed.toml');
  });
...
const template = (overrides: Partial<SandboxTemplate> = {}): SandboxTemplate => ({
  ...
  has_image: true,
  image: 'zedai/sandbox-base:2026.08',
...
describe('template card readability', () => {
  ...
  test('the source tag reads as plain English, not a raw enum', () => {
    expect(describeSource(template({ source: 'platform' }), 2).label).toBe('Zed platform');
    ...
    expect(describeSource(template({ source: 'toml' }), 1).label).toBe('zed.toml');
    expect(describeSource(template({ source: 'toml' }), null).label).toBe('zed.toml');
  });
  ...
  test('the base cell states the real image even for the platform default', () => {
    ...
    expect(describeBase(template({ is_default: true }))).toMatchObject({
      label: 'Base image',
      value: 'zedai/sandbox-base:2026.08',
      mono: true,
    });
    expect(
      describeBase(
        template({
          ...
          image: null,
          has_dockerfile: true,
          dockerfile_path: '.zed/Dockerfile',
        }),
      ),
    ).toMatchObject({ label: 'Built from', value: '.zed/Dockerfile', mono: true });
    ...
    ).toMatchObject({ label: 'Base image', value: 'Zed default', mono: false });
  });
