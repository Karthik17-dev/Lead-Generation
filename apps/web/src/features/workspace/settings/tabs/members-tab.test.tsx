...
const accessRequest = (o: Partial<ProjectAccessRequest>): ProjectAccessRequest => ({
  ...
  requester_email: 'requester@zed.com',
  message: null,
  status: 'pending',
  reviewed_by: null,
  reviewed_at: null,
...
describe('MembersTabView', () => {
  ...
  test('renders one row per member, with an Account role and a Workspace access column', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[
          member({ user_id: 'u1', email: 'owner@zed.com', account_role: 'owner' }),
          member({ user_id: 'u2', email: 'viewer@zed.com', account_role: 'member' }),
        ]}
      />,
    );
    expect(out).toContain('Account role');
    expect(out).toContain('Workspace access');
    expect(out).toContain('owner@zed.com');
    expect(out).toContain('viewer@zed.com');
  ...
  test('the table is the shared bordered `Table`, the same one Secrets uses', () => {
    const out = renderToStaticMarkup(
      <MembersTabView members={[member({ user_id: 'u1', email: 'a@zed.com' })]} />,
    );
  ...
  test('the member count renders beside the People tab label once there are members', () => {
    const withMembers = renderToStaticMarkup(
      <MembersTabView
        members={[member({ user_id: 'u1' }), member({ user_id: 'u2', email: 'b@zed.com' })]}
      />,
    );
  ...
  test('the search field filters the table by email, client-side', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[
          member({ user_id: 'u1', email: 'ada@zed.com' }),
          member({ user_id: 'u2', email: 'grace@zed.com' }),
        ]}
        memberSearch="ada"
      />,
    );
    expect(out).toContain('ada@zed.com');
    expect(out).not.toContain('grace@zed.com');
  ...
  test('a search matching nobody shows an empty state, not a headerless table', () => {
    const out = renderToStaticMarkup(
      <MembersTabView members={[member({ email: 'ada@zed.com' })]} memberSearch="zzz" />,
    );
  ...
  test('an account member with no project access reads as an em dash, not a hidden row', () => {
    const out = renderToStaticMarkup(
      <MembersTabView members={[member({ email: 'none@zed.com' })]} />,
    );
    expect(out).toContain('none@zed.com');
  ...
  test('project invites live on the Invites tab under a plain-language title', () => {
    ...
    expect(out).toContain('pending@zed.com');
  ...
  test('access requests live on the Invites tab under a plain-language title', () => {
    ...
    expect(out).toContain('requester@zed.com');
  ...
  test('an Invites tab with nobody waiting shows an empty state, not a blank panel', () => {
    ...
    expect(out).not.toContain('pending@zed.com');
  });
  ...
  test('account invites live on the Invites tab, under a title distinct from the workspace list', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        ...
        members={[member({})]}
        accountId="acc1"
        accountInvites={[accountInvite({ email: 'joiner@zed.com' })]}
      />,
    );
    expect(out).toContain('Invited to this account');
    expect(out).toContain('joiner@zed.com');
  ...
  test('the account role is a read-only Badge, not a Select, when canUpdateAccountRole is false', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[member({ user_id: 'u1', email: 'other@zed.com' })]}
        currentUserId="viewer"
      />,
    );
    expect(out).not.toContain('Account role for other@zed.com');
    expect(out).not.toContain('role="combobox"');
  });
  ...
  test('the account role becomes a Select for another member when canUpdateAccountRole is true', () => {
    ...
    expect(out).toContain('Account role for other@zed.com');
    expect(out).toContain('role="combobox"');
  });
  ...
  test('a "Remove from account" button renders for another member when canRemoveFromAccount is true', () => {
    ...
    expect(out).toContain('Remove other@zed.com from account');
  });
  ...
  test('no "Remove from account" button when canRemoveFromAccount is false', () => {
    ...
    expect(out).not.toContain('Remove other@zed.com from account');
  });

  test("both account-role controls are hidden on the viewer's own row, even with both permissions", () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[member({ user_id: 'viewer', email: 'self@zed.com' })]}
        currentUserId="viewer"
        canUpdateAccountRole
        canRemoveFromAccount
      />,
    );
    expect(out).not.toContain('Account role for self@zed.com');
    expect(out).not.toContain('Remove self@zed.com from account');
  ...
  test('a busy account row shows a spinner instead of either control', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[member({ user_id: 'u1', email: 'busy@zed.com' })]}
        currentUserId="viewer"
        canUpdateAccountRole
        canRemoveFromAccount
    ...
    expect(out).not.toContain('Account role for busy@zed.com');
    expect(out).not.toContain('Remove busy@zed.com from account');
  });

  test('"Remove from account" is disabled for the account\'s sole owner', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[member({ user_id: 'u1', email: 'owner@zed.com', account_role: 'owner' })]}
        currentUserId="viewer"
        canRemoveFromAccount
      />,
    );
  ...
  test('"Remove from account" is enabled for an owner when a second owner exists', () => {
    const out = renderToStaticMarkup(
      <MembersTabView
        members={[
          member({ user_id: 'u1', email: 'owner1@zed.com', account_role: 'owner' }),
          member({ user_id: 'u2', email: 'owner2@zed.com', account_role: 'owner' }),
        ]}
        currentUserId="viewer"
        canRemoveFromAccount
      />,
    ...
    expect(out).toContain('Remove owner1@zed.com from account');
    expect(out).toContain('Remove owner2@zed.com from account');
  });
