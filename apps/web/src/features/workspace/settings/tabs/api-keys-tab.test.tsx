...
describe('ApiKeysTabView', () => {
  ...
  test('renders the header title and description', () => {
    ...
    expect(out).toContain('Let the Zed CLI, a script, or a CI job use this workspace.');
  });
  ...
  test('opens on the keys pane', () => {
    ...
    expect(out).not.toContain('zed login --token');
  });
  ...
  test('the usage pane carries the snippets and the rules form together', () => {
    ...
    expect(out).toContain('zed login --token');
  ...
  test('the curl example uses this deployment’s own API base', () => {
    const out = renderToStaticMarkup(
      <ApiKeysTabView section="usage" apiBase="https://dev-api.zed.com/v1" />,
    );
    expect(out).toContain('curl https://dev-api.zed.com/v1/projects');
    expect(out).not.toContain('curl https://api.zed.com/v1/projects');
  });

  test('falls back to the public API base when the container has none', () => {
    const out = renderToStaticMarkup(<ApiKeysTabView section="usage" />);
    expect(out).toContain('curl https://api.zed.com/v1/projects');
  });
