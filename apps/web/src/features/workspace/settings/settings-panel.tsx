...
import { getProjectDetail, type ZedProject } from '@zed/sdk';
...
function RailTriggerBody({
  ...
  return (
    <>
      ...
      {showCount ? (
        <Badge variant="zed" size="xs" className={cn('tabular-nums', !horizontal && 'ml-auto')}>
          {count}
        </Badge>
      ) : attention ? (
        <span
