...
import { deleteProjectProviderOAuth, deleteProjectSecret, upsertProjectSecret } from '@zed/sdk';
          ...
          <>
            ...
            {row.connected && untouched && status === 'idle' ? (
              <>
                ...
                >
                  <Check className="text-zed-green size-3.5 shrink-0" weight="fill" />
                </span>
                {onRemoveKey && (
                  <InputGroupButton
                    size="icon-xs"
...
function KeyStatusGlyph({ status }: { status: ProviderKeyStatus }) {
  ...
  return (
    <span role="status" title={label} aria-label={label} className="flex shrink-0 items-center">
      ...
      {status === 'error' && (
        <Warning className="text-zed-red size-3.5 shrink-0" weight="fill" />
      )}
    </span>
  );
}
...
export function ProviderConnect({
  ...
  const searchable = useMemo(
    () => LLM_PROVIDERS.filter((provider) => provider.id !== 'zed'),
    [],
  );
        ...
        description={
          removeEntry ? (
            <span className="text-xs">
              ...
              , for everyone on it. Zed does not keep a copy — you will need the key again to put
              it back.
            </span>
          ) : null
        }
