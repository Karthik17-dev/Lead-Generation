export const ENVIRONMENT_PROTECTION_USERNAME = 'zed';
export const ENVIRONMENT_HEALTH_PATH = '/api/health';
export const ENVIRONMENT_ACCESS_COOKIE = '__Secure-zed_test_access';
...
export async function deriveEnvironmentAccessCookie(password: string): Promise<string> {
  const input = new TextEncoder().encode(`zed-test-access:${password}`);
