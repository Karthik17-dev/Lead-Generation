...
export type BlogPostEntry = {
  ...
  /** Show the Zed mark in the cover lockup (default true). */
  coverZed?: boolean;
  /**
...
const agiReadyArchitecture: BlogPostEntry = {
  ...
  title: 'AGI-ready architecture: what it really means, and how Zed is built for it',
  description:
    "AGI-ready doesn't mean an architecture that produces AGI. It means one that absorbs a 100× capability jump without losing state or granting uncontrolled access. How Zed is built for it.",
  date: '2026-07-17',
  author: 'marko',
  cover: '/banner.png',
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'This is the single most important invariant, and it is the one most “agent platforms” violate. If the model’s context window is where your state lives, you have no state — you have a lucky streak that ends when the session ends. In Zed, everything is files in a git repo. The manifest, the agents, the skills, the connectors, the policies, and the memory are all versioned, diffable, owned files. The model proposes; a trusted subsystem persists.',
    },
    {
      type: 'code',
      code: `# zed.yaml — one file that defines this project.
zed_version: 2
...
triggers:
  - slug: nightly-access-review
    ...
    {
      type: 'p',
      text: 'Authentication answers who the agent is. Authorization answers what it may do — and the model must never be the final authority on the second question. In Zed, every session runs under a scoped identity with a single token carrying claims for principal, project, session, and agent grant. Connector credentials are bound server-side and injected at runtime; they never enter the sandbox environment, the transcripts, or the model’s view.',
    },
    {
      type: 'ul',
      items: [
    ...
    {
      type: 'p',
      text: 'An intelligent agent should never execute directly inside the control plane. In Zed, every session is its own isolated Linux sandbox on its own git branch — a disposable machine the agent owns, with filesystem, network, and process isolation. Thousands run in parallel on the same config without colliding, because none of them share state. Egress and credentials are controlled at the network boundary, and the sandbox assumes the agent-generated code is untrusted even when the model looks reliable.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'A capable system without budget control can create effectively unlimited spend. Zed tracks token, compute, sandbox, and tool cost against per-run and per-objective budgets, with holds and reconciliation so a streamed completion is billed at the rate it actually used. Idle sandboxes are reaped — quiet machines are stopped, not billed forever. Fan-out into hundreds of subtasks stays a controlled, cost-bounded operation rather than an open tab.',
    },
    { type: 'h2', text: 'More capability, not more authority' },
    {
      type: 'p',
      text: 'That is the whole thesis in one line. A 100× model should make Zed do 100× more work — it should not get 100× more access. Authority is capped by identity, policy, sandbox, and review, and none of those are controlled by the model. The capability jumps; the guardrails hold. That is what AGI-ready means in practice, and it is the difference between a platform that survives the next model and one that has to be rebuilt around it.',
    },
    ...
    {
      type: 'p',
      text: 'The companies that win the next decade of AI won’t be the ones with the best model. They’ll be the ones whose platform can take whatever model shows up next and put it to work safely. If that operating layer is what you’re missing, the [introduction](/blog/introducing-zed) and the [company-as-a-repo thesis](/blog/ai-transformation-company-os) are the next reads.',
    },
    {
      type: 'cta',
      title: 'Build for the jump, not the model.',
      body: 'Zed is the Autonomous Company Operating System — open-source, self-hostable, any model. Start one project free.',
    },
  ],
};

const zedVsQm: BlogPostEntry = {
  slug: 'zed-vs-qm',
  title: 'Zed vs QM: two open agent platforms, two different units of work',
  description:
    'QM and Zed both give teams persistent agents, isolated computers, Slack and web access, and self-hosting. The decisive difference is deeper: QM organizes work around people and rooms; Zed organizes it around git-backed projects and reviewable sessions.',
  date: '2026-08-02',
  author: 'marko',
  cover: '/banner.png',
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'This comparison is based on [QM main at `7f2c916`](https://github.com/yc-software/qm/tree/7f2c916360f1797a8ff2a77ce2ce40c5fabab087), its published `@yc-software/qm` `0.1.4` package, and Zed main at `3006838` on August 2, 2026. We read the runtime, deployment code, security model, storage paths, tests, and operator runbooks. This is a source comparison, not a feature-page comparison.',
    },
...
git project -> session -> isolated computer + session branch
    ...
    {
      type: 'p',
      text: 'QM starts with the social graph. A person, Slack channel, group message, or project room gets a durable scope. That scope owns memory, files, credentials, schedules, skills, and a computer. Zed starts with the work graph. A project is a git repository and `zed.yaml`; each session receives a sandbox and a branch, and durable changes return through a change request.',
    },
    {
      type: 'callout',
      text: 'QM is closest to a persistent AI colleague for every person and room. Zed is closest to a versioned operating system where many agents work on isolated branches of the company.',
    },
    { type: 'h2', text: 'What they genuinely share' },
    {
      ...
      items: [
        ...
        '**Background work** — QM has crons and watches; Zed has cron and signed-webhook triggers.',
    ...
    {
      type: 'p',
      text: 'Zed makes the opposite trade. Every session exposes one OpenCode REST runtime through the sandbox daemon. `@zed/sdk` owns session startup, runtime resolution, SSE, files, errors, and the mapping between a Zed session and its native OpenCode conversation. Web, mobile, CLI, and white-label clients use the same contract.',
    },
    {
      type: 'p',
      text: 'QM wins if harness interchangeability is the requirement. Zed wins if every client needs one stable, typed, deeply integrated runtime surface. Zed still routes many model providers through its gateway; it standardizes the **agent runtime**, not the model vendor.',
    },
    ...
    {
      type: 'p',
      text: 'Zed separates operational state from authoritative company configuration. Supabase Postgres stores accounts, projects, sessions, sandboxes, triggers, grants, audit events, usage, and gateway logs. The project repo stores the agents, skills, memory, policies, and runtime configuration people are expected to edit and review. OpenCode state lives outside `/workspace`, so runtime internals do not pollute the company repo.',
    },
    {
      type: 'p',
      text: 'The practical consequence is important. QM makes continuity of the colleague and room the default. Zed makes reproducibility, diff, rollback, and promotion to `main` the default. A QM room can keep accumulating local context. A Zed project can show exactly which agent changed the company and which person accepted it.',
    },
    ...
    {
      type: 'p',
      text: 'Zed assigns a computer to a session. Daytona, Platinum, and E2B implement one provider interface. The session branch and sandbox share the session identity. The control plane extends a bounded sandbox deadline when it observes a turn start, gateway LLM activity, or authenticated preview traffic. A terminal turn shortens that deadline to the idle grace period. Passive traffic from an open conversation tab cannot keep the sandbox alive, and one continuous running stretch is capped at 24 hours. A stopped session can resume on the same provider identity or recover through the provider-specific path.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'Zed treats `@zed/sdk` as a public product boundary. `createZed({ getToken })` returns one client for project and session lifecycle, files, streaming, runtime health, previews, and errors. React hooks, a TypeScript server entry, the real `zed` CLI, mobile, desktop, and the web app build on that package. The API and OpenCode transport are implementation details for host applications.',
    },
    {
      type: 'p',
      text: 'This is one of the clearest selection criteria. Choose QM when you primarily deploy and operate the included collaboration product. Choose Zed when you also need to embed the platform, build another host, automate it from a CLI, or expose project/session primitives to customers.',
    },
...
npm exec qm -- outputs --json`,
    ...
    {
      type: 'p',
      text: 'They could coexist, but no supported connection exists today. QM could own the conversational scope while Zed runs project sessions and change requests behind it. That adapter would have to preserve identity, grants, delivery provenance, and idempotency across both systems. Treat it as a connector project, not a configuration flag.',
    },
...
const introducingZed: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Zed is the **command center for the AI agents that do your work** — one place to build a workforce of agents, connect them to your tools, run them on your terms, and keep every result accountable to a human.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'In Zed, a **project** is one git repository. The repo *is* the project: its files, its history, its agents, its automations, its settings — all of it lives in git. Start fresh with a private repo Zed hosts for you, or bring an existing one on GitHub.',
    },
    {
      type: 'ul',
      items: [
    ...
    { type: 'h2', text: 'zed.yaml: the single source of truth' },
    {
      type: 'p',
      text: 'At the root of every project sits one file: `zed.yaml`. Any repo with a valid manifest at its root *is* a Zed project — that file defines what the project is, what it’s allowed to do, and how it runs. Here’s a real one:',
    },
    {
      type: 'code',
      code: `# zed.yaml — the one file that defines this project.
zed_version: 2
...
const zedVsClaudeCowork: BlogPostEntry = {
  slug: 'zed-vs-claude-cowork',
  title: 'Zed vs Claude Cowork: a desktop assistant, or a company-wide agent platform?',
  description:
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'It’s not just the machine that’s landlocked — it’s the knowledge. In Cowork, each person’s agents, skills, and context stay on their own desktop. In Zed, your agents, skills, and memory are **files in one shared repo**: what one person teaches, every teammate — and every agent — gets, and it compounds over time instead of resetting person by person.',
    },
    { type: 'h2', text: 'The model lock-in tax' },
    {
      type: 'p',
      text: 'Cowork only runs on Anthropic’s models, at Anthropic’s prices. Zed lets you **bring your own key and run any model** — and the savings aren’t small. An open-weight model like **GLM-5.2** runs about **5–7× cheaper** than Claude Opus or GPT on output ($4.40 vs $25–30 per 1M tokens), and models like **DeepSeek** are **50×+ cheaper** on output. Route a cheap model for the bulk of the work and a frontier model only where it earns its keep.',
    },
    {
      type: 'callout',
    ...
    {
      type: 'p',
      text: 'They can even coexist: a power user keeps Cowork on their desktop while the company runs its shared, governed workforce on Zed.',
    },
    {
      type: 'cta',
      title: 'Love agents that do the work? Run a whole fleet — on your own terms.',
      body: 'Connect your tools and hand a Zed agent a real task. Free to start, free to self-host.',
    },
  ],
};
...
const personalAgentsVsCompanyOs: BlogPostEntry = {
  ...
  title: 'Personal AI agents vs a company OS: Zed, OpenClaw, and Hermes',
  description:
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'They share Zed’s core values: open, self-hosted, your models, your data. So why build Zed? Because a **personal agent** and a **company operating system** are different problems — and stretching one into the other is where it gets painful.',
    },
    ...
    { type: 'h2', text: 'Side by side' },
    {
      ...
      rows: [
        ...
        },
        {
          ...
          zed: 'Allow / ask / block per tool, as code',
        },
        {
          dimension: 'Isolated sandbox per task',
          them: 'Optional / personal',
          zed: 'One isolated machine per session, on its own branch',
        },
        {
          dimension: 'Versioned, auditable, reversible',
          them: 'Limited',
          zed: 'Git-backed — full history',
        },
      ],
    },
    { type: 'h2', text: 'When to pick which' },
...
const beyondTheChatBox: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'With a chat assistant, you’re the runtime: you ask, it answers, and you copy-paste between the chat window and your real tools to get anything done. With Zed, you hand off a task and an agent **goes and does it** — 30+ minutes of real, multi-step work across your connected tools, with full context on your company, returning a finished deliverable for review.',
    },
      ...
      rows: [
        {
          ...
          zed: 'Agents act across your tools, end to end',
        },
        {
          dimension: 'Runs a fleet in parallel',
          them: 'One supervised session',
          zed: 'Thousands of isolated agents at once',
        },
        {
          dimension: 'Choose your models',
    ...
    {
      type: 'p',
      text: 'The point isn’t that chat assistants are bad. They’re excellent at what they’re built for, and they belong in the stack. A Zed agent that manages a vendor risk review might start by asking a chat assistant to digest a SOC 2 report — then take that output and run the full workflow. The key is knowing which tool fits which job:',
    },
    {
      type: 'ul',
      items: [
        ...
        '**Code completion in-IDE.** Tools like Claude Code and Cursor are brilliant at diffing, refactoring, and writing code in your editor. Zed agents orchestrate those same tools at scale.',
    ...
    {
      type: 'p',
      text: 'This isn’t "stop using ChatGPT." Use a chat assistant for quick answers, drafting, and thinking out loud. Use Zed for the work that has to actually get done — repeatedly, across your tools, owned by you, running while you sleep. One is a brilliant place to ask. The other is where your company’s work runs.',
    },
    {
      type: 'cta',
      title: 'Go from asking questions to running the work.',
      body: 'Hand a Zed agent a real task and get a finished result back. Free to start, free to self-host.',
    },
  ],
};
...
const secureAiAgentToolAccess: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Zed was built around that boundary. Tool access does not belong in a prompt and raw credentials do not belong in an agent sandbox. In Zed, connections are part of the project operating layer: declared as files, brokered server-side, granted per agent, governed by policy, and reviewed when durable work changes the company. If you want the larger architecture first, read [Introducing Zed](/blog/introducing-zed) or the [company OS post](/blog/ai-transformation-company-os).',
    },
    {
      type: 'p',
...
agents:
  ...
  release-bot:
    ...
    {
      type: 'p',
      text: 'That is the model behind the Zed Connector. Every session gets a scoped Connector token. The agent discovers tools, describes their schemas, and calls them through the Zed API. The gateway enforces the project grant and connector policy, resolves credentials outside the sandbox, runs the request, and audits the call. The [connections guide](/docs/guides/connecting-tools) is explicit: the agent never holds third-party credentials.',
    },
    {
      type: 'callout',
    ...
    {
      type: 'p',
      text: 'That is why Zed frames the product as an Autonomous Company Operating System, not another assistant with more connectors. A company does not need one more place to paste keys. It needs a Git-backed AI command center where the tools, credentials, policies, and agent work are part of the same owned system.',
    },
    {
      type: 'cta',
...
const aiTransformationCompanyOs: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Zed is the **Autonomous Company Operating System**: an AI command center where a workforce of agents does real work, and everything that defines the system is files in one Git repo you own. For consultancies and AI-transformation teams, that matters because the deliverable is no longer a single chatbot. The deliverable is a governed workspace the client can keep running after the pilot. If you want the full product spine first, read [Introducing Zed](/blog/introducing-zed).',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'In Zed, a project is a repo. That repo contains the company’s agents, skills, memory, triggers, connector policy, sandbox definition, and operating instructions. One `zed.yaml` defines how the workspace runs. Every session happens on an isolated branch. Every persistent change comes back through a change request.',
    },
    {
      type: 'code',
      code: `acme-ai-workspace/
├─ zed.yaml              # project, sandboxes, triggers, connectors, policy
├─ .zed/opencode/
...
└─ docs/                    # source-of-truth operating docs`,
    ...
    {
      type: 'p',
      text: 'Those are not abstract use cases for us. Zed runs internal sweeps for production errors, PR review, docs maintenance, weekly briefs, outbound research, and this SEO/blog loop from the same project-native model: agents with skills, memory, tools, triggers, and a reviewed path for durable changes.',
    },
...
const zedVsGlean: BlogPostEntry = {
  slug: 'zed-vs-glean',
  title: 'Zed vs Glean: search or an agent platform that runs work?',
  description:
  ...
  blocks: [
      ...
      rows: [
        {
          ...
          zed: 'Build & run agents that do the work',
        },
        {
          dimension: 'Runs a fleet of agents in parallel',
          them: 'Assistants bolted onto search',
          zed: 'Thousands of agents, isolated sandboxes',
        },
        {
    ...
    {
      type: 'p',
      text: 'They can coexist, too. Plenty of companies will keep Glean as the search layer and run the work itself on Zed — agents that read, decide, and act, with the operating layer they need to do it governed as code. (The desktop side has its own parallel: [how Zed compares to Claude Cowork](/blog/zed-vs-claude-cowork).) If that operating layer is what you’re missing, the [company OS post](/blog/ai-transformation-company-os) and the [secure connector model](/blog/secure-ai-agent-tool-access) are the next reads.',
    },
    {
      type: 'cta',
...
const zedVsPoetic: BlogPostEntry = {
  slug: 'zed-vs-poetic',
  title: 'Zed vs Poetic: both turn workflows into code — the difference is who owns the code',
  description:
    'Poetic compiles your procedures into a purpose-built language it runs for you. Zed keeps the workflow as ordinary code in a repo you own, and gates the boundary where it touches the world. A technical comparison of two answers to the same problem.',
  date: '2026-07-31',
  author: 'marko',
  cover: '/banner.png',
  tags: ['Comparisons', 'Architecture', 'Open Source'],
  ...
  blocks: [
    {
      type: 'lead',
      text: 'Poetic and Zed start from the same observation: an agent that improvises a high-stakes process a thousand times a day will not do it the same way a thousand times. The fix both companies reach for is code — pin the workflow to something you can read, review and re-run. Where the two diverge is what that code is, where it lives, and who ends up holding it. That is the whole comparison, and it is a real one.',
    },
    {
      type: 'logos',
      label: 'Compared here:',
    ...
    {
      type: 'p',
      text: 'Zed does not make a determinism claim, and this post will not pretend otherwise. A Zed session is a model loop. It is more general and it is less repeatable. If your problem is one well-bounded process, run at enormous volume, where a 1% deviation is a regulatory event, Poetic is purpose-built for exactly that and Zed is not.',
    },
    { type: 'h2', text: 'The same instinct, applied one layer down' },
    {
      type: 'p',
      text: 'Zed reaches for code too, but it never compiles anything. There is no Zed language. The workflow is ordinary TypeScript and markdown sitting in a git repo — a `SKILL.md` that tells an agent how your company does a job, a script next to it for the logic that deserves to be pinned down, a `zed.yaml` that declares the connectors, triggers and policies. You read it with `cat`. You review it with `git diff`. You test the script without an agent anywhere near it.',
    },
    {
      type: 'p',
      text: 'What Zed does own is narrower and, we would argue, the part that actually needs owning: the boundary where that code touches the outside world. Every connector call goes through one server-side chokepoint, the Connector gateway, and the typed client is part of [`@zed/sdk`](https://www.npmjs.com/package/@zed/sdk).',
    },
    { type: 'h2', text: 'What "verifiable" means here, concretely' },
    {
      type: 'p',
      text: 'For Poetic, "verifiable" means the compiled procedure executes deterministically. For Zed it means something narrower and different: every action that leaves the sandbox is typed, risk-classified, policy-checked, optionally paused for a human, and written to an audit row — and none of that is enforced by the agent, so none of it can be talked out of by the agent. The gateway is the only path.',
    },
    {
      type: 'p',
    ...
    {
      type: 'code',
      code: `import { createZed } from '@zed/sdk';
...
if (action?.risk !== 'write') {
  throw new Error(\`refusing to run: catalog says \${action?.risk ?? 'unknown'}\`);
    ...
    {
      type: 'p',
      text: 'And here is the shape Poetic targets — read, branch, act — written as a Zed skill script. Note what is not in it: no API key, no polling loop, no prompt. The branching is a plain `if`. The credential is resolved server-side and never enters the sandbox. A gated write returns an authenticated approval URL, ends the request, and resumes the Zed session through a durable callback after one human decision:',
    },
    {
      type: 'code',
      code: `import { type ConnectorCallResult } from '@zed/sdk';
...
for (const dispute of open.data?.disputes ?? []) {
  ...
  if (result.status === 'pending_approval') {
    ...
    break; // Zed resumes this session after the human approves or denies.
  }
  ...
  if (!result.ok) throw new Error(\`close_dispute \${dispute.id}: \${result.reason}\`);
    ...
    {
      type: 'callout',
      text: 'Both snippets above type-check under `strict` against the published `@zed/sdk` source, and the package unit suite covers route selection, the call envelope, error mapping and catalog flattening. The approval handoff, risk classification and audit row are enforced in the gateway, not in this client — a script cannot opt out of them by not calling them.',
    },
    {
      type: 'p',
      text: 'The honest scope note: the Connector client in `@zed/sdk` is a typed client for the action boundary. It is not a workflow compiler and it does not make your workflow deterministic. It makes the *edges* of your workflow legible. The determinism you get is the determinism of the TypeScript you wrote around it. That is a weaker guarantee than Poetic’s and a more general one.',
    },
    ...
    },
    {
      type: 'ul',
      items: [
        '**A project is a git repo.** Not a workspace in our cloud — a repository. `zed init` turns a directory into one; `zed ship` pushes it up and runs it. Clone it and you have the whole thing.',
        '**`zed.yaml` is the manifest.** Connectors, triggers, channels, required secrets, policies and where agent config lives — one file, in your repo, in the diff.',
    ...
    { type: 'h2', text: 'Where this comparison does not favour us' },
    {
      ...
      items: [
        ...
        '**Someone has to write the script.** Poetic drafts the workflow from your recordings and documents, with their team alongside you. Zed expects you to be comfortable in a repo — and an agent will happily write the skill for you, but you still review it.',
    ...
    { type: 'h2', text: 'When to pick which' },
    {
      ...
      zed:
        'you want the workflow to stay yours: agents, skills, policies and memory as files in [one repo you own](/blog/introducing-zed), any model, self-hostable, with a typed and audited boundary on every action agents take [across departments](/enterprise).',
    },
    {
      type: 'p',
      text: 'These are not mutually exclusive, and pretending otherwise would be dishonest. A bank can reasonably run Poetic on dispute adjudication and Zed for everything else the company does — the two answer different questions. If the connector boundary is the part you care about, [the secure tool-access model](/blog/secure-ai-agent-tool-access) goes deeper on the gateway. If it is the architecture, [AGI-ready architecture](/blog/agi-ready-architecture) explains why state lives in files rather than a context window. The other two comparisons — [Claude Cowork](/blog/zed-vs-claude-cowork) and [Glean](/blog/zed-vs-glean) — cover the desktop and the search layer.',
    },
    {
      type: 'cta',
 ...
 *  - SELF-HOST is NOT air-gapped — `zed self-host start` pulls images from
...
const openSourceAiManagementSystem: BlogPostEntry = {
  ...
  title: 'What Zed actually is: the open-source AI Management System, layer by layer',
  description:
  ...
  leadMedia: {
    ...
    sources: [
      { src: '/media/showcase/zed-showcase-1920.mp4', type: 'video/mp4' },
      { src: '/media/showcase/zed-showcase-1280.mp4', type: 'video/mp4' },
    ],
    alt: 'Zed in the browser: a project and its connectors, agents, skills and schedules, then a session working on a cloud computer and returning a finished deck.',
    aspectRatio: '1920 / 1200',
  },
  blocks: [
    {
      type: 'lead',
      text: 'Zed is the open-source AI Management System — the leading open-source alternative to Claude Cowork and ChatGPT Work. One git repo holds the agents, the skills, the memory and the connector config. Every session runs on its own isolated machine, on its own branch. Any model, your keys. Work lands through a change request. Self-host it, or run it on our cloud. This is the long version of that paragraph: every layer, in the order you meet it, with the parts we do not claim marked as such.',
    },
    ...
    {
      type: 'p',
      text: 'Which leaves most companies choosing between a toy and a cage: a single-tenant demo with no isolation, no version history and no permission model, or renting your company back from the lab that keeps your data, your configuration and your model. Zed refuses both. The refusal is not a philosophy — it is six layers, and you can read every one of them.',
    },
    { type: 'h2', text: '01 · One git repo that is the company' },
    {
      type: 'p',
      text: 'A Zed project is a git repository, and that repository *is* the company. `zed.yaml` is the Zed layer: the machine a session boots on, the connectors, the triggers, the secret names, and what each agent may touch. The OpenCode configuration beside it is the runtime the agents think in. Everything past those two files is markdown.',
    },
    {
      type: 'p',
      text: 'So the whole company answers to `grep`. Every agent prompt, every skill, every remembered fact and every grant is a line in a file with an author, a timestamp and a diff, and undo is `git revert`. `zed init` turns any directory into a Zed; `zed ship` checks it compiles, asks for the secrets it is missing, and brings it live.',
    },
    {
      type: 'code',
      code: `# zed.yaml — the Zed layer of the repo.
zed_version: 2
runtime: opencode
default_agent: zed
...
triggers:
  - slug: memory-reflector
    ...
    {
      type: 'p',
      text: 'Merging one is a capability of its own, and it is **default-deny for agents**. An agent gets it only if an admin grants `project.cr.merge` in `zed.yaml`, and widening that grant is itself a change somebody else has to approve. It is not a human-only gate, and we would rather state that precisely than sell you a stronger claim than the code makes.',
    },
    ...
    {
      type: 'p',
      text: 'Both are entries in `zed.yaml`, so the 3am job has an author and a history like everything else, and both inherit exactly the reach of the agent they name. The prompt is a template: a webhook fire renders `{{ body.* }}`, a cron fire renders `{{ cron.schedule }}`, `{{ cron.timezone }}` and `{{ cron.scheduled_for }}`. Every fire is a clean slate by default, or a trigger can re-prompt a session it already owns, keyed off the payload, so one customer keeps one thread.',
    },
...
→ Active host is now cloud`,
    ...
    { type: 'h2', text: 'Side by side' },
    {
      ...
      rows: [
        ...
        },
        {
          ...
          them: 'Not published',
          zed: '`git diff` on every agent, skill and grant',
        },
        {
        ...
        },
        {
          ...
          zed: 'Managed cloud, your VPC, or your own on-prem network',
        },
        {
          dimension: 'Reading the source',
          them: 'Closed',
          zed: 'Open source — clone it and read it',
        },
        {
          dimension: 'Isolation per unit of work',
          them: 'Not published',
          zed: 'One isolated machine and one branch per session',
        },
        {
          dimension: 'How work lands',
          them: 'Inside the product',
          zed: 'A change request you read as a diff first',
        },
        {
          dimension: 'Audit record',
          them: 'Not published',
          zed: 'Recorded on every plan; reading it is its own permission',
        },
        {
          dimension: 'Getting started',
          them: 'A Claude Max plan, or a paid metered ChatGPT plan',
          zed: 'Free to start, free to self-host',
        },
      ],
    },
    {
    ...
    },
    {
      type: 'ul',
      items: [
        '**Not air-gapped.** `zed self-host start` pulls its images over the internet and reaches a sandbox provider, so a fully disconnected install is not a shipped capability. Isolated topologies get scoped directly with us.',
        ...
        '**One harness, one runtime.** Zed runs OpenCode. That is the shipped path, and it is the only one this post describes.',
      ],
    },
    { type: 'h2', text: 'When to pick which' },
    {
...
const theOnlyMoatThatMatters: BlogPostEntry = {
  ...
  description:
    'Every AI product is converging on the same architecture. The only defensible advantage is a data flywheel — a learning loop where every interaction makes your system better. Here is what that means, and why Zed is built for it.',
  date: '2026-08-02',
  author: 'marko',
  cover: '/banner.png',
  ...
  blocks: [
    ...
    { type: 'h2', text: 'How Zed is built for the data flywheel' },
    {
      type: 'p',
      text: 'Zed was designed from the ground up around this thesis. Every architectural decision — the git-native model, the model-agnostic gateway, the isolated sandbox environment, the skill system — is aimed at one thing: enabling your company to build a learning loop that compounds. Here is how each layer works.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'This is the ethos open-source has always represented: platforms that enable more value on top than they capture inside. Zed is open-source, self-hostable, and model-agnostic by design. We want every company that uses Zed to build its own compounding advantage — not to make Zed the only company that gets smarter.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: "Zed is the open-source AI operating system where your company's knowledge compounds. Connect your tools, deploy an agent, and start accumulating your own token capital. Free to start, free to self-host.",
    },
  ],
};
...
const twoKindsOfCapital: BlogPostEntry = {
  ...
  blocks: [
    ...
    },
    {
      ...
      body: 'Zed is the open-source AI OS where your company\u2019s knowledge compounds. Free to start, free to self-host, free to own your learning loop.',
    },
  ],
};
...
const testOfSovereignty: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'h2',
      text: 'Where Zed stands',
    },
    {
      type: 'p',
      text: 'Zed passes every item on the sovereignty checklist. Model-agnostic gateway \u2014 swap any LLM without rewriting your skills. Everything is files in a git repo \u2014 your token capital is versioned, portable, forkable. Fully self-hostable. Open source under a permissive license.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'Zed passes the test of sovereignty. Free to start, free to self-host, free to own your AI future.',
    },
  ],
};
...
const theConvergenceOfAiProducts: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Writer.com raised a $200 million series C. They cloned Zed\u2019s open-source code to build their agent layer. This is not unusual. It is the norm. If you build something useful, someone will copy the architecture. The question is: what happens after they copy it?',
    },
    {
      type: 'callout',
    ...
    },
    {
      ...
      body: 'Zed is built on the feedback loop. Deploy it, use it, and watch it compound. Start building yours.',
    },
  ],
};
...
const frontierEcosystem: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Zed is open-source, self-hostable, and model-agnostic for exactly this reason. We want every company to build its own compounding advantage on top of AI, not rent it from someone else. The data flywheel that makes your company better over time should belong to you, not to a model provider in San Francisco.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'Zed is open-source and built for companies that want to own their AI future. Self-host it, connect your own models, and build something that compounds.',
    },
  ],
};
...
const goodBusinessesDontNeedMoats: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'h2',
      text: 'Where this lands for Zed',
    },
    {
      type: 'p',
      text: 'Zed is open-source. Anyone can clone the repo. Anyone can self-host. Anyone can build a competing product. There is no moat in the VC sense.',
    },
    {
      type: 'p',
      text: 'But the cloud business is real. The brand is real. The open-source community is real. The customer relationships are real. And most importantly, the **data flywheel** is real \u2014 every company that uses Zed builds a compounding advantage that belongs to them, not to us. That is not a moat around Zed. It is a moat around our customers.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'Zed is the platform for companies that want to own their AI learning loop. Self-host it, connect your models, and build something that compounds over time.',
    },
  ],
};
