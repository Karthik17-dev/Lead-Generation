...
const KNOWN_DOM_MUTATION_NOISE_MESSAGES = [
  ...
  // (Zed Frontend prod, application_id 2346967): `InvalidNodeTypeError`,
...
const PAPER_SHADER_NULL_CONTEXT_NOISE_PATTERNS = [
  ...
  // `https://zed.com/projects/1d0153d2-…` (project page), browser Firefox
...
// https://github.com/karthikxa/AI-Management-System/pull/4524). The API bounds every
...
// in first-party Zed code: the `did=undefined` query param is the SDK's
...
// (Zed Frontend prod, application_id 2346967): `SyntaxError`, message
...
// `https://zed.com/auth` (auth page — the consent banner loads there
...
// (`https://zed.com/projects/834686a1-…/sessions/54f7abe9-…`), browser
...
// (Zed Frontend prod, application_id 2346967): `ReferenceError`, message
...
// `https://zed.com/` (marketing homepage), browser iPhone iOS 18.7 Safari
...
// script — NEVER in first-party Zed code — and surfaces as an UNCAUGHT global
...
// `https://zed.com/projects/61df2bc0-…` (project page), browser Chrome on
...
// first-party Zed API surface). Deliberately NOT added to
// `sentry.client.config.ts`'s `ignoreErrors` list — that gate has no frame
// context, so a bare-string match there could swallow a real first-party
 ...
 * automation script, never first-party Zed code. Requires the EXACT message
...
// (Zed Frontend prod, application_id 2346967): `TimeoutError`, message
// `signal timed out`, 24 occurrences / 0 identified users, first 2026-05-16
// (recurring), last 2026-08-12 04:08:45 UTC, mechanism
// `auto.browser.global_handlers.onunhandledrejection` (UNCAUGHT,
// `handled:false`), request URL
// `https://zed.com/projects/…/sessions/…` (session pages), browser Chrome
