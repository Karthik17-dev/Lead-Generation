import { type ZedAccount, type ZedProject, listProjectsForAccount } from '@zed/sdk';
...
export type LandingResolution =
  | { kind: 'project'; project: ZedProject; accountId: string }
...
export async function resolveLandingDestination(input: {
  ...
  const projectLists = new Map<string, ZedProject[]>();
...
function canCreateIn(account: ZedAccount): boolean {
