...
import type { ZedAccount, ZedProject } from '@zed/sdk';
...
function account(id: string, role: 'owner' | 'admin' | 'member'): ZedAccount {
  return { account_id: id, name: id, account_role: role };
}

function project(id: string, accountId: string): ZedProject {
  return { project_id: id, account_id: accountId, name: id } as ZedProject;
}

function fakeClient(
  projectsByAccount: Record<string, ZedProject[]>,
