...
function trigger(overrides: Partial<ProjectTrigger> = {}): ProjectTrigger {
  return {
    slug: 'daily-digest',
    path: 'zed.yaml#triggers.daily-digest',
    name: 'Daily digest',
    type: 'cron',
    agent: 'build',
    model: null,
...
describe('triggerStatus — what it is, not what you can do to it', () => {
  test('enabled reads Active and tints green', () => {
    ...
    expect(status.tileClassName).toContain('zed-green');
    expect(status.iconClassName).toContain('zed-green');
  });
  ...
  test('disabled reads Paused and stays neutral', () => {
    ...
    expect(status.tileClassName).not.toContain('zed-');
    expect(status.iconClassName).not.toContain('zed-');
  });
});
