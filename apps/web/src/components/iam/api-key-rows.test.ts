...
import type { AccountToken, ZedProject } from '@zed/sdk';
import {
...
function serviceAccount(over: Partial<ServiceAccount> = {}): ServiceAccount {
  return {
    ...
    name: 'github-actions',
    description: null,
    // Real shape: `zed_sa_` + 8 characters + `…`, already trimmed server-side.
    public_prefix: 'zed_sa_abcd1234…',
    status: 'active',
    last_used_at: null,
    expires_at: null,
...
function project(over: Partial<ZedProject> = {}): ZedProject {
  return { project_id: 'proj_1', name: 'Checkout API', ...over } as ZedProject;
}
...
describe('shortKeyHint', () => {
  ...
  test("leaves a service account's already-trimmed prefix whole", () => {
    expect(shortKeyHint('zed_sa_abcd1234…')).toBe('zed_sa_abcd1234…');
  });

  test('never doubles the ellipsis when the cut lands on one', () => {
    expect(shortKeyHint('zed_sa_abcd1234…', 19)).toBe('zed_sa_abcd1234…');
    expect(shortKeyHint('zed_sa_abcdefgh…', 10)).toBe('zed_sa_…');
  });
});
...
describe('buildApiKeyRows', () => {
  ...
  test('automation keys are always workspace-wide — scope belongs to their policies', () => {
    ...
    expect(row?.scopeLabel).toBeNull();
    expect(row?.hint).toBe('zed_sa_abcd1234…');
  });
...
describe('filterApiKeyRows', () => {
  ...
  test('search matches the key hint and the scope label too', () => {
    expect(filterApiKeyRows(rows, { search: 'zed_sa_' }).map((r) => r.id)).toEqual(['ci']);
