...
export function parseDaysField(label: string, raw: string, max: number): DaysField {
  ...
  if (parsed > max) {
    return { ok: false, error: `${label}: ${max} days is the most Zed allows.` };
  }
  ...
  return (
    <section className="space-y-4">
      <SettingsSubsectionHeader
        title="Key rules"
        description="Applies to keys people create here. Keys Zed creates for a running session end with that session."
      />
