...
/**
 * Zed mints one of these per session and injects it into the sandbox as
 * `ZED_TOKEN` (`apps/api/src/platform/services/session-sandbox.ts`,
 ...
 * (created from the scope picker in this tab, or by `zed login` inside a
 ...
 * in an ellipsis (`zed_sa_` + 8 characters + `…`, `generateServiceAccountSecret`),
