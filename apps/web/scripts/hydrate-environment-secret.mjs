...
export function renderEnvironmentExports(raw, environment = process.env) {
  ...
  for (const [key, value] of Object.entries(parsed)) {
    if (!ENVIRONMENT_KEY.test(key)) {
      throw new Error(`ZED_ENV_JSON key ${JSON.stringify(key)} is not a valid environment name`);
    }
    if (typeof value !== 'string') {
      throw new Error(`ZED_ENV_JSON key ${JSON.stringify(key)} must be a string`);
    }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const raw = process.env.ZED_ENV_JSON;
