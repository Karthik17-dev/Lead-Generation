# Enterprise onboarding packet — self-hosted Zed

> Customer-facing checklist for onboarding a new enterprise account onto a
> self-hosted Zed deployment. Derived from `docs/runbooks/self-hosting.md`
...
| # | Item | Why | Notes |
|---|---|---|---|
| 1 | **A VPS or cloud VM** (recommended) with Docker Engine + Compose plugin installed | Zed self-host is one generic Docker Compose system; runs on any Linux VPS, EC2, Droplet, bare metal | Minimum: 4 vCPU / 8 GB RAM / 50 GB disk for evaluation; size up for production. `scripts/zed-selfhost-up.sh` installs Docker on a fresh Linux box. |
...
| 3 | **A sandbox provider API key** — Daytona (default), Platinum, or E2B | Agent sessions run in cloud sandbox VMs outside the customer's network; they call back to the Zed API over the public internet | Daytona is the recommended default. |
...
| 7 | **(Enterprise) License entitlement** — issued by Zed sales | Unlocks SSO/SCIM/RBAC/audit (hidden behind 402 without it) | `ENTERPRISE_LICENSE_AVAILABLE=true` (or `--enterprise-license` at `init`). |

## What we do (the 3–4 hour white-glove setup)

### Phase 1 — Provision + reachability (~45 min)

1. **Provision the box.** SSH in; run `bash <(curl -fsSL https://raw.githubusercontent.com/karthikxa/AI-Management-System/main/scripts/zed-selfhost-up.sh) --domain <domain> --email ops@<domain>`. This installs Docker, runs `zed self-host init`, and starts the stack.
2. **Verify reachability.** `curl https://api.<domain>/v1/health` → expect `{"status":"ok","environment":"prod","version":"<version>"}`. `curl https://<domain>` → expect the Zed frontend.
...
4. **Configure the sandbox provider.** `zed self-host env set DAYTONA_API_KEY=... DAYTONA_SERVER_URL=https://app.daytona.io/api DAYTONA_TARGET=us` → `zed self-host start`.
5. **Configure managed-git.** `zed self-host env set MANAGED_GIT_PROVIDER=github MANAGED_GIT_GITHUB_TOKEN=... MANAGED_GIT_GITHUB_OWNER=<customer-org>` → `zed self-host start`.
...
7. **Unlock the enterprise entitlement.** `zed self-host env set ENTERPRISE_LICENSE_AVAILABLE=true` → `zed self-host start`.
8. **Register the IdP.** Self-serve path (recommended): sign in as account owner → Account → Settings → Identity → SAML SSO → Configure → Import IdP metadata. Zed registers the IdP with the self-hosted Supabase Auth server-side — the customer never touches Supabase directly. Entity ID / ACS URL / metadata endpoint are all derived from the customer's own `ZED_DOMAIN`.
...
11. **Connect integrations.** `zed self-host configure` (interactive) or `zed self-host env set ...` for Pipedream connectors (optional), SMTP (if provided), and any customer-specific LLM provider keys.
12. **Restrict account creation** (enterprise default). `zed self-host env set ZED_RESTRICT_ACCOUNT_CREATION=true ZED_PUBLIC_RESTRICT_ACCOUNT_CREATION=true` → `zed self-host start`. (New signups only via SSO/SCIM after this.)
...
14. **Auto-updater policy.** Default: `stable` channel, daily auto-update. For enterprise: consider `ZED_AUTO_UPDATE=false` + a planned maintenance window for controlled updates (curate via the `Promote Self-Host Stable` workflow). State the RPO/RTO explicitly.
15. **Handoff.** Walk the customer's operator through `zed self-host` CLI surface (`init`/`doctor`/`status`/`update`/`rollback`/`env`/`logs`/`configure`), the runbook (`docs/runbooks/self-hosting.md`), and the support path.

## Success criteria (certification checklist)

- [ ] `terraform apply` or bootstrap script clean from scratch (no errors)
- [ ] All containers healthy (`zed self-host status`); migrate exit 0
...
- [ ] Auto-updater timer ran once and no-oped cleanly (or `ZED_AUTO_UPDATE=false` + documented maintenance window)
...
- [ ] Customer's operator can run `zed self-host status`/`logs`/`update`/`rollback` without help
...
- **Rollback** = `zed self-host update --release <previous-version>` (or re-run `Promote Self-Host Stable` with an older version). The auto-updater's `flock` + per-service start-first swap means a failed swap never aborts the rest of the rollout — a degraded outcome self-heals on the next run.

## After onboarding

- **Continuous support.** The customer's operator owns day-2 operations via the runbook. Zed FDE stays available for escalations.
...
- **Learning / education.** Bring Zed into the customer's teams as a practice — the agent/skill/connectors/session/memory model, the change-request workflow, the IAM surface. The goal is the customer's team running Zed autonomously, not depending on FDE for routine work.
