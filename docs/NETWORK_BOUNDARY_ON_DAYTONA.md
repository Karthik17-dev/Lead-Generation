...
**Platinum** sandbox provider: Zed registers the credential with Platinum at session
...
| | Platinum | Daytona |
| --- | --- | --- |
| PID 1 | `pt-init` as **root** | `daytona` as `zed` (uid 1001) |
| own kernel | yes (`kthreadd`, `kworker` visible) | no — Docker, bridge `172.20.0.0/16` |
| agent uid | `zed` (1001) | `zed` (1001) |
...
the funnel and Zed supplying the injection.
...
`strategy:'broker'` + `consumer:'http_broker'` (backend `zed_fetch`) is **provider-agnostic
...
Daytona session: saves clean, `delivery_status: 'available'`, and the in-sandbox `zed` CLI is
...
| transparent to the agent | **yes** | no — must call `zed secrets call` |
...
**Why customers do not have it: reach, not capability.** `zed secrets call` is a string in a
...
session agent ran `zed secrets call BROKER_PROOF https://postman-echo.com/get`:
...
Three facts in one response: the header **arrived at the upstream** (so Zed injected it
...
`zed secrets call` was unreachable in practice because "models reach for tools far more
...
> `BROKER_PROOF`: HTTPS broker. Use `zed secrets call BROKER_PROOF <https-url> [options]`.
> So I need to use `zed secrets call BROKER_PROOF https://postman-echo.com/get`.
...
only for requests routed through `secret_call` / `zed secrets call`. Everything else in
...
1. **Add an MCP tool** for the broker so the model discovers it the way it discovers everything
   ...
   assumed the `zed-connectors` MCP server was a live surface with eight tools on it. It is
   not, for two independent reasons found while implementing:

   - **It was broken.** The daemon registered `zed connector mcp` (singular) while the CLI
   ...
   - **It is disabled.** `ZED_CONNECTORS_MCP_ENABLED` appears in tests and nowhere else —
   ...
   (`/tmp/zed/secret-capabilities.md`), which already names `zed secrets call`.
...
3. ~~**Fix the dead limb**: `broker`/`git_proxy` advertises `delivery_status:'available'`
   ...
   `ZED_GIT_AUTH_TOKEN`) but it is live. And `delivery_status` documents itself, in a
...
### Track B — an enforced Zed egress proxy (gated on the §7 experiment)

Daytona supplies the non-bypassable funnel; Zed supplies everything Platinum's edge does.

```
guest ──(runner iptables: block-all + allow only proxy)──▶ Zed egress proxy ──▶ upstream
                                                            │
...
- **Ephemeral per-sandbox CA**, minted at provision, destroyed with the sandbox. Never a
  long-lived Zed root in a customer image.
...
`zed-default-b539f1be09d3`, control vs treatment, deleted after.
...
- **Enforcement** comes from `networkAllowList` restricted to the Zed proxy's address.
...
Control-vs-treatment on throwaway sandboxes from `zed-default-b539f1be09d3`.
...
throwaway Daytona sandbox (`zed-default-b539f1be09d3`), proxy bundled and executed inside
...
The obvious reading of §7.3 is "stand up a public Zed proxy service": new deployment, new
TLS surface, a stable address to pin, and Zed terminating customer traffic at a brand-new
...
```
guest client ──HTTPS──▶ in-guest shim ──HTTPS──▶ Zed API ──▶ upstream
...
reads the request, and relays it to Zed, which does what it already does today: resolve
the secret, apply the host/method policy, inject, perform the request, redact the echo.

Why this is the better answer here:

- **No new public infrastructure.** Sandboxes already reach the Zed API — it is how every
...
- **Zed does not MITM arbitrary traffic.** Only requests the shim relays reach us, and only
...
The consequence to state plainly: with the allow-list pinned to Zed, a session in boundary
...
implementation. Run inside a **real Zed session** against the **real dev broker**, with an
...
The chain: unmodified client → in-guest shim (terminates TLS, holds nothing) → Zed broker
...
- This session was Platinum-backed (`kaab-demo` follows the dev default). The shim is
  provider-agnostic by construction — it is in-guest code plus HTTPS to the Zed API, and
...
Built as `apps/zed-sandbox-agent-server/src/egress-shim/`. Items 1-3 below were the plan;
...
1. **The shim runs in the daemon.** No new image artifact — the daemon already ships in the
   ...
   rules are the `delivery:'network'` entries of `ZED_SECRET_CAPABILITIES`, and the rest are
   `ZED_PROJECT_ID` / `ZED_API_URL` / `ZED_CLI_TOKEN`. No new API plumbing at all.
...
4. **Optional: the allow-list.** Still not required for the security property — an agent that
   ...
   restriction, a separate feature. It also needs a stable Zed egress address, which
...
**`ZED_CLI_TOKEN`, not `ZED_TOKEN`.** The sandbox holds two credentials and the obvious
one is wrong. `ZED_TOKEN`/`ZED_SANDBOX_TOKEN` is the daemon's own identity and carries no
...
Absent `ZED_CLI_TOKEN`, the shim does not start and says which piece was missing.
...
```
agent curl ──▶ in-guest shim ──▶ Zed broker route ──▶ postman-echo.com
```
...
`ZED_CLI_TOKEN` is in the agent's own shell env (the CLI and git need it) and
...
dev-api and api.zed.com are both Cloudflare-fronted on the same anycast
...
- **Daytona snapshot quota** — 225 of 200 at the time of writing; the bulk is live product
  data (`zed-meta-*` 117, `zed-app-*` 38). Needs a retention policy, not a one-off purge.
...
4. Can `on_echo` be preserved through a Zed proxy without stalling responses, or do we accept
