# One Zed Token + CLI-Centric Platform
...
1. **One token.** The platform has accumulated 17 distinct credential types.
   ...
   mental model is: *one Zed token; on your laptop it is you, inside a
...
| 2 | Personal access token | `zed_pat_` | human, laptop/automation | `zed login`, dashboard (`apps/api/src/accounts/core/tokens.ts`) |
| 3 | Session connector token | `zed_pat_` + `session_id` + `agent_grant` + `service_account_id` (same `account_tokens` table) | launching user ∩ agent grant | every session provision (`mintConnectorToken`, `apps/api/src/platform/services/session-sandbox.ts:108`) |
| 4 | Sandbox daemon token | `zed_sb_` | the daemon machine — no user identity | every session provision |
| 5 | Service account | `zed_sa_` | non-human IAM principal | dashboard, or auto-minted identity-only per `[[agents]]` agent (secret discarded) |
| 6 | User API key | `zed_` | the account, programmatic | dashboard API-keys page |

Non-identity credentials:

| # | Credential | What it is |
| --- | --- | --- |
| 7 | `zed_gw_` gateway key | project-scoped external LLM access (`apps/api/src/llm-gateway/gateway-keys.ts`) |
...
| 9 | `zed_scim_` | IdP provisioning bearer (enterprise) |
| 10 | `zed_tnl_` | tunnel machine credential (Computer connector) |
...
| 13 | `zed_oat_` / `zed_ort_` | Zed-as-OAuth-server access/refresh for external clients |
| 14 | `X-Zed-User-Context` | 60s-TTL HMAC header, signed with the sandbox token (not a bearer) |
...
- **The session connector token already implements the "one token" model**:
  it is a `zed_pat_` row with `project_id`, `session_id`, `agent_grant`,
  `service_account_id`. Locally the same prefix is a plain human PAT. The
  model is right; the sprawl is everything *around* it.
- **`ZED_LLM_API_KEY` is the same connector PAT** — the sandbox already
...
- **Exactly two secrets matter inside a sandbox**: the identity token and the
  ...
  (`ZED_CLI_TOKEN`; `ZED_SANDBOX_TOKEN`/
  `ZED_TOKEN`/`INTERNAL_SERVICE_KEY`/`TUNNEL_TOKEN` — the last three are
...
- **Agent switching is allowed, and identity follows the agent.** The proxy
  ...
  `apps/zed-sandbox-agent-server/src/llm-proxy.ts`) — built for warm-fork
...
- **CLI project resolution already matches the desired model**:
  `--project` → sandbox env → `.zed/link.json` → global default
  (`apps/cli/src/project-link.ts:89`). What's missing is the *always-bound*
  UX: `zed login` never binds a project, and an unbound project is a
  dead-end error (`apps/cli/src/command-helpers.ts:59-64`) instead of an
  interactive picker.

### Known defects found in the audit

1. `zed_sa_` bearers are only accepted by `supabaseAuth`, not
...
3. `ZED_TOKEN` is overloaded: it aliases the *daemon* token today, and the
...
### One identity token: "the Zed token"

One token family (`zed_pat_`, one table: `account_tokens`), defined by
...
agent:      optional — the agent whose grant applies (zedCli/connectors/env)
...
| --- | --- | --- |
| Laptop / `zed login` | you | your role |
...
A sandbox holds **exactly two secrets under exactly two names**: the Zed
...
| `ZED_TOKEN`, `INTERNAL_SERVICE_KEY`+`TUNNEL_TOKEN` in-sandbox aliases | rename to the two canonical names; drop aliases (new sandbox images only — same rollout caveat as the opencode-wedge fix) |
| `zed_` user API key | absorb into the token family: it is a PAT (or SA token) with no narrowing claims |
| `zed_gw_` gateway key | absorb: a project-scoped token whose grant is LLM-only (`zedCli: [], connectors: [], llm: true`) — gateway already accepts PATs, so this is a mint-surface change, not an auth change |
| `zed_sa_` bearer | keep the principal, unify the credential: an SA-owned row in `account_tokens` (`principal_type` column) so minting/listing/revocation/expiry policy is one system |
| Supabase JWT | keep — browser-only, third-party minted, already converges at middleware |

End state: **6 identity types → 2** (Zed token + browser JWT), and the
...
- [x] Accept `zed_sa_` in `combinedAuth` (or document the restriction as
...
- [ ] `token_context` on `/accounts/me` covers every token type; `zed
...
- [x] Delete `ZED_ENFORCE_SESSION_AGENT_LOCK` (the lock becomes
...
- [ ] `account_tokens.principal_type` (`user` | `service_account`); migrate
      `zed_sa_` bearer validation onto the same path.
- [ ] Absorb user API keys: new mints create claims-less PATs; `zed_`
...
- [ ] One dashboard page + one CLI surface (`zed token ls/mint/revoke`)
      for every token the account has issued, with provenance
      (login/session/manual/gateway) visible per row.
- [ ] SDK: one `ZedToken` type; kill per-credential client paths.

### Phase A3 — env var rename completion

- [ ] Sandbox env: exactly `ZED_TOKEN` (identity — the agent's token) and
      `ZED_SANDBOX_TOKEN` (machine). Bake new images accepting both old
...
- [x] `zed login` ends by binding a global default project: if the
      account has projects, interactive picker (single project =
      auto-bind); if none, offer `zed init` / create; `--no-project` and
...
- [x] Unbound project + TTY = inline picker, not a dead end: replace the
      ...
      error).
- [ ] `zed` bare landing shows the bound project prominently (it already
      shows host/account/project context) and says how to switch.
- [ ] `zed projects use` (already interactive) advertised as the switch
...
`zed code` and rejected: sessions are general work sessions, not coding
...
today as `zed sessions new --wait` + `zed sessions connect`; revisit
...
- [ ] Parity gap list (below) converted into `zed` command work items,
...
- [ ] Every CLI verb maps to an IAM action string (the `zedCli` grant
...
1. Session permission/approval + question-prompt replies — a CLI-driven
   ...
   CLI; it hangs. Highest impact. **Shipped: PR #4299** (`zed sessions
...
3. Skills/commands/file-writes as first-class verbs (`zed skills`,
   `zed commands`) — today requires raw git.
4. LLM gateway keys/budgets/usage — agents provisioning sub-tools or
   checking spend.
5. Guided agent-config editing (`zed agents edit` with validation).
...
2. Agent switch = the target agent's grant is applied before any tool executes
   ...
   per prompt, and the connector and Zed-CLI gates read that row at call
...
2. What is the right name for the deferred one-command session verb — and
   should it be the bare `zed` behavior once logged-in+bound
...
2. A sandbox env dump shows exactly two `ZED_*` secrets.
3. Switching agents mid-session: `zed token` inside the sandbox shows the
...
4. A service account can drive every `combinedAuth` route it has policies
   for.
5. Fresh laptop: `curl install → zed login → zed sessions new --wait
   → zed sessions connect` reaches a live cloud session with nothing else
   typed — login already bound the project.
6. CI fails when a new API route lands with neither a CLI mapping nor an
   exemption.
7. `/accounts/me token_context`, `zed token`, and sandbox health report
