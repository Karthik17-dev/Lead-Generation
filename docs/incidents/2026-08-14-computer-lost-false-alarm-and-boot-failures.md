...
sandbox is born with `ZED_REPO_URL` / `ZED_API_URL` pointing at that
...
and bakes its URL into every sandbox's env (`ZED_REPO_URL`,
`ZED_API_URL`, `ZED_LLM_BASE_URL`). Quick tunnels die server-side while
...
`apps/api/.env` sets `INTERNAL_ZED_ENV=dev`, so laptop-created boxes carry
the SAME `zed.env=dev` label as the deployed dev fleet (verified on the live
Daytona box). The orphan sweep (`projects/reaping/orphan-boxes.ts`) stops any
running `zed.env=dev` box older than 60 min that has no row in ITS OWN
...
4. **Tunnel liveness.** `scripts/dev-local.sh`: probe `ZED_URL/v1/health`
   through the tunnel every 60 s; on failure restart cloudflared, update
   `ZED_URL`, and block new provisions with an explicit "local tunnel down"
   error until it passes. Consider a named tunnel for stability.
5. **Own env namespace for laptops.** Set `INTERNAL_ZED_ENV=local` (or
   ...
   labels. Kill switch exists today: `ZED_ORPHAN_BOX_REAP_ENABLED=false`.
...
#   labels {"zed.env":"dev","zed.managed":"true", ...}
