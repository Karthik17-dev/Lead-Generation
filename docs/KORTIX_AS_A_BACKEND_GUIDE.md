# Drive Zed as a Backend

Use a Zed API key to create and manage project sessions from your server.
...
Each session has one Zed owner. Each session also has one project and one
unified cost record.
...
```bash
curl -sS -X POST \
  "$ZED_API_URL/projects/$ZED_PROJECT_ID/sessions" \
...
const session = await zed.project(projectId).sessions.create({
  agent_name: "support",
  opencode_model: "zed/glm-5.2",
...
`createScopedZed` isolates the token and runtime state for one server
...
```ts
const projectHandle = zed.project(projectId);
...
```bash
curl -sS \
  "$ZED_API_URL/projects/$ZED_PROJECT_ID/sessions/$SESSION_ID/scope" \
  -H "Authorization: Bearer $ZED_API_KEY"
```
...
```bash
curl -sS \
  "$ZED_API_URL/usage/session-costs?project_id=$ZED_PROJECT_ID&limit=25&offset=0" \
  -H "Authorization: Bearer $ZED_API_KEY"
```
...
```bash
curl -sS \
  "$ZED_API_URL/usage/session-costs/$SESSION_ID?project_id=$ZED_PROJECT_ID" \
  -H "Authorization: Bearer $ZED_API_KEY"
```
...
const detail = await zed.billing.sessionCosts.get(sessionId, {
  accountId,
  projectId,
});

const sameDetail = await zed.session(projectId, sessionId).cost();
```
...
```ts
const handle = zed.session(projectId, session.session_id);
