...
broker. Zed rejects an unenforceable transparent policy with `400`.
...
the explicit broker API. Zed registers the value with Platinum. Platinum
...
```bash
zed secrets delivery ANTHROPIC_API_KEY egress \
...
- The env builder emits no name for an `egress` row, so
  `ZED_PROJECT_SECRET_NAMES` never lists it.
...
| 2 | An agent `secrets:` list NAMES the identifier | `zed.yaml`, by hand or through the grant route below | `resolveSecretDelivery` withholds the row. No binding. |
...
| The manifest is `zed.toml` | `400` | `manifest_v1_unsupported` |
...
manifest_v1_unsupported`. The message asks for an upgrade to `zed_version: 2`
...
- Plain HTTP to a policy host is refused by the proxy, not by Zed.
...
- A grant against a `zed.toml` project returns `400` with
