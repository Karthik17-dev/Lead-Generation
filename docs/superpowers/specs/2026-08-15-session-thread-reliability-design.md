...
Scope: `apps/web` session chat, `@zed/sdk` session runtime, `apps/api` session lifecycle, `apps/zed-sandbox-agent-server`
...
4. Daemon orphan finalizer: on boot/respawn, an assistant turn with no `time.completed` is aborted via `POST /session/:id/abort` (`apps/zed-sandbox-agent-server/src/main.ts:1016-1023`, `:1339-1350`).
...
- Every prompt can dispose/respawn OpenCode: `syncSandboxEnvForPrompt` posts `refreshModels: true` on every prompt (`apps/api/src/projects/lib/sandbox-env-sync.ts:631`); any env delta triggers `opencode.reloadConfig()` (`apps/zed-sandbox-agent-server/src/routes/env.ts:196-242`). A dispose fires `server.instance.disposed` → producer 3. Post-wake, the env delta is maximal, so the **first send after a wake** is the most likely to fabricate aborts — exactly the reported symptom.
...
- The OpenCode pin is persisted per Zed session and used as the paint key before `/start` resolves (the `persistedPin` path becomes load-bearing everywhere, not only when the web list is warm).
...
- `stopSession` and the reaper call the daemon `/zed/abort` before `provider.stop()`, closing turns with `time.completed` — removing the orphan class at the source.
