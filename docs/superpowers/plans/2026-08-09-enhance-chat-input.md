...
**Tech Stack:** TipTap 3.27 on ProseMirror (both MIT, already installed), `@tiptap/suggestion` + `@tiptap/extension-mention` (MIT, to add), TanStack Query 5 (installed), Floating UI (installed via `@tiptap/react`), Tailwind + Zed tokens, `bun:test`.
...
export const ComposerEditor = forwardRef<ComposerEditorHandle, ComposerEditorProps>(
  function ComposerEditor({ placeholder, disabled, autoFocus, onSubmit, onEmptyChange }, ref) {
    ...
    return (
      <EditorContent
        editor={editor}
        className={cn('zed-composer-editor w-full text-base sm:text-sm', disabled && 'opacity-50')}
      />
    );
  },
);
...
```ts
// menu-items.ts
import type { Agent, Session } from '@zed/sdk/react';
 ...
 *
 * Done from the host rather than by lowering staleTime in @zed/sdk: that
...
@media (pointer: coarse) {
  .zed-composer-toolbar button { min-height: 2.5rem; min-width: 2.5rem; }
}
```
...
export type { ProviderListResponse } from '@zed/sdk/react';
```
