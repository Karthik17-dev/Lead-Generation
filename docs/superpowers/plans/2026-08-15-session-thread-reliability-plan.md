...
- Files: `apps/zed-sandbox-agent-server/src/main.ts` (`inspectRoot` :1285-1308, `confirmTurnOrphaned` :1320-1337), reference `turn-auto-resume.ts:128-146`.
...
- Files: `apps/api/src/projects/lib/sandbox-env-sync.ts:631`, `apps/api/src/sandbox-proxy/pre-prompt-env-sync.ts:121-129,312-324`, `apps/zed-sandbox-agent-server/src/routes/env.ts:196-242`.
...
- Change: persisted OpenCode pin per Zed session is the paint key; IDB/memory paint runs independent of `/start` resolution in all hosts.
...
- Change: best-effort daemon `/zed/abort` (bounded) before `provider.stop()`; stop proceeds regardless of abort outcome.
...
- Files: `apps/zed-sandbox-agent-server/src/main.ts:1008-1075,1185-1218,1291-1307`, `opencode-turn-state.ts:44-58`.
...
- Verify (proposed): `pnpm test` green locally; after merge + Deploy Dev, the three journeys pass against dev.zed.com; deployed SHA confirmed.
