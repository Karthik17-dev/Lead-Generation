...
- Known-good baselines: web `bun test src/` = 5915 pass / 6 fail; SDK
  `pnpm --filter @zed/sdk test` = 1856 pass / 0 fail. The 6 web failures
...
The Zed sandbox proxy dedupes prompt deliveries by
...
across attempts. `pnpm --filter @zed/sdk test` and
`pnpm --filter @zed/sdk typecheck` both green.
